...
parser.add_argument(
    ...
    default=False,
    help=(
        "Keep outbound/confirm unchanged, then replace all return-stage VLM, "
        "hint/arbiter action decisions with a deterministic reverse-breadcrumb "
        "controller. The online stop gate may be retained for a controlled "
        "action-generator ablation. Requires --route_memory."
    ),
)
parser.add_argument("--path_following_lookahead_m", type=float, default=0.65)
parser.add_argument("--path_following_turn_enter_deg", type=float, default=28.0)
parser.add_argument("--path_following_turn_exit_deg", type=float, default=12.0)
parser.add_argument("--path_following_stop_distance_m", type=float, default=0.35)
parser.add_argument("--path_following_command_duration_s", type=float, default=0.5)
...
if args_cli.deterministic_path_following_return:
    if not args_cli.route_memory:
        parser.error("--deterministic_path_following_return requires --route_memory")
    incompatible = [
        name for name in ("hint_action_arbiter", "stuck_recovery")
        if bool(getattr(args_cli, name, False))
    ]
    if incompatible:
        parser.error(
            "--deterministic_path_following_return replaces these return policies; "
            "disable: " + ", ".join("--" + name for name in incompatible)
        )
...
def main():
    ...
    def _flush_v11_consumer_events():
        ...
        if v11_consumer_guard is None or v11_consumer_log_handle is None:
            return
        ...
        if len(events) > v11_consumer_event_index:
            v11_consumer_log_handle.flush()
            v11_consumer_event_index = len(events)

    def _evaluate_v11_consumer(operation, anchor_index):
    ...
    path_follower = None
    if args_cli.deterministic_path_following_return:
        path_follower = DeterministicPathFollower(PathFollowingConfig(
            lookahead_m=float(args_cli.path_following_lookahead_m),
            turn_enter_deg=float(args_cli.path_following_turn_enter_deg),
            turn_exit_deg=float(args_cli.path_following_turn_exit_deg),
            stop_distance_m=float(args_cli.path_following_stop_distance_m),
            command_duration_s=float(args_cli.path_following_command_duration_s),
            # Keep the paper comparator's terminal subsystem unchanged.  With
            # ReturnStopGate enabled, this controller replaces only the return
            # action generator and emits turn/forward commands, never stop.
            allow_stop=not bool(getattr(args_cli, "stop_gate", False)),
        ))
        print(f"[path_follower] enabled: {dataclasses.asdict(path_follower.cfg)}", flush=True)
    ...
    _last_path_following_decision = None
    ...
    while simulation_app.is_running():
            ...
            if num_steps == target_steps:
                if phase in ("outbound", "return"):
                    ...
                    if phase == "return" and args_cli.route_memory:
                        ...
                        if args_cli.deterministic_path_following_return:
                            # Geometry is consumed numerically below; do not build,
                            # inject, commit, or log a language hint on return.
                            proposed_instruction_text, proposed_hint_event = current_instruction_text, None
                        else:
                    ...
                    _last_path_following_decision = None
                    if phase == "return" and path_follower is not None:
                        if _gate_vlm_progress is None:
                            raise RuntimeError("path follower has no route-memory progress estimate")
                        _last_path_following_decision = path_follower.decide(
                            route_agent.outbound_breadcrumbs(),
                            _gate_vlm_progress.current_pose_from_start,
                            _gate_vlm_progress.distance_to_start_m,
                        )
                        vlm_vel_commands = list(_last_path_following_decision.command)
                        time_to_go = float(_last_path_following_decision.duration_seconds)
                        is_parseable = True
                        stream_output = (
                            "[DeterministicPathFollower] "
                            f"{_last_path_following_decision.kind}"
                        )
                        phase_events.append({
                            "step": int(num_steps),
                            "phase": phase,
                            "event": "deterministic_path_following_command",
                            **_last_path_following_decision.as_log_dict(),
                        })
                    else:
                    ...
                    _last_hint_action_decision = None
                    if hint_action_arbiter is not None and phase == "return" and path_follower is None:
                    ...
                    _last_recovery_decision = None
                    if stuck_recovery is not None and phase == "return" and path_follower is None:
                    ...
                    if _last_path_following_decision is None:
                        phase_events.append({
                            "step": int(num_steps),
                            "phase": phase,
                            "event": "vlm_command",
                    ...
                    print(
                        f"[{phase}] policy output: {stream_output}\n"
                    ...
                    if stop_gate is not None and phase == "return":
                        ...
                        if (
                            ...
                            and not _evaluate_v11_consumer(
                                "forced_stop", _v11_stop_anchor_index
                            )
                        ):
                            ...
                            and not _evaluate_v11_consumer(
                                "vlm_stop_veto", _v11_stop_anchor_index
                            )
                        ):
                            _last_gate_decision = dataclasses.replace(
                                _last_gate_decision,
        ...
        if _last_path_following_decision is not None:
            _traj_record["deterministic_path_following"] = _last_path_following_decision.as_log_dict()
