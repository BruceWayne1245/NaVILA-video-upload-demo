...
class RouteMemoryAgent:
    def __init__(
        ...
        **_: object,
        ...
        # Dense, non-privileged breadcrumb polyline for the deterministic
        # return-policy ablation.  These poses use the exact same accumulated
        # outbound motion as anchor creation; simulator metadata/world_pose is
        # never consulted by the path follower.
        self._outbound_breadcrumbs: list[list[float]] = [[0.0, 0.0, 0.0]]
        self._breadcrumb_spacing_m: float = min(0.25, max(0.10, self.anchor_spacing_m / 4.0))
        self._breadcrumb_heading_rad: float = math.radians(12.0)
    ...
    def update_outbound_motion(
        ...
        last_breadcrumb = self._outbound_breadcrumbs[-1]
        breadcrumb_delta = relative_delta(last_breadcrumb, self._outbound_pose_from_start)
        if (
            math.hypot(float(breadcrumb_delta[0]), float(breadcrumb_delta[1]))
            >= self._breadcrumb_spacing_m
            or abs(float(breadcrumb_delta[2])) >= self._breadcrumb_heading_rad
        ):
            self._outbound_breadcrumbs.append(list(self._outbound_pose_from_start))
    ...
    def finalize_outbound(
        ...
        final_breadcrumb_delta = relative_delta(
            self._outbound_breadcrumbs[-1], self._outbound_pose_from_start
        )
        if (
            math.hypot(float(final_breadcrumb_delta[0]), float(final_breadcrumb_delta[1])) > 1e-6
            or abs(float(final_breadcrumb_delta[2])) > 1e-6
        ):
            self._outbound_breadcrumbs.append(list(self._outbound_pose_from_start))
    ...
    def outbound_breadcrumbs(self) -> list[list[float]]:
        """Return a defensive copy of the outbound route in start coordinates."""
        return [list(pose) for pose in self._outbound_breadcrumbs]
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "outbound_breadcrumbs": [list(map(float, pose)) for pose in self._outbound_breadcrumbs],
            "return_start_pose_from_start": [float(x) for x in self._return_start_pose_from_start],
            "return_pose_from_return_start": [float(x) for x in self._return_pose_from_return_start],
