...
4. **Spatially-contiguous match-region scoring:** proposed to address the ep994 "wide-basin" pattern more directly than Scan Context's own whole-grid column-average similarity — e.g. scoring the largest *connected* block of agreeing cells rather than the average across the whole polar grid, or a maximum-clique-style consistency check on ICP correspondences (see ROVER/PointDSC/STV-SC above). Deliberately deferred until the P3 flip-fix + threshold-tuning batch shows whether this failure mode is still present once the flip bug is out of the picture.
5. **Scan Context threshold validation:** `min_similarity=0.3`/`min_similarity_margin=0.05` are a reasoned guess from the flip-fix batch's symptoms, not fit to a real similarity-score distribution — needs inspection of actual per-step diagnostics once a clean batch is available.
