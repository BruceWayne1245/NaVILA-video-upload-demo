# NaVILA anchor/terminal/hint-action training data

This directory builds three supervised sequence datasets from saved NaVILA
...
`data/v1/hint_action_decision.jsonl.gz`

- one row per true-far return movement query where VLM and route hint can be
  compared;
- counterfactual labels compare both directions against the oracle route
  direction: `override_hint`, `keep_vlm`, or `abstain`;
- the historical bearing-trust gate is an evaluation baseline, never a label;
- STOP is excluded, and collision clearance remains an independent hard gate.
...
`reports/v1/`. Terminal v2 and wider-candidate pilot results are in
`models/v2/` and `reports/v2/`. See `MODEL_CARD.md` for intended use and
safety limitations.
