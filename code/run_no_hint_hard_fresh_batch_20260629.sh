#!/usr/bin/env bash
# Pure baseline hard batch (2026-06-29).
# Runs the 11 hard episodes with no route hint, no stop gate, and no
# confirm-phase yaw alignment — VLM navigates purely on visual input.
# Compare against:
#   stop_gate_oracle_hard_fresh_20260629  (5/11)
#   direct_oracle_align_yaw_hard_20260629 (5/11)
set -u
...
PORT="${PORT:-54321}"
RUN_TAG="${RUN_TAG:-no_hint_hard_fresh_20260629}"
START_AT="${START_AT:-0}"
...
main() {
  ...
  echo "Config: no route hint, no stop gate, no yaw alignment (pure VLM baseline)" | tee -a "${LOG_DIR}/batch.log"
