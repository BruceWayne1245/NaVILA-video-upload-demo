...
def visual_sequential_pair_relocalization(
    current_descriptor: object,
    current_anchor: object,
    next_anchor: object,
    *,
    diagnostics: Optional[dict] = None,
    matcher_backend: str = "loftr",
    return_candidates: bool = True,
) -> object:
    """Relocalize only against the ordered current/next rear-camera pair.

    Anchor identity is supplied by RouteMemoryAgent's monotonic pair state.  No
    global retrieval or fallback search is performed.  Each saved rear RGB-D
    view is exposed as a standalone visual descriptor and passed through the
    existing feature/depth RANSAC estimator.  This uses RGB correspondences and
    sparse RGB-D geometry; it never reads local LiDAR-map descriptor fields.
    """
    pair = [anchor for anchor in (current_anchor, next_anchor) if anchor is not None]
    estimates = []
    for anchor in pair:
        rear = build_rear_view_descriptor(getattr(anchor, "descriptor", None))
        if rear is None:
            _diagnostic_inc(diagnostics, "visual_pair_missing_rear_rgbd")
            continue
        # Prevent feature_depth_anchor_relocalization from trying the same rear
        # frame a second time through its optional rear-view branch.
        for key in list(rear):
            if key.startswith("rear_"):
                rear.pop(key, None)
        rear["view"] = "rear"
        visual_anchor = copy.copy(anchor)
        visual_anchor.descriptor = rear
        ...
        if result:
            estimates.extend(result if isinstance(result, list) else [result])
    ...
    return max(
        estimates,
        key=lambda estimate: float(estimate.confidence or 0.0)
        * math.sqrt(max(1, int(estimate.inlier_count or 0))),
    )
...
def _nearest_neighbor_2d(source_points: np.ndarray, target_points: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """2026-08-15 (investigations/2026-08-11-.../ICP_PERFORMANCE_AND_KDTREE_
    PROPOSAL.md): was brute-force O(N*M) pairwise distance (chunked, no
    spatial index) -- the dominant per-relocalization-query cost, since this
    runs once per ICP iteration (<=16) inside every one of the 24 yaw-seed
    sweep entries, per candidate anchor. cKDTree.query is EXACT nearest-
    neighbor (not approximate) -- same correspondences a brute-force search
    would return, just found in O(N log M) instead of O(N*M), so this is a
    pure speed win with no expected change to any downstream ICP output
    (theta/translation/median_residual_m/overlap_ratio/ambiguity diagnostics).
    Deliberately does NOT touch the 24-seed count itself -- that sweep is the
    project's actual mitigation for ICP converging to a wrong local minimum
    (the "confidently-wrong ICP" failure mode), a real accuracy trade-off
    that needs separate validation, not a free win like this swap."""
    source = np.asarray(source_points, dtype=np.float32)
    target = np.asarray(target_points, dtype=np.float32)
    if len(source) == 0 or len(target) == 0:
        return (
            np.zeros((len(source),), dtype=np.int32),
            np.full((len(source),), float("inf"), dtype=np.float32),
        )
    tree = cKDTree(target)
    distances, nearest = tree.query(source, k=1)
    return nearest.astype(np.int32), distances.astype(np.float32)
