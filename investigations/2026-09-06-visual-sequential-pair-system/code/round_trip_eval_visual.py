...
from relocalization import (
    ...
    visual_sequential_pair_relocalization,
    voxel_downsample_2d,
...
parser.add_argument(
    ...
    choices=("none", "oracle_anchor", "feature_depth", "sift_depth", "loftr_depth", "lidar_local_map", "scan_context", "fused", "sequential_pair", "visual_sequential_pair"),
    default="none",
...
parser.add_argument(
    "--visual_gt_monitor", action="store_true", default=False,
    help="Record Isaac truth to an analysis-only sidecar; never consumed by control.",
)
...
parser.add_argument(
    "--sequential_pair_closure_cooldown_attempts",
    type=int,
    default=0,
    help=(
        "2026-08-14 (no-yaw-align 100ep batch investigation): the closure "
        "precheck's bearing reconciliation (--sequential_pair_closure_"
        "reconciliation_signal=bearing) computes atan2(dy, dx) of each side's "
        "own reading -- numerically degenerate right when a reading's own "
        "distance is near zero, which is exactly 'current'’s situation for "
        "the first several attempts after every promotion (including return-"
        "start itself, where finalize_outbound() places the robot exactly on "
        "the anchor). Real replay data: bearing disagreement swings 120-179 "
        "deg from sub-decimeter noise while current's own distance is under "
        "~0.5m, then settles once it grows past that. Neither ICP's own "
        "reported distance (unreliable to meter-scale) nor accumulated dead-"
        "reckoning since return-start (docs/odometry_consumers_20260704.md "
        "item 5: the equivalent accumulated-yaw gate measured drifting to 160 "
        "deg within a single episode) are safe signals to gate this on -- "
        "this instead skips (not rejects) the closure comparison for this "
        "many attempts after the anchor most recently became 'current' (reset "
        "on every promotion), using a plain attempt counter with no ICP or "
        "odometry dependency at all. 0 (default) preserves existing behavior."
    ),
)
parser.add_argument(
...
parser.add_argument(
    "--sequential_pair_evict_unreliable_current",
    action="store_true",
    default=False,
    help=(
        "sequential_pair only (2026-08-03, opt-in, off by default; line-2 "
        "Phase 0.2, mechanism B parts (ii)/(iii) -- planned 2026-07-21, never "
        "built until now). --sequential_pair_reliability_demote_current only "
        "waives the quality bar; it does nothing once current is so bad next "
        "can never independently clear close_enough/trend_ok either, or once "
        "_record_next_anchor_trend's quarantine check (which has no "
        "current-unreliable escape hatch at all) starts blacklisting next "
        "candidates off a poisoned current. When on: once current has been "
        "persistently unreliable for --current_evict_stall_attempts "
        "consecutive attempts, force-advance to whichever of {current, next} "
        "is currently more reliable (skipping close_enough/trend_ok -- this "
        "is an escape from an already-degenerate state, not a confirmed "
        "promotion), and _record_next_anchor_trend skips its disagreement "
        "check entirely while current is persistently unreliable. Requires "
        "--sequential_pair_reliability_demote_current (reuses its reliability "
        "bookkeeping)."
    ),
)
parser.add_argument("--current_evict_stall_attempts", type=int, default=30, help=argparse.SUPPRESS)
parser.add_argument(
    "--current_evict_mode",
    type=str,
    default="streak",
    choices=["streak", "window"],
    help=(
        "2026-08-04. 'streak' (default, byte-identical to prior behavior): "
        "--current_evict_stall_attempts CONSECUTIVE unreliable attempts, "
        "reset to 0 by a single reliable-looking one. 'window': a bounded-"
        "window vote over the last --current_evict_window attempts, firing "
        "once --current_evict_min_votes of them were unreliable -- mirrors "
        "--sequential_pair_promotion_window/_min_votes's own pattern, "
        "tolerant of an occasional good-looking blip instead of losing all "
        "accumulated progress to it."
    ),
)
parser.add_argument("--current_evict_window", type=int, default=40, help=argparse.SUPPRESS)
parser.add_argument("--current_evict_min_votes", type=int, default=30, help=argparse.SUPPRESS)
parser.add_argument(
...
parser.add_argument(
    "--sequential_pair_stale_relocalization_distrust",
    action="store_true",
    default=False,
    help=(
        "sequential_pair only (2026-08-04, opt-in, off by default). "
        "route_memory_agent.py's _latest_relocalization/_latest_next_candidate_"
        "relocalization only refresh on an attempt where that role's own "
        "candidate is present, deliberately holding the last value otherwise -- "
        "but nothing downstream could previously tell a value that just "
        "refreshed apart from one silently reused for 1000+ steps because the "
        "anchor stopped producing any candidate at all (root cause of ep368's "
        "stop_gate freeze, line2_phase01_30ep_20260803: ground-truth distance "
        "converged 1.45m->0.26m while the reported anchor-relocalization "
        "distance sat bit-for-bit frozen at 4.52m the entire time). When on, a "
        "reading that has gone --stale_relocalization_max_attempts attempts "
        "without refreshing gets the same confidence-floor-cap + "
        "distance_authority_low_reliability=True treatment Injection C already "
        "gives a bad-U reading -- an independent signal from Injection C's own "
        "(a stale cache can look perfectly confident, it just isn't being "
        "asked anymore)."
    ),
)
parser.add_argument("--stale_relocalization_max_attempts", type=int, default=30, help=argparse.SUPPRESS)
parser.add_argument(
    "--sequential_pair_relocalization_uncertainty_mode",
    type=str,
    default="fixed",
    choices=["fixed", "growing"],
    help=(
        "2026-08-04. 'fixed' (default, byte-identical prior behavior): the "
        "hard-threshold stale-distrust above. 'growing': mirrors the "
        "isolated Route2 branch's [d-0.35, d+0.35] interval that widens by "
        "dead-reckoned travel since the last trusted reading (investigations/"
        "2026-07-28-anchor-support-recovery) -- exposes a continuously-"
        "widening distance_uncertainty_radius_m = "
        "--stale_uncertainty_base_floor_m + (distance traveled since this "
        "role's estimate last refreshed), starting from the FIRST stale "
        "attempt rather than only after --stale_relocalization_max_attempts. "
        "Independent of --sequential_pair_stale_relocalization_distrust (can "
        "be combined with it, or used alone). Consumed by "
        "--stop_gate_use_uncertainty_interval; this flag alone only "
        "populates the field, it does not change any decision on its own."
    ),
)
parser.add_argument("--stale_uncertainty_base_floor_m", type=float, default=0.35, help=argparse.SUPPRESS)
parser.add_argument(
...
parser.add_argument(
    "--sequential_pair_promotion_ambiguity_gate",
    action="store_true",
    default=False,
    help=(
        "sequential_pair only (2026-08-03, opt-in, off by default; line-2 "
        "Phase 1.1, mechanism A). quality_ok/close_enough/trend_ok never look "
        "at this attempt's own absolute confidence or ambiguity flags -- "
        "letting promotions through on partial_pose_degenerate, near-tied "
        "ratio (ambiguous_high_confidence), or confidence as low as 0.45-0.49 "
        "(ep19/88/95 of investigations/2026-07-28-downgrade-batch-mechanism-"
        "failure-classification/FINDINGS.md). When on: a candidate whose "
        "best_to_second_score_ratio is >= "
        "--sequential_pair_current_confidence_ambiguity_gate_threshold (same "
        "ambiguity definition project-wide, no separate threshold), or whose "
        "own confidence is < --sequential_pair_promotion_min_confidence, "
        "cannot cast a promotable vote this attempt -- it is recorded as a "
        "'no' vote via the normal bounded_evidence bookkeeping, same "
        "convention as the closure-mismatch override."
    ),
)
parser.add_argument("--sequential_pair_promotion_min_confidence", type=float, default=0.35, help=argparse.SUPPRESS)
parser.add_argument(
    "--sequential_pair_promotion_anomaly_gate",
    action="store_true",
    default=False,
    help=(
        "sequential_pair only (2026-08-04, opt-in, off by default). The "
        "07-23-proposed but never-built 'cheap, vision-free dynamic anomaly "
        "detection' (investigations/2026-07-23-confidently-wrong-convergence-"
        "and-vision-path/FINDINGS.md): ambiguity_gate only looks at this "
        "attempt's own ratio/confidence, not whether the reading is changing "
        "implausibly fast between attempts -- ep93's real signature (believed "
        "distance collapsed 19.7m->2.6m while true distance only went "
        "14.6m->8.7m over the same window, >2x too fast; an entire "
        "downstream anchor chain got confidently-wrong-matched at once, "
        "avalanche-promoting past both close_enough and quality_ok "
        "simultaneously). When on: a single-attempt bearing jump beyond "
        "--sequential_pair_promotion_anomaly_max_bearing_jump_deg or a "
        "distance collapse beyond --sequential_pair_promotion_anomaly_"
        "max_collapse_m, versus the SAME candidate anchor's own previous "
        "attempt, blocks promotion this attempt -- same 'no' vote override "
        "convention as ambiguity_gate."
    ),
)
parser.add_argument(
    "--sequential_pair_promotion_anomaly_max_bearing_jump_deg",
    type=float, default=90.0, help=argparse.SUPPRESS,
)
parser.add_argument(
    "--sequential_pair_promotion_anomaly_max_collapse_m",
    type=float, default=1.5, help=argparse.SUPPRESS,
)
parser.add_argument(
...
parser.add_argument(
    "--sequential_pair_promotion_model_shadow",
    type=str,
    default="",
    help="(2026-07-28) Off by default (empty path). Path to a PromotionModelBundle artifact "
         "(scripts/promotion_controller_runtime.py) -- a dwell-history-aware promote/quarantine/wait "
         "classifier trained offline on covisibility_records + ground-truth anchor poses (see "
         "investigations/2026-07-28-promotion-quarantine-controller-model/). When set, every "
         "sequential_pair relocalization attempt is ALSO scored by this model and logged to "
         "<result_dir>/promotion_controller_shadow.jsonl alongside what the real heuristic "
         "(candidate_promote/_record_next_anchor_trend/bounded_evidence) actually decided. This is "
         "pure observation: enforcement_enabled/controller_effect are always False in the log, nothing "
         "here can change anchor identity, promotion, quarantine, hints, or motors -- mirrors "
         "--reliability_v11_decision_shadow's shadow-only contract exactly. See "
         "investigations/2026-07-28-promotion-quarantine-controller-model/INTEGRATION_PLAN.md for the "
         "planned Phase 3 (opt-in live enforcement) that would eventually consume this bundle for real; "
         "not implemented yet -- PromotionModelBundle.load() only accepts mode='shadow' today.",
)
parser.add_argument(
    "--sequential_pair_promotion_model_quarantine_threshold",
    type=float,
    default=0.65,
    help="Only used with --sequential_pair_promotion_model_shadow. proba_quarantine threshold for the "
         "shadow PromotionDecisionPolicy (F1-optimal region found offline was 0.6-0.7; see FINDINGS.md "
         "section 4.1). Purely logged in shadow mode unless --sequential_pair_promotion_model_active_promote "
         "is also set, in which case this threshold's quarantine branch still only ever suppresses an "
         "active promote call (see that flag's help) -- it never writes to _quarantined_anchor_indices. "
         "2026-07-31 combined-batch (0728+reliable30v3, 13086 rows) threshold sweep found 0.85 is the "
         "F1-optimal operating point for the quarantine class specifically (precision 0.297->0.560 vs. "
         "this 0.65 default, at a recall cost of 0.737->0.549) -- worth raising if quarantine shadow "
         "logging is what you actually care about tuning; irrelevant to promote/wait quality either way.",
)
parser.add_argument(
    "--sequential_pair_promotion_model_active_promote",
    action="store_true",
    default=False,
    help="(2026-07-31, Phase 3 -- promote/wait ONLY, quarantine stays shadow-only by explicit user "
         "decision). Off by default. Requires --sequential_pair_promotion_model_shadow to also be set "
         "(same bundle is reused). When set, the model's promote-vs-wait call (scored on the exact same "
         "covisibility_records the shadow path already logs) REPLACES the bounded_evidence vote-aggregation "
         "result inside RouteMemoryAgent._select_sequential_pair_relocalization for real -- this is no "
         "longer shadow/observation-only, it changes which anchor gets promoted. quality_ok/close_enough/"
         "trend_ok are deliberately NOT reapplied as a second gate on top of the model (see "
         "route_memory_agent.py's sequential_pair_promotion_model_active_promote docstring for why -- "
         "briefly, quality_ok has a known false-negative failure mode Injection B was added to work around, "
         "and the model's own features already subsume it). closure_reject_reason's cross-check veto and "
         "the short-baseline-unresolved withhold both still apply on top of the model's call, unchanged. "
         "The model's quarantine class, if it fires, only suppresses this attempt's promote (mapped to "
         "'not promote', i.e. same as wait) -- it never adds to _quarantined_anchor_indices; that mechanism "
         "is still exclusively driven by the existing --sequential_pair_quarantine_mode heuristic. See "
         "investigations/2026-07-28-promotion-quarantine-controller-model/INTEGRATION_PLAN.md Phase 3 and "
         "this repo's 2026-07-31 reliable30v3 dwell-based re-validation for why promote/wait (not "
         "quarantine) were judged ready: precision/recall/AUC replicated almost exactly across two "
...
parser.add_argument(
    "--sequential_pair_v11_quarantine_veto",
    action="store_true",
    default=False,
    help="(2026-08-15, investigations/2026-08-15-v11-quarantine-veto). Off by default. Independent from "
         "--sequential_pair_promotion_model_quarantine_veto above (that one is the abandoned from-scratch "
         "promote/wait/quarantine controller; this one consumes Route2's separately-trained, "
         "already-validated V1.1 reliability model's distance head, loaded read-only from "
         "--v11_portable_model_path). Same AND-gate convention as the other veto: only ever blocks a "
         "heuristic-approved quarantine.add(), never initiates one. Fires per-anchor when V1.1 confidently "
         "vouches for that anchor's most recent reading (p_distance_bad_0p5 <= "
         "--v11_veto_max_p_distance_bad). Verified via a 194-reading out-of-sample replay against real "
         "captured ICP data from a batch V1.1 never trained on: rescues 41/87 (47%%) false-positive "
         "quarantines with 0/65 wrongly-rescued genuinely-bad ones. Fails open (no veto opinion, heuristic "
         "unchanged) if the V1.1 repository/artifact at --v11_portable_model_path is unavailable.",
)
parser.add_argument(
    "--v11_portable_model_path",
    type=str,
    default="/home/teambruce/navila-reliability-v1_1/artifacts/reliability_v1_1_portable_shadow.json",
    help="Path to the V1.1 portable (sklearn-free) shadow artifact, only read when "
         "--sequential_pair_v11_quarantine_veto is set.",
)
parser.add_argument(
    "--v11_veto_max_p_distance_bad",
    type=float,
    default=0.5,
    help="Threshold on V1.1's distance-head p_distance_bad_0p5 below which a reading is considered "
         "'confidently good' for --sequential_pair_v11_quarantine_veto's purposes. Default 0.5 matches the "
         "value validated offline (100%% precision, 0/54 wrong, on the 2026-08-15 out-of-sample check).",
)
parser.add_argument(
...
parser.add_argument(
    "--sequential_pair_vision_disagreement_mode",
    choices=["diagnostic", "downgrade"],
    default="diagnostic",
    help="(2026-07-26) Requires --sequential_pair_loftr_rear_yaw_check. 'diagnostic' (default): "
         "purely read-only, matches production behavior exactly. 'downgrade': when ICP self-reports "
         "high confidence (>=0.9, the same threshold used to define 'confidently_wrong' throughout "
         "investigations/2026-07-26-camera-yaw-fix-and-residual-confidence-gate/) while the "
         "independently-gated vision check (Stage-1 margin + Stage-2 residual + Stage-2 "
         "min-translation, same investigation) disagrees by >=45deg, multiplies this attempt's "
         "confidence by --sequential_pair_vision_disagreement_confidence_penalty, letting the "
         "existing low-confidence rejection / candidate-ranking logic decide the consequence rather "
         "than a new independent override. A 400-real-sample offline shadow replay of this exact "
         "precondition found it fires rarely (4.5%% of attempts) with 94.4%% precision (ICP really was "
         "wrong when it fired) -- not yet validated with a live round-trip A/B, so this stays off "
         "(diagnostic) by default.",
)
parser.add_argument(
    "--sequential_pair_vision_disagreement_confidence_penalty",
    type=float,
    default=0.3,
    help="(2026-07-26) Only used when --sequential_pair_vision_disagreement_mode=downgrade and the "
         "flag actually fires. Multiplicative penalty applied to that attempt's confidence.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=False,
    help=(
        "Run the frozen Reliability V1.1 portable model online as a strictly "
        "observational shadow. Predictions and 249 causal features are written "
        "to an independent JSONL file and are never returned to RouteMemoryAgent, "
        "hint, promotion, quarantine, or stop logic."
    ),
)
parser.add_argument(
...
parser.add_argument(
    ...
    default="",
    help=(
        "Frozen shadow-only portable V1.1 JSON artifact. Required with "
        "--reliability_v11_online_shadow."
    ),
)
parser.add_argument(
...
parser.add_argument(
    "--stop_gate_corroboration_overrides_low_reliability",
    action="store_true",
    default=False,
    help=(
        "2026-08-04, opt-in, off by default (Mechanism D fix, investigations/"
        "2026-07-28-downgrade-batch-mechanism-failure-classification/"
        "FINDINGS.md). Injection C's distance_authority_low_reliability check "
        "returns deferred unconditionally BEFORE stop_gate_anchor_corroboration's "
        "own veto check ever runs -- silently disabling that safety net "
        "whenever the same reading is also flagged low-reliability, even "
        "though corroboration is explicitly designed to be trusted "
        "independent of this attempt's own reading quality (FORCE's own "
        "anchor-corroboration already ignores this flag entirely). Root "
        "cause of ep205's documented mismatch: d=10.76m, anchor's own "
        "remaining=7.05m -- both corroboration conditions satisfied, but the "
        "low-reliability defer fired first. When on, corroboration's veto "
        "gets first refusal; only falls through to defer if corroboration "
        "doesn't independently confirm the stop is wrong."
    ),
)
parser.add_argument(
    "--stop_gate_blind_budget",
    action="store_true",
    default=False,
    help=(
        "2026-08-04, opt-in, off by default. Borrows Route2's (a separate, "
        "isolated branch's) 'positive terminal evidence only' principle: a "
        "distance_authority_low_reliability-driven 'deferred' is, in effect, "
        "an unconditional accept of whatever the VLM proposed, with no limit "
        "on how many consecutive times that can happen -- confirmed as a "
        "real cost on this project's own data (investigations/2026-08-01-"
        "anchor0-fix-and-quarantine-veto/FINDINGS.md: the same false 'I'm "
        "home' VLM claim was correctly vetoed once at high confidence, then "
        "let through later purely because confidence happened to read low "
        "that attempt). When on, once --stop_gate_blind_budget_max_attempts "
        "consecutive attempts have had no trustworthy distance authority at "
        "all, an otherwise-deferred VLM-issued stop is treated like a veto "
        "(inject movement, don't accept) instead."
    ),
)
parser.add_argument("--stop_gate_blind_budget_max_attempts", type=int, default=8, help=argparse.SUPPRESS)
parser.add_argument(
    "--stop_gate_use_uncertainty_interval",
    action="store_true",
    default=False,
    help=(
        "2026-08-04, opt-in, off by default. Requires "
        "--sequential_pair_relocalization_uncertainty_mode=growing to have "
        "anything to consume. When on, accept requires the WORST case (d + "
        "distance_uncertainty_radius_m) still <= r_in and veto requires the "
        "BEST case (d - radius) still > r_out, instead of comparing the "
        "point d directly -- an interval straddling either boundary falls "
        "through to deferred (subject to --stop_gate_blind_budget above) "
        "rather than a point comparison confidently picking a side under "
        "growing doubt. Does not touch FORCE or the not-high_conf/anchor-"
        "corroboration branch -- see the flag's own docstring in stop_gate.py."
    ),
)
parser.add_argument(
    "--stop_gate_require_cross_role_agreement",
    action="store_true",
    default=False,
    help=(
        "2026-08-04, opt-in, off by default. current and next roles' ICP "
        "matches are against DIFFERENT anchors' point clouds -- a genuine "
        "independent cross-check, unlike anchor-corroboration's own reuse of "
        "this same reading (which is exactly why the 'confidently wrong' "
        "false-FORCE class -- ep669/93/581/427, investigations/2026-07-22 "
        "through 2026-08-01 -- kept recurring: an entire downstream anchor "
        "chain can get confidently-wrong-matched at once, so the anchor's "
        "own static remaining and this attempt's live reading end up being "
        "the SAME poisoned identity agreeing with itself, not two "
        "independent signals). When on, FORCE and ACCEPTED both "
        "additionally require the OTHER role's own independently-derived "
        "distance to be within --stop_gate_cross_role_max_disagreement_m of "
        "d -- missing data (that role currently has no estimate) is treated "
        "as 'cannot verify,' not 'disagrees,' and does not block."
    ),
)
parser.add_argument("--stop_gate_cross_role_max_disagreement_m", type=float, default=1.5, help=argparse.SUPPRESS)
parser.add_argument(
    "--hint_action_arbiter",
    action="store_true",
    default=False,
...
parser.add_argument(
    "--hint_action_arbiter_stop_veto",
    action="store_true",
    default=False,
    help=(
        "sequential_pair only (2026-08-03, opt-in, off by default; line-2 "
        "Phase 1.2, mechanism E). --hint_arbiter_min_relocalization_confidence "
        "gates both direction-override AND stop-suppression under one bar "
        "(default 0.90) -- below it the arbiter goes silent on an erroneous "
        "voluntary stop too, unless stop_gate's own (separately gapped) "
        "corroboration happens to catch it. When on, a `stop` VLM output gets "
        "its own suppression path using --hint_action_arbiter_stop_veto_min_confidence "
        "(typically well below the direction-override bar) and the anchor's "
        "own static route-remaining distance (not the noisy live reading) "
        "exceeding --hint_action_arbiter_stop_veto_anchor_remaining_min_m -- "
        "the same anchor-corroboration idea stop_gate already uses. Direction-"
        "override behavior above is completely unchanged."
    ),
)
parser.add_argument(
    "--hint_action_arbiter_stop_veto_min_confidence", type=float, default=0.5, help=argparse.SUPPRESS
)
parser.add_argument(
    "--hint_action_arbiter_stop_veto_anchor_remaining_min_m", type=float, default=3.0, help=argparse.SUPPRESS
)
parser.add_argument(
    "--hint_arbiter_turn_override_completes_full_angle",
    action="store_true",
    default=False,
    help="(2026-08-15, investigations/2026-08-15-hint-action-turn-gate-fix). Off by default. When a "
         "left/right override fires, (1) skips the clear-path (occupied_in_local_map_path) gate entirely -- "
         "that check tests straight-line forward clearance, which is not a meaningful safety question for an "
         "in-place turn -- and (2) rotates by the full bearing-derived angle in one command instead of the "
         "fixed --hint_arbiter_turn_step_deg increment, so one gated approval completes the whole correction "
         "instead of needing repeated re-approval every VLM query. forward overrides are unchanged (kept on "
         "the existing small-increment, re-checked-every-query cadence -- an open-loop long forward takeover "
         "would remove VLM's frame-by-frame visual obstacle avoidance, a real collision risk a turn does not "
         "carry).",
)
parser.add_argument(
    "--hint_arbiter_trend_confidence",
    action="store_true",
    default=False,
    help="(2026-08-15, investigations/2026-08-15-hint-confidence-collapse-patterns). Off by default. Ports "
         "stop_gate.py's already-implemented trend_confidence mechanism to hint_action_arbiter's own "
         "relocalization_confidence gate: a reading that fails the single-attempt min_relocalization_"
         "confidence bar can still be trusted if a rolling window of recent readings corroborates it "
         "(>= --hint_arbiter_trend_confidence_min_high_conf_votes independently high-confidence readings in "
         "the last --hint_arbiter_trend_confidence_window, agreeing with this attempt's distance within "
         "--hint_arbiter_trend_confidence_max_distance_spread_m). Targets the sub-case where a reading is "
         "actually accurate but its raw confidence noisily dips below the threshold; does NOT resolve a "
         "genuinely persistent ICP ambiguity at a specific anchor (that doesn't average out).",
)
parser.add_argument(
    "--hint_arbiter_trend_confidence_window", type=int, default=5, help=argparse.SUPPRESS
)
parser.add_argument(
    "--hint_arbiter_trend_confidence_min_samples", type=int, default=3, help=argparse.SUPPRESS
)
parser.add_argument(
    "--hint_arbiter_trend_confidence_min_high_conf_votes", type=int, default=3, help=argparse.SUPPRESS
)
parser.add_argument(
    "--hint_arbiter_trend_confidence_max_distance_spread_m", type=float, default=1.0, help=argparse.SUPPRESS
)
parser.add_argument(
...
def atomic_jsonl_dump(path, rows):
    """Atomically persist a complete JSONL file."""
    destination = os.path.abspath(path)
    parent = os.path.dirname(destination)
    os.makedirs(parent, exist_ok=True)
    temporary = os.path.join(
...
def route_memory_descriptor_from_infos(infos, env=None, include_local_map=True):
...
def main():
    ...
    active_promotion_feature_builder = None
    v11_veto_scorer = None
    if args_cli.sequential_pair_v11_quarantine_veto:
        from v11_veto import V11QuarantineVetoScorer

        # v11_portable_model_path is the full path to the artifact JSON, e.g.
        # <v11_root>/artifacts/reliability_v1_1_portable_shadow.json --
        # V11QuarantineVetoScorer wants the repo root separately (it also
        # needs <v11_root>/candidate/scripts on sys.path for the runtime
        # module), so split it back apart here rather than change the
        # scorer's constructor signature.
        _v11_artifact_dir = os.path.dirname(args_cli.v11_portable_model_path)
        _v11_root = os.path.dirname(_v11_artifact_dir)
        v11_veto_scorer = V11QuarantineVetoScorer(
            v11_root=_v11_root,
            portable_artifact_relpath=os.path.join(
                os.path.basename(_v11_artifact_dir), os.path.basename(args_cli.v11_portable_model_path)
            ),
            max_p_distance_bad=float(args_cli.v11_veto_max_p_distance_bad),
        )
        if not v11_veto_scorer.available:
            print(
                f"[v11-veto] WARNING: failed to load V1.1 artifact from "
                f"{args_cli.v11_portable_model_path} ({v11_veto_scorer._load_error}); "
                f"--sequential_pair_v11_quarantine_veto will fail open (no veto opinion, "
                f"heuristic quarantine behaves exactly as if the flag were off) for this run.",
                flush=True,
            )
        else:
            # 2026-08-15: pure diagnostic addition (mirrors [v11-shadow]
            # initialized's existing pattern) -- confirms the scorer loaded
            # successfully; does not change any decision. Added mid-batch
            # (line2_v11veto_turngate_trendconf_100ep_20260815) -- safe
            # because each episode is a fresh isaaclab.sh subprocess, so only
            # episodes not yet started pick this up; the in-flight one keeps
            # running on its already-imported code.
            print(
                f"[v11-veto] initialized max_p_distance_bad="
                f"{args_cli.v11_veto_max_p_distance_bad} enforcement=quarantine_veto_only "
                "controller_effect=False",
                flush=True,
            )
        v11_veto_scorer.start_episode(
            f"{args_cli.result_suffix or 'round_trip'}::ep{int(args_cli.episode_idx)}"
        )
    if args_cli.sequential_pair_promotion_model_active_promote and not args_cli.sequential_pair_promotion_model_shadow:
        raise RuntimeError(
            "--sequential_pair_promotion_model_active_promote requires "
    ...
    if args_cli.sequential_pair_promotion_model_shadow:
        ...
        promotion_bundle = PromotionModelBundle.load(
            args_cli.sequential_pair_promotion_model_shadow, mode="shadow"
        )
        promotion_episode_key = (
            f"{args_cli.result_suffix or 'round_trip'}::"
            f"ep{int(args_cli.episode_idx)}"
        )
        promotion_decision_policy = PromotionDecisionPolicy(
            quarantine_threshold=args_cli.sequential_pair_promotion_model_quarantine_threshold
        )
        promotion_shadow_session = PromotionShadowJsonlSession(
            promotion_bundle,
            episode_key=promotion_episode_key,
            log_path=os.path.join(result_dir, "promotion_controller_shadow.jsonl"),
            decision_policy=promotion_decision_policy,
        ...
        if (
            ...
            # but sharing one instance would double-append to its rolling
    ...
    if args_cli.reliability_v11_online_shadow:
        ...
        print(
            ...
            "enforcement=False controller_effect=False",
            flush=True,
        )
    ...
    if args_cli.route_relocalization_backend in feature_relocalization_backends:
    ...
    elif args_cli.route_relocalization_backend == "visual_sequential_pair":
        # Ordered-pair visual localization: current/next identity is maintained
        # by RouteMemoryAgent.  This backend never performs global retrieval.
        def _visual_sequential_pair_relocalizer(descriptor, anchors):
            return visual_sequential_pair_relocalization(
                descriptor,
        ...
        route_relocalizer = _visual_sequential_pair_relocalizer
        ...
        def _sequential_pair_relocalizer_with_v11_shadow(descriptor, anchors):
            ...
            result = sequential_pair_anchor_relocalization(
                ...
                *route_agent.sequential_target_anchor_pair(),
                ...
                vision_disagreement_confidence_penalty=args_cli.sequential_pair_vision_disagreement_confidence_penalty,
            )
            if v11_shadow_session is not None:
                new_records = records[record_start:]
                v11_shadow_session.score_records(
                    new_records,
                    step=int(num_steps),
                )
                if new_records:
                    v11_shadow_last_attempt = int(new_records[-1]["attempt"])
            if active_promotion_feature_builder is not None:
                # 2026-07-31 (Phase 3, promote/wait only): scores THIS attempt's
                # records right now, synchronously, so route_agent can consume
                # the decision a few lines below (inside update_relocalization's
                # continuation, same call stack, same attempt) -- unlike
                # promotion_shadow_pending above, this cannot be deferred to
                # after update_return_motion() returns, the real promote
                # decision needs it immediately. Uses its own feature-builder
                # instance (see where it's constructed) so this never
                # double-counts an attempt into promotion_shadow_session's
                # rolling history.
                ...
                if active_new_records:
                    try:
                        ...
                        active_feat = active_promotion_feature_builder.build_attempt(
                            promotion_episode_key, active_attempt, active_new_records
                        )
                        if active_feat is not None:
                            active_proba = promotion_bundle.predict_proba(active_feat)
                            active_decision = promotion_decision_policy.decide(active_proba)
                            # Only "promote" ever sets True -- "wait" and
                            # "quarantine" both fall through to "do not
                            # promote" (i.e. wait) in route_memory_agent.py;
                            # quarantine is never allowed to touch
                            # _quarantined_anchor_indices from here.
            ...
            if v11_veto_scorer is not None:
                # 2026-08-15 (investigations/2026-08-15-v11-quarantine-veto):
                # synchronous, like active_promotion_feature_builder above --
                # route_agent's quarantine mechanisms run inside
                # update_relocalization()'s continuation right after this
                # closure returns, in the same attempt, so the veto dict must
                # be populated before then. Overwritten (not merged) each
                # attempt: a stale True from an earlier attempt must not keep
                # vetoing a quarantine decision made on fresher evidence.
                v11_new_records = records[record_start:]
                if v11_new_records:
                    v11_attempt = int(v11_new_records[-1]["attempt"])
                    route_agent._active_v11_veto_by_anchor = v11_veto_scorer.score_attempt(
                        v11_attempt, v11_new_records
                    )
                    # 2026-08-15: pure diagnostic addition, see the
                    # [v11-veto] initialized comment above -- only prints
                    # when the scorer actually forms an opinion (non-empty
                    # dict), to avoid spamming every attempt.
                    if route_agent._active_v11_veto_by_anchor:
                        print(
                            f"[v11-veto] step={num_steps} attempt={v11_attempt} "
                            f"opinions={route_agent._active_v11_veto_by_anchor}",
                            flush=True,
                        )
                else:
                    route_agent._active_v11_veto_by_anchor = {}
            if promotion_shadow_session is not None:
                new_records = records[record_start:]
                if new_records:
                    promotion_shadow_pending = {
                        "step": int(num_steps),
                        "attempt": int(new_records[-1]["attempt"]),
                        "records": new_records,
                        "idx_before": current_idx_before,
    ...
    relocalization_interval_backends = set(feature_relocalization_backends) | {
        "lidar_local_map", "scan_context", "fused", "sequential_pair", "visual_sequential_pair",
    }
    route_agent = RouteMemoryAgent(
        ...
        sequential_pair_closure_cooldown_attempts=int(
            args_cli.sequential_pair_closure_cooldown_attempts
        ),
        ...
        sequential_pair_evict_unreliable_current=bool(args_cli.sequential_pair_evict_unreliable_current),
        current_evict_stall_attempts=args_cli.current_evict_stall_attempts,
        current_evict_mode=args_cli.current_evict_mode,
        current_evict_window=args_cli.current_evict_window,
        current_evict_min_votes=args_cli.current_evict_min_votes,
        ...
        sequential_pair_stale_relocalization_distrust=bool(
            args_cli.sequential_pair_stale_relocalization_distrust
        ),
        stale_relocalization_max_attempts=args_cli.stale_relocalization_max_attempts,
        sequential_pair_relocalization_uncertainty_mode=args_cli.sequential_pair_relocalization_uncertainty_mode,
        stale_uncertainty_base_floor_m=args_cli.stale_uncertainty_base_floor_m,
        ...
        sequential_pair_promotion_ambiguity_gate=bool(args_cli.sequential_pair_promotion_ambiguity_gate),
        sequential_pair_promotion_min_confidence=args_cli.sequential_pair_promotion_min_confidence,
        sequential_pair_promotion_anomaly_gate=bool(args_cli.sequential_pair_promotion_anomaly_gate),
        promotion_anomaly_max_bearing_jump_deg=args_cli.sequential_pair_promotion_anomaly_max_bearing_jump_deg,
        promotion_anomaly_max_collapse_m=args_cli.sequential_pair_promotion_anomaly_max_collapse_m,
        ...
        sequential_pair_v11_quarantine_veto=bool(args_cli.sequential_pair_v11_quarantine_veto),
    ...
    if getattr(args_cli, "stop_gate", False):
        stop_gate = ReturnStopGate(
            ...
            corroboration_overrides_low_reliability=bool(
                getattr(args_cli, "stop_gate_corroboration_overrides_low_reliability", False)
            ),
            blind_budget_enabled=bool(getattr(args_cli, "stop_gate_blind_budget", False)),
            blind_budget_max_attempts=int(getattr(args_cli, "stop_gate_blind_budget_max_attempts", 8)),
            use_uncertainty_interval=bool(
                getattr(args_cli, "stop_gate_use_uncertainty_interval", False)
            ),
            require_cross_role_agreement=bool(
                getattr(args_cli, "stop_gate_require_cross_role_agreement", False)
            ),
            cross_role_max_disagreement_m=float(
                getattr(args_cli, "stop_gate_cross_role_max_disagreement_m", 1.5)
            ),
        )
        print(
            ...
            f"corroboration_overrides_low_reliability="
            f"{stop_gate.corroboration_overrides_low_reliability} "
            f"blind_budget_enabled={stop_gate.blind_budget_enabled} "
            f"blind_budget_max_attempts={stop_gate.blind_budget_max_attempts} "
            f"use_uncertainty_interval={stop_gate.use_uncertainty_interval} "
            f"require_cross_role_agreement={stop_gate.require_cross_role_agreement} "
            f"cross_role_max_disagreement_m={stop_gate.cross_role_max_disagreement_m}",
            flush=True,
        )
    ...
    if getattr(args_cli, "hint_action_arbiter", False):
        hint_action_arbiter = HintActionArbiter(HintActionArbiterConfig(
            ...
            turn_override_completes_full_angle=bool(
                getattr(args_cli, "hint_arbiter_turn_override_completes_full_angle", False)
            ),
            trend_confidence_enabled=bool(getattr(args_cli, "hint_arbiter_trend_confidence", False)),
            trend_confidence_window=int(getattr(args_cli, "hint_arbiter_trend_confidence_window", 5)),
            trend_confidence_min_samples=int(
                getattr(args_cli, "hint_arbiter_trend_confidence_min_samples", 3)
            ),
            trend_confidence_min_high_conf_votes=int(
                getattr(args_cli, "hint_arbiter_trend_confidence_min_high_conf_votes", 3)
            ),
            trend_confidence_max_distance_spread_m=float(
                getattr(args_cli, "hint_arbiter_trend_confidence_max_distance_spread_m", 1.0)
            ),
            ...
            stop_veto_enabled=bool(getattr(args_cli, "hint_action_arbiter_stop_veto", False)),
            stop_veto_min_confidence=float(
                getattr(args_cli, "hint_action_arbiter_stop_veto_min_confidence", 0.5)
            ),
            stop_veto_anchor_remaining_min_m=float(
                getattr(args_cli, "hint_action_arbiter_stop_veto_anchor_remaining_min_m", 3.0)
            ),
        ))
        print(
    ...
    visual_gt_monitor = None
    if args_cli.visual_gt_monitor:
        visual_gt_monitor = VisualGroundTruthMonitor(
            os.path.join(result_dir, "visual_ground_truth"),
            episode_id=int(args_cli.episode_idx),
            scene_id=os.path.splitext(
                os.path.basename(str(episode.get("scene_id") or "unknown"))
            )[0],
        )
    ...
    while simulation_app.is_running():
                ...
                if phase in ("outbound", "return"):
                    ...
                    if hint_action_arbiter is not None and phase == "return":
                        ...
                        if _last_hint_action_decision.override:
                            ...
                            if _last_hint_action_decision.override_command is not None:
                                # 2026-08-15: turn_override_completes_full_angle emits the
                                # real (unsnapped) bearing in replacement_output for logging,
                                # but get_vel_command() only recognizes 15/30/45 substrings --
                                # a real angle like "31 degree" would silently fall through to
                                # its small default. Bypass parse_vlm_command entirely and use
                                # the arbiter's own precomputed command/duration instead.
                                vlm_vel_commands = _last_hint_action_decision.override_command
                                time_to_go = _last_hint_action_decision.override_duration_s
                                is_parseable = True
                            else:
                                vlm_vel_commands, time_to_go, is_parseable = parse_vlm_command(stream_output)
                        print(
                    ...
                    _last_recovery_decision = None
                    if stuck_recovery is not None and phase == "return":
                    ...
                    if stop_gate is not None and phase == "return":
                        ...
                        if _last_gate_decision.decision in ("vetoed", "blind_budget_exhausted"):
                        ...
                        print(
                            ...
                            f"anchor_role={_last_gate_decision.anchor_progress_role} "
                            f"anchor_idx={_last_gate_decision.anchor_index} "
                            f"anchor_remaining={_last_gate_decision.anchor_route_remaining} "
                            f"current_idx={_last_gate_decision.current_anchor_index} "
                            f"current_unreliable={_last_gate_decision.current_anchor_unreliable} "
                            f"evict_streak={_last_gate_decision.current_evict_streak} "
                            f"stale_attempts={_last_gate_decision.relocalization_stale_attempts} "
                            f"blind_streak={_last_gate_decision.blind_streak} "
                            f"uncertainty_radius={_last_gate_decision.distance_uncertainty_radius_m} "
                            f"cross_role_distance={_last_gate_decision.cross_role_distance_to_start_m}",
                            flush=True,
                        )
        ...
        if args_cli.route_memory:
            ...
            route_descriptor = route_memory_descriptor_from_infos(
                infos,
                env,
                include_local_map=(args_cli.route_relocalization_backend != "visual_sequential_pair"),
            )
        ...
        if visual_gt_monitor is not None:
            visual_gt_monitor.record_step(
                step=int(num_steps),
                phase=phase,
                env=env,
                route_agent=route_agent,
                progress=(progress if phase == "return" else None),
                command=vlm_vel_commands,
                output=stream_output,
            )
    ...
    promotion_shadow_summary = (
        promotion_shadow_session.close() if promotion_shadow_session is not None else None
    )
    if v11_shadow_session is not None:
    ...
    measurements["round_trip"] = {
        ...
        "reliability_v11_online_shadow": v11_shadow_summary,
        "reliability_v11_decision_shadow_requested": bool(
            args_cli.reliability_v11_decision_shadow
        ),
        "sequential_pair_promotion_model_shadow": (
            args_cli.sequential_pair_promotion_model_shadow or None
        ),
        "sequential_pair_promotion_model_quarantine_threshold": float(
            args_cli.sequential_pair_promotion_model_quarantine_threshold
        ),
        "sequential_pair_promotion_model_active_promote": bool(
            args_cli.sequential_pair_promotion_model_active_promote
        ),
        ...
        "sequential_pair_reliability_demote_current": bool(args_cli.sequential_pair_reliability_demote_current),
        "sequential_pair_evict_unreliable_current": bool(args_cli.sequential_pair_evict_unreliable_current),
        "current_evict_stall_attempts": int(args_cli.current_evict_stall_attempts),
        "current_evict_mode": str(args_cli.current_evict_mode),
        "current_evict_window": int(args_cli.current_evict_window),
        "current_evict_min_votes": int(args_cli.current_evict_min_votes),
        ...
        "stale_relocalization_max_attempts": int(args_cli.stale_relocalization_max_attempts),
        "sequential_pair_relocalization_uncertainty_mode": str(
            args_cli.sequential_pair_relocalization_uncertainty_mode
        ),
        "stale_uncertainty_base_floor_m": float(args_cli.stale_uncertainty_base_floor_m),
        ...
        "sequential_pair_promotion_ambiguity_gate": bool(args_cli.sequential_pair_promotion_ambiguity_gate),
        "sequential_pair_promotion_min_confidence": float(args_cli.sequential_pair_promotion_min_confidence),
        "sequential_pair_promotion_anomaly_gate": bool(args_cli.sequential_pair_promotion_anomaly_gate),
        "sequential_pair_promotion_anomaly_max_bearing_jump_deg": float(
            args_cli.sequential_pair_promotion_anomaly_max_bearing_jump_deg
        ),
        "sequential_pair_promotion_anomaly_max_collapse_m": float(
            args_cli.sequential_pair_promotion_anomaly_max_collapse_m
        ),
        "sequential_pair_promotion_mode": args_cli.sequential_pair_promotion_mode,
        ...
        "sequential_pair_vision_disagreement_mode": args_cli.sequential_pair_vision_disagreement_mode,
        "sequential_pair_vision_disagreement_confidence_penalty": float(
            args_cli.sequential_pair_vision_disagreement_confidence_penalty
        ),
        ...
        "stop_gate": {
            ...
            "corroboration_overrides_low_reliability": (
                stop_gate.corroboration_overrides_low_reliability if stop_gate is not None else None
            ),
            "blind_budget_enabled": stop_gate.blind_budget_enabled if stop_gate is not None else None,
            "blind_budget_max_attempts": (
                stop_gate.blind_budget_max_attempts if stop_gate is not None else None
            ),
            "use_uncertainty_interval": (
                stop_gate.use_uncertainty_interval if stop_gate is not None else None
            ),
            "require_cross_role_agreement": (
                stop_gate.require_cross_role_agreement if stop_gate is not None else None
            ),
            "cross_role_max_disagreement_m": (
                stop_gate.cross_role_max_disagreement_m if stop_gate is not None else None
            ),
        },
    }

    if visual_gt_monitor is not None:
        visual_gt_monitor.close(route_agent)
