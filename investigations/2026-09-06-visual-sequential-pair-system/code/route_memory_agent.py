...
class RelativeStartProgress:
    ...
    # 2026-07-18: research-only oracle-supervision experiment (round_trip_eval.py
    # computes this from Isaac's privileged ground truth, RouteMemoryAgent never
    # looks at oracle state itself -- this field only tells make_hint/_make_anchor_hint
    # to take the existing hedged-wording branch instead of emitting the specific,
    # oracle-flagged-inaccurate numeric hint). Default False: no behavior change.
    oracle_supervision_suppressed: bool = False
    # 2026-08-03 (line-2 Phase 0.1 diagnostic, mechanism D investigation):
    # which anchor ROLE (`"next"` or `"current"`) _anchor_progress() actually
    # used to build this progress object, and that anchor's own index --
    # purely observational, never read by any decision logic. Added because
    # stop_gate's anchor-corroboration veto (module docstring: "the anchor
    # currently tracked as 'current'... its own distance-from-start") silently
    # receives whichever anchor sequential_pair_report_next_anchor's preference
    # (or its current-fallback) picked THIS attempt -- confirmed on ep205 of
    # investigations/2026-07-28-downgrade-batch-mechanism-failure-classification
    # that the anchor_route_remaining_m cited there (7.05m, anchor7) was the
    # NEXT role, not current (anchor10, route_remaining 10.08m) -- but the
    # live gate_authority_d logged that step doesn't cleanly match either
    # anchor's raw numbers either, so the exact substitution path is still not
    # fully pinned down. This field exists so the next time this mismatch is
    # hit, the actual role/anchor is visible in phase_events without needing
    # to reconstruct it after the fact from covisibility_records.
    anchor_progress_role: Optional[str] = None
    # 2026-08-03 (line-2 Phase 0.2 diagnostic): the actual internal CURRENT
    # index (_target_anchor_index) and eviction's own live state, independent
    # of anchor_progress_role above -- that field can point at "next" (see its
    # docstring), so it cannot be used to tell whether eviction ever fires.
    # Purely observational, never read by any decision logic.
    current_target_anchor_index: Optional[int] = None
    current_anchor_unreliable: Optional[bool] = None
    current_evict_streak: Optional[int] = None
    # 2026-08-04 (frozen-relocalization-reading fix): consecutive attempts
    # since the estimate actually behind THIS progress object (whichever role
    # anchor_progress_role says was used) last had a fresh candidate appear in
    # `estimates` -- 0 means this attempt's own reading. Purely observational
    # unless sequential_pair_stale_relocalization_distrust is on. See
    # _current_stale_attempts' docstring at __init__ for the ep368 case this
    # was built to detect and the reasoning for why "unreliable" (a fresh-
    # reading-quality signal) cannot see this on its own.
    relocalization_stale_attempts: Optional[int] = None
    # 2026-08-04 (growing-uncertainty variant, see
    # sequential_pair_relocalization_uncertainty_mode's docstring in
    # RouteMemoryAgent.__init__): +/- radius (m) around distance_to_start_m
    # this reading should be treated as uncertain by, given how long it's
    # been since this role's estimate last refreshed and how far the robot
    # has moved since. None means "no interval info available, use the point
    # distance as-is" (mode="fixed", or this role is currently fresh) --
    # never a signal to ignore, always additive to distance_to_start_m.
    distance_uncertainty_radius_m: Optional[float] = None
    # 2026-08-04 (cross-role agreement): the OTHER role's (current's if this
    # progress object is "next", or vice versa) own independently-derived
    # distance_to_start_m, via the identical formula -- a genuine second
    # opinion from a different anchor's point cloud, unlike anchor-
    # corroboration's reuse of this same reading. None when that role
    # currently has no estimate at all ("cannot verify", not "disagrees").
    # See stop_gate_require_cross_role_agreement's docstring for the consumer.
...
class RouteMemoryAgent:
    def __init__(
        ...
        sequential_pair_closure_cooldown_attempts: int = 0,
        ...
        # 2026-08-15 (investigations/2026-08-15-v11-quarantine-veto): a SEPARATE,
        # independent veto from sequential_pair_promotion_model_quarantine_veto
        # above -- that one is the abandoned from-scratch promote/wait/quarantine
        # controller (Route1 explicitly reverted to hardcode-only on 2026-08-03).
        # This one consumes Route2's existing, already-validated V1.1 reliability
        # model (distance head) purely as a "confidently good, don't quarantine"
        # safety check on top of the existing heuristic quarantine mechanisms --
        # it can only PREVENT a quarantine.add(), never initiate one. Verified via
        # a 194-reading out-of-sample replay against real captured ICP data
        # (V1.1 never trained on this batch): rescues 41/87 false-positive
        # quarantines with 0/65 wrongly-rescued genuinely-bad ones. Populated
        # per-attempt by round_trip_eval.py's relocalizer closure into
        # _active_v11_veto_by_anchor (anchor_index -> bool), mirroring
        # _active_quarantine_veto_decision's own attempt-scoped-flag convention.
        sequential_pair_v11_quarantine_veto: bool = False,
        ...
        sequential_pair_report_next_anchor_suppress_if_stale: bool = False,
        ...
        # 2026-07-22 (investigations/2026-07-21-gating..., trend-cascade fix): when on,
        # the position-trend quarantine (_record_next_anchor_trend) also draws from the
        # SAME anti-cascade budget as Injection A (_reliability_quarantines_since_promotion),
        # so A + trend TOGETHER can never blacklist more than reliability_quarantine_max_chain
        # anchors between two promotions. Trend had no cap of its own and could seal a pin by
        # quarantining an entire self-similar downstream (ep134). Off by default (byte-identical
        # to prior behaviour); only meaningful when reliability_quarantine is also enabled.
        reliability_quarantine_shared_trend_budget: bool = False,
        ...
        # 2026-08-03 (line-2 Phase 0.2, investigations/2026-07-28-downgrade-
        # batch-mechanism-failure-classification/MODIFICATION_PLAN.md Phase
        # 0.2 -- planned 2026-07-21 as Injection B part (ii)/(iii), never
        # built until now). sequential_pair_reliability_demote_current above
        # only covers part (i): waiving the current-quality dominance bar so
        # a reliable next can outrun a bad current. It does nothing once
        # current is so bad next can never independently clear
        # close_enough/trend_ok either (ep344/355: current poisoned from
        # attempt 1, drives 10 downstream quarantine cascades since
        # _record_next_anchor_trend has no current_unreliable escape hatch
        # at all). When on: (ii) after current has been
        # _current_persistently_unreliable() for current_evict_stall_attempts
        # consecutive attempts, force-advance to whichever of {current, next}
        # is currently more reliable (skipping close_enough/trend_ok --
        # this is an escape from an already-degenerate state, not a normal
        # confirmed promotion), and (iii) _record_next_anchor_trend skips its
        # disagreement check entirely while current is persistently
        # unreliable (a bad current cannot be used as the reference to judge
        # next's trend). Requires sequential_pair_reliability_demote_current
        # (reuses its _current_reliability_history bookkeeping -- no new
        # signal, just new consumers of it). Off by default.
        sequential_pair_evict_unreliable_current: bool = False,
        current_evict_stall_attempts: int = 30,
        # 2026-08-04 (line-2 Phase 0.2 follow-up, investigations/2026-08-03-
        # model-rollback-and-line2-phase0-1-resume/FINDINGS.md sec.12's open
        # concern): the streak above resets to 0 on a single reliable-looking
        # reading, even though current_unreliable is itself a bad-fraction
        # over a retained history that can flicker across its own threshold
        # attempt-to-attempt (see current_reliability_quarantine_bad_fraction
        # elsewhere). A real 30ep batch (line2_phase01_30ep_20260803, ep678)
        # showed this: evict_streak climbed to 19/30 then reset to 0, so
        # eviction never fired despite current staying unreliable (0.98-0.998
        # confidence, "confidently wrong") for the whole surrounding window.
        # "window" mode replaces the hard streak with a bounded-window vote,
        # mirroring sequential_pair_promotion_window/_min_votes's own
        # already-established pattern (_record_promotion_vote) instead of
        # inventing a new primitive: the last current_evict_window attempts'
        # current_unreliable readings are kept per current-anchor-index, and
        # eviction fires once current_evict_min_votes of them are True --
        # tolerant of an occasional good-looking blip instead of losing all
        # progress to it. "streak" (default) is byte-identical to prior
        # behavior; window is a new opt-in variant, not a replacement.
        current_evict_mode: str = "streak",
        current_evict_window: int = 40,
        current_evict_min_votes: int = 30,
        ...
        # _current_stale_attempts' docstring above): Injection C's distrust
        # above only reacts to a BAD fresh reading -- it says nothing about a
        # reading that stopped refreshing altogether, which is what actually
        # froze stop_gate's `d` for 1500+ steps in ep368. When on, the same
        # confidence-floor-cap + distance_authority_low_reliability=True
        # treatment Injection C already gives a bad-U reading is applied once
        # the role actually being reported (current or next, whichever
        # _anchor_progress picked) has gone stale_relocalization_max_attempts
        # attempts without a fresh candidate -- independent flag/threshold
        # from Injection C's own U-based one, since "stale" and "bad-looking"
        # are different failure signatures (a stale cache can look perfectly
        # confident, it just isn't being asked anymore).
        sequential_pair_stale_relocalization_distrust: bool = False,
        stale_relocalization_max_attempts: int = 30,
        # 2026-08-04 (stop_gate-review follow-up, growing-uncertainty variant
        # of the staleness fix above -- mirrors the isolated Route2 branch's
        # [d-0.35, d+0.35] interval that widens by dead-reckoned travel since
        # the last trusted reading, investigations/2026-07-28-anchor-support-
        # recovery). "fixed" (default) is the hard-threshold behavior above,
        # byte-identical. "growing" instead exposes a continuously-widening
        # distance_uncertainty_radius_m = stale_uncertainty_base_floor_m +
        # (integrated distance traveled since this role's estimate last
        # refreshed), starting from the FIRST stale attempt rather than only
        # after stale_relocalization_max_attempts -- stop_gate (which knows
        # r_in/r_out, this file deliberately does not) decides what to do
        # with it via --stop_gate_use_uncertainty_interval. Reuses
        # self._return_pose_from_return_start's existing bounded, per-step-
        # refreshed integration (NOT the deleted unbounded arc-length
        # odometer -- this only ever spans stale-onset to next refresh or
        # episode end, and resets to zero the moment a fresh reading arrives,
        # same discipline as every other streak/window in this file).
        sequential_pair_relocalization_uncertainty_mode: str = "fixed",
        stale_uncertainty_base_floor_m: float = 0.35,
        ...
        # 2026-08-03 (line-2 Phase 1.1, mechanism A). Reuses
        # current_confidence_ambiguity_gate_threshold as the ratio bar (no
        # separate threshold -- one ambiguity definition project-wide).
        sequential_pair_promotion_ambiguity_gate: bool = False,
        sequential_pair_promotion_min_confidence: float = 0.35,
        # 2026-08-04 (cheap vision-free anomaly gate, see its firing site's
        # comment for the ep93 rationale). Off by default. Bearing jump is
        # compared across consecutive attempts against the SAME candidate
        # anchor index only (reset the moment that candidate is promoted or
        # a different anchor becomes "next"), same per-anchor-history scoping
        # already used by _promotion_distance_history etc. 90deg/1.5m are
        # generous round numbers (a real single-attempt viewpoint shift can
        # itself cause tens of degrees of bearing noise and ~0.3-0.75m of
        # real closing distance per relocalization interval; both defaults
        # sit well above that, catching only implausible jumps, not normal
        # noise) -- not fit against project-wide data, tune with real batches.
        sequential_pair_promotion_anomaly_gate: bool = False,
        promotion_anomaly_max_bearing_jump_deg: float = 90.0,
        promotion_anomaly_max_collapse_m: float = 1.5,
        multiframe_anchor_window: int = 1,
        multiframe_anchor_symmetric_enabled: bool = False,
        multiframe_anchor_backward_distance_m: float = 1.5,
        multiframe_anchor_forward_distance_m: float = 1.5,
        multiframe_anchor_forward_stall_updates: int = 300,
        return_frame_buffer_enabled: bool = False,
        return_frame_buffer_window_m: float = 1.0,
        return_frame_buffer_max_frames: int = 400,
        **_: object,
    ):
        ...
        # Symmetric motion-integrated anchor submap (2026-07-16, opt-in,
        # separate mechanism from multiframe_anchor_window above -- see
        # investigations/2026-07-16-matching-primitive-strategy/FINDINGS.md
        # Part 3.5): that mechanism merges frames captured BEFORE the
        # anchor-spacing trigger only, as a fixed STEP count -- confirmed
        # 2026-07-08/09 not to help genuine rotational self-similarity,
        # since a short (~0.8m), one-sided window sampled from essentially
        # the same vantage point sees the same repeating local structure.
        # This instead accumulates real DISTANCE (robust to walking-speed
        # variation) both BEFORE and AFTER the trigger crossing, delaying
        # anchor finalization to also capture forward frames -- motivated by
        # a 2026-07-16 ground-truth check finding 9/10 known-bad anchors
        # have a real >30deg route turn within 0-5.3m (mostly 0-2m or
        # ~0.00m) of the anchor's own position: the landmark objectively
        # exists nearby, a single instantaneous capture at an arbitrary
        # trigger moment just frequently misses it, on either side. The
        # anchor's own registered pose/distance stay pinned to the exact
        # trigger crossing (see _begin_pending_anchor/_finalize_pending_
        # anchor) -- only the stored descriptor gets the wider submap, so
        # edge_from_previous and every other consumer of anchor position are
        # unaffected by how wide the accumulation window is.
        self.multiframe_anchor_symmetric_enabled: bool = bool(multiframe_anchor_symmetric_enabled)
        self.multiframe_anchor_backward_distance_m: float = float(multiframe_anchor_backward_distance_m)
        self.multiframe_anchor_forward_distance_m: float = float(multiframe_anchor_forward_distance_m)
        self.multiframe_anchor_forward_stall_updates: int = max(1, int(multiframe_anchor_forward_stall_updates))
        self._outbound_symmetric_frame_buffer: list[tuple[float, list[float], object]] = []
        self._pending_anchor: Optional[dict] = None
        # Return-side live frame buffer (2026-07-16, opt-in): today, a fresh
        # real point cloud is built every single return-phase step
        # (local_map_descriptor_from_env in round_trip_eval.py) but silently
        # discarded on every non-interval update_relocalization call -- every
        # ICP attempt matches a single instantaneous live scan, even once the
        # anchor side above may hold a richer multi-frame submap. This
        # accumulates the last return_frame_buffer_window_m of real per-step
        # return frames (reprojected via current_absolute_pose_from_start(),
        # which already tracks return-phase pose every step) into one merged
        # "current" descriptor at each actual ICP-attempt step. Deliberately
        # a SMALLER window than the anchor side's default (1.0m vs 1.5m each
        # direction): reprojection here relies on return-phase dead
        # reckoning, the same integration relocalization exists to correct
        # and that drifts over the return phase -- unlike outbound dead
        # reckoning, which this project already treats as comparatively
        # trustworthy. A smaller window bounds how much drift the merge
        # itself can accumulate. Backward-only by construction (no future
        # return frames exist yet).
        self.return_frame_buffer_enabled: bool = bool(return_frame_buffer_enabled)
        self.return_frame_buffer_window_m: float = float(return_frame_buffer_window_m)
        self.return_frame_buffer_max_frames: int = max(1, int(return_frame_buffer_max_frames))
        ...
        # 2026-08-04 (frozen-relocalization-reading root cause -- ep368 of
        # line2_phase01_30ep_20260803, and the previously-flagged r30v3-ep500
        # "distance estimate frozen bit-for-bit for ~3400 steps" case,
        # investigations/2026-08-03-model-rollback-and-line2-phase0-1-resume/
        # FINDINGS.md sec.5): _latest_relocalization/_latest_next_candidate_
        # relocalization only ever update on an attempt where THIS role's own
        # candidate is present -- deliberately, this is the "hold the last
        # accepted value" convention documented at their own update sites.
        # But nothing downstream could previously tell a value that just
        # refreshed apart from one that has been silently reused, bit-for-
        # bit, for 1500+ steps because the anchor stopped producing ANY
        # candidate at all (confirmed directly for ep368: ground-truth
        # distance_to_start_m converged 1.45m->0.26m over this same window
        # while the reported anchor-relocalization distance sat frozen at
        # 4.52m the entire time) -- stop_gate kept correctly-by-its-own-logic
        # vetoing a stop against a reading that was, unknown to it, ancient.
        # These count consecutive attempts since each role's cache was last
        # genuinely refreshed; see _select_sequential_pair_relocalization's
        # current_est/next_est computation for where they update, and
        # sequential_pair_stale_relocalization_distrust for the opt-in
        # consumer (mirrors Injection C's distrust_downstream convention).
        self._current_stale_attempts: int = 0
        self._next_stale_attempts: int = 0
        # 2026-08-04 (growing-uncertainty variant, see
        # sequential_pair_relocalization_uncertainty_mode's docstring above):
        # the robot's own integrated current_pose_from_start (2 floats: x, y
        # -- yaw not needed for a travel-distance radius) at the moment each
        # role's staleness streak began (0->1 transition). None whenever that
        # role is fresh. Only ever read when mode="growing".
        self._current_stale_onset_pose: Optional[list[float]] = None
        self._next_stale_onset_pose: Optional[list[float]] = None
        ...
        # 2026-07-31 (Phase 3, promote/wait only -- INTEGRATION_PLAN.md,
        # investigations/2026-07-28-promotion-quarantine-controller-model/):
        # off by default. When True, _select_sequential_pair_relocalization
        # replaces ONLY the bounded_evidence vote-aggregation step below with
        # the learned model's promote-vs-wait call (set externally, once per
        # attempt, via _active_promotion_model_decision -- see round_trip_
        # eval.py's promotion-model closure, which scores the same
        # covisibility_records the shadow path already validated). Deliberately
        # does NOT touch quality_ok (kept as a hard AND-gate would reproduce
        # the exact "confidently wrong current freezes next" failure Injection B
        # exists to fix -- the model's own features already subsume what
        # quality_ok checks) nor quarantine (that stays on
        # _record_next_anchor_trend/heuristic control only, per user decision
        # 2026-07-31 -- this flag has zero effect on _quarantined_anchor_indices).
        # closure_reject_reason and short_baseline vetoes still apply AFTER the
        # model's call, unchanged -- those guard "is this reading resolvable at
        # all", a different question from "should we promote", and are cheap
        # insurance against exactly the failure classes each was added for.
        self.sequential_pair_promotion_model_active_promote: bool = bool(
        ...
        self.sequential_pair_v11_quarantine_veto: bool = bool(sequential_pair_v11_quarantine_veto)
        # anchor_index -> "V1.1 confidently trusts this reading" (True), reset
        # and repopulated once per attempt by round_trip_eval.py's relocalizer
        # closure -- a plain dict read (like _quarantine_vetoed_by_model's plain
        # bool read), not consume-and-reset, since multiple quarantine mechanisms
        # below may all check the same anchor within one attempt.
        self._active_v11_veto_by_anchor: dict[int, bool] = {}
        ...
        # 2026-08-14 (no-yaw-align 100ep batch, closure-veto-at-return-start
        # investigation): the closure precheck's bearing reconciliation
        # (sequential_pair_closure_reconciliation_signal="bearing") computes
        # atan2(dy, dx) of each side's own reading -- well-defined at normal
        # range, but numerically degenerate right when a reading's own
        # distance is near zero, which is exactly "current"'s situation for
        # the first several attempts after every promotion (including
        # return-start itself, where finalize_outbound() places the robot
        # exactly on the anchor) and after every subsequent promotion (the
        # newly-promoted current was, by definition, just approached). Real
        # replay data (ep517 of the batch above): bearing disagreement swings
        # 120-179 deg purely from sub-decimeter noise while current_dist <
        # ~0.5m, then settles to single digits/tens of degrees once it grows
        # past that -- a real geometric mismatch, not measurement noise.
        # Neither ICP's own reported distance (the thing under test, known
        # unreliable to meter-scale -- see this project's confidently-wrong
        # ICP findings) nor accumulated dead-reckoning since return-start (see
        # docs/odometry_consumers_20260704.md item 5: the equivalent
        # accumulated-yaw-reference gate measured drifting to 160 deg within a
        # single episode) are safe signals to gate this on. This instead uses
        # a plain attempt counter since the anchor most recently became
        # "current" (reset on every promotion, see the pruning block in
        # _select_sequential_pair_relocalization) -- no ICP or odometry
        # dependency at all, just "how many attempts has *this* current anchor
        # had". 0 (default) preserves existing behavior (no cooldown).
        self.sequential_pair_closure_cooldown_attempts: int = int(
            sequential_pair_closure_cooldown_attempts
        )
        self._closure_cooldown_counter: int = 0
        ...
        # 2026-07-19, opt-in, no-op unless sequential_pair_report_next_anchor is
        # also on: the simplest possible fix for the freezing bug found this
        # session -- next's own cached estimate (_latest_next_candidate_
        # relocalization) only updates when a fresh next_est is accepted this
        # attempt (see the comment there), and has no time/attempt-based
        # staleness handling at all, so a long streak of rejected/unconfirmed
        # attempts leaves the hint reporting a byte-identical stale reading for
        # many consecutive VLM turns (confirmed on real data: ep319's return
        # phase repeated one frozen reading across 19 consecutive hints /
        # ~1075 steps / ~215 underlying ICP attempts, none of which refreshed
        # it). Deliberately NOT an odometry-based interpolation/propagation
        # fix (the user's explicit call, 2026-07-19: odometry drifts too
        # easily to trust for this) -- instead, if no fresh next_est has been
        # accepted since the previous VLM query, suppress the hint entirely
        # for this turn (report nothing rather than a known-stale number).
        # Falls back to whatever base_instruction would otherwise be (no
        # System Hint line at all) -- distinct from the existing "filter lost
        # lock" hedged-wording path, which still asserts a (uncertain)
        # direction; this asserts nothing.
        self.sequential_pair_report_next_anchor_suppress_if_stale: bool = bool(
            sequential_pair_report_next_anchor_suppress_if_stale
        )
        self._next_candidate_update_seq: int = 0
        self._next_candidate_seq_at_last_hint: Optional[int] = None
        ...
        self.reliability_quarantine_shared_trend_budget: bool = bool(reliability_quarantine_shared_trend_budget)
        ...
        self.sequential_pair_stale_relocalization_distrust: bool = bool(
            sequential_pair_stale_relocalization_distrust
        )
        self.stale_relocalization_max_attempts: int = max(1, int(stale_relocalization_max_attempts))
        self.sequential_pair_relocalization_uncertainty_mode: str = str(
            sequential_pair_relocalization_uncertainty_mode
        )
        self.stale_uncertainty_base_floor_m: float = float(stale_uncertainty_base_floor_m)
        self._current_reliability_history: dict[int, list[float]] = {}
        # 2026-08-03 (line-2 Phase 0.2): explicit current-eviction state.
        self.sequential_pair_evict_unreliable_current: bool = bool(sequential_pair_evict_unreliable_current)
        self.current_evict_stall_attempts: int = max(1, int(current_evict_stall_attempts))
        self._current_evict_streak_anchor: Optional[int] = None
        self._current_evict_streak_count: int = 0
        # 2026-08-04: windowed-vote eviction variant (see current_evict_mode's
        # docstring above). Independent state from the streak fields above so
        # switching modes never leaves stale counters from the other mode.
        self.current_evict_mode: str = str(current_evict_mode)
        self.current_evict_window: int = max(1, int(current_evict_window))
        self.current_evict_min_votes: int = max(1, int(current_evict_min_votes))
        self._current_evict_vote_history: dict[int, list[bool]] = {}
        # 2026-08-04: observability for sequential_pair_promotion_ambiguity_gate
        # (see its firing site's comment). None when the gate didn't block this
        # attempt (either off, or candidate_promote was already False/never a
        # promotion candidate); "ambiguous_ratio"/"low_confidence" identify
        # which of the gate's two checks fired. Reset every
        # _select_sequential_pair_relocalization call, read by
        # update_relocalization right after.
        self._last_promotion_ambiguity_gate_reason: Optional[str] = None
        self.sequential_pair_promotion_anomaly_gate: bool = bool(sequential_pair_promotion_anomaly_gate)
        self.promotion_anomaly_max_bearing_jump_deg: float = float(promotion_anomaly_max_bearing_jump_deg)
        self.promotion_anomaly_max_collapse_m: float = float(promotion_anomaly_max_collapse_m)
        # keyed by candidate anchor index, same per-anchor-history scoping as
        # _promotion_distance_history -- pruned on promotion below.
        self._next_candidate_anomaly_history: dict[int, tuple[float, float]] = {}
        # "bearing_jump"/"distance_collapse" identify which check fired;
        # mirrors _last_promotion_ambiguity_gate_reason's own convention.
        self._last_promotion_anomaly_reason: Optional[str] = None
        ...
        self.sequential_pair_promotion_ambiguity_gate: bool = bool(
            sequential_pair_promotion_ambiguity_gate
        )
        self.sequential_pair_promotion_min_confidence: float = float(
            sequential_pair_promotion_min_confidence
        )
        if self.enabled:
    ...
    def update_outbound_motion(
        ...
        if self.multiframe_anchor_symmetric_enabled and descriptor is not None:
            self._outbound_symmetric_frame_buffer.append(
                (float(self._outbound_distance_m), list(self._outbound_pose_from_start), descriptor)
            )
            # Cutoff is relative to the TRIGGER distance while a pending
            # anchor is accumulating forward (fixed reference point -- the
            # backward window it needs was already fully defined the moment
            # the trigger fired), not to the still-advancing current
            # distance -- using current distance here would keep pruning
            # further forward as the pending anchor accumulates and evict
            # its own early backward frames before it ever gets to use them.
            # Before any trigger, only the backward margin needs protecting
            # (in case a trigger fires at the current distance).
            reference_distance = (
                self._pending_anchor["trigger_distance_m"]
                if self._pending_anchor is not None
                else self._outbound_distance_m
            )
        ...
        if self._pending_anchor is not None:
            self._maybe_finalize_pending_anchor()
    ...
    def update_return_motion(
        ...
        **_: object,
        ...
        dx, dy, _ = [float(v) for v in delta]
        self._return_distance_m += float(math.hypot(dx, dy))
        if self.return_frame_buffer_enabled and local_descriptor is not None:
            pose = self.current_absolute_pose_from_start()
            self._return_frame_buffer.append((float(self._return_distance_m), list(pose), local_descriptor))
            cutoff = self._return_distance_m - self.return_frame_buffer_window_m
            while self._return_frame_buffer and self._return_frame_buffer[0][0] < cutoff:
                self._return_frame_buffer.pop(0)
            # Frame-count safety valve independent of the distance criterion
            # -- a long in-place-rotation dwell contributes ~0 to
            # _return_distance_m per step, so the distance window alone
            # would not bound buffer growth during such a dwell (same risk
            # multiframe_anchor_forward_stall_updates guards on the anchor
            # side, just measured in frames here instead of update calls).
            if len(self._return_frame_buffer) > self.return_frame_buffer_max_frames:
                del self._return_frame_buffer[: len(self._return_frame_buffer) - self.return_frame_buffer_max_frames]
        self.update_relocalization(local_descriptor=local_descriptor, relocalization=relocalization)

    def _merge_return_frame_buffer(self, current_pose: list[float]) -> Optional[dict]:
        frames = [(pose, desc) for (_dist, pose, desc) in self._return_frame_buffer]
        return self._merge_point_frames(frames, current_pose)
    ...
    def finalize_outbound(
        ...
        if self.enabled and self._pending_anchor is not None:
            # Outbound can legitimately end mid-forward-accumulation -- flush
            # with whatever forward frames accumulated so far rather than
            # dropping the anchor or leaving _pending_anchor dangling.
            self._finalize_pending_anchor()
    ...
    def update_relocalization(
        ...
        if not estimates and self.relocalizer is not None:
            if (
                ...
                return None
            self._force_next_relocalization = False
            effective_descriptor = local_descriptor
            if self.return_frame_buffer_enabled:
                merged = self._merge_return_frame_buffer(self.current_absolute_pose_from_start())
                if merged is not None:
                    effective_descriptor = merged
            estimates = self._coerce_relocalizations(
                self.relocalizer(effective_descriptor, self.anchors)
            )
        elif estimates:
            self._force_next_relocalization = False
        estimates = [
        ...
        if estimate is None:
            self.relocalization_events.append({
                ...
                "promotion_ambiguity_gate_blocked": self._last_promotion_ambiguity_gate_reason,
                "promotion_anomaly_blocked": self._last_promotion_anomaly_reason,
            })
            return None
    ...
    def _v11_veto_permits_quarantine(self, idx: int) -> bool:
        """2026-08-15: True unless V1.1 confidently vouches for anchor idx's
        most recent reading (see sequential_pair_v11_quarantine_veto's
        docstring at __init__) -- i.e. this returns False (blocks the
        quarantine) only when the model actively disagrees with the heuristic.
        Missing/no-opinion data (flag off, anchor not scored this attempt)
        fails open, same convention as _quarantine_vetoed_by_model."""
        if not self.sequential_pair_v11_quarantine_veto:
            return True
        vetoed = self._active_v11_veto_by_anchor.get(idx) is True
        if vetoed:
            # 2026-08-15: pure diagnostic addition -- the moment this veto
            # actually changes behavior (a quarantine that would have fired
            # is blocked). Does not alter the return value or any other
            # logic. Safe to add mid-batch: each episode is a fresh
            # subprocess, so this only affects episodes not yet started.
            print(
                f"[v11-veto] BLOCKED a quarantine for anchor idx={idx} "
                "(V1.1 confidently vouches for this anchor's most recent reading)",
                flush=True,
            )
        return not vetoed
    ...
    def _record_next_anchor_stability(self, estimates: list[AnchorRelocalization]) -> None:
        ...
        for estimate in estimates:
            ...
            ) and not self._quarantine_vetoed_by_model() and self._v11_veto_permits_quarantine(idx):
    ...
    def _record_next_anchor_trend(self, estimates: list[AnchorRelocalization]) -> None:
        ...
        if current_est is None:
            return
        # 2026-08-03 (line-2 Phase 0.2, mechanism B part (iii)): this whole
        # function judges "next" purely by disagreement against "current" --
        # if current is itself persistently unreliable (Injection B's own
        # signal, already used to waive the promotion-side quality bar just
        # above in _select_sequential_pair_relocalization), a disagreement
        # here is most likely current's fault, not next's, exactly like
        # closure_reject_veto's own current_unreliable bypass. Confirmed root
        # cause of ep344/355 (investigations/2026-07-28-downgrade-batch-
        # mechanism-failure-classification/FINDINGS.md mechanism B): a current
        # poisoned from attempt 1 drove 10 downstream NEXT_SKIP quarantines
        # via this exact check, with no escape hatch. Gated on the new flag
        # (not directly on demote_current) so turning this on is a deliberate
        # opt-in, per this project's off-by-default convention.
        if (
            self.sequential_pair_evict_unreliable_current
            and self._current_persistently_unreliable(current_idx)
        ):
            return
        for estimate in estimates:
            idx = int(estimate.anchor_index)
            ...
            if self._quarantine_vetoed_by_model():
                continue
            if not self._v11_veto_permits_quarantine(idx):
                continue
            ...
            # have used reliability_quarantine_max_chain quarantines since the last promotion,
            # trend stops banning too -- so it can no longer seal a pin by blacklisting an
            # entire downstream (the position-trend quarantine has no cap of its own; A's
            # max_chain previously only bounded A). Flag off => unchanged byte-behaviour.
            if (
                self.reliability_quarantine_shared_trend_budget
                and self.sequential_pair_reliability_quarantine_enabled
            ):
    ...
    def _record_next_anchor_quality(self, estimates: list[AnchorRelocalization]) -> None:
        ...
        for estimate in estimates:
            ...
            if (
                mean_ratio > self.quarantine_next_quality_threshold
                and not self._quarantine_vetoed_by_model()
                and self._v11_veto_permits_quarantine(idx)
            ):
                self._quarantined_anchor_indices.add(idx)
    ...
    def _record_next_anchor_reliability(self, estimates: list[AnchorRelocalization]) -> None:
        ...
        for estimate in estimates:
            ...
            if (
                bad_fraction > self.reliability_quarantine_bad_fraction
                and not self._quarantine_vetoed_by_model()
                and self._v11_veto_permits_quarantine(idx)
            ):
                self._quarantined_anchor_indices.add(idx)
    ...
    def _select_sequential_pair_relocalization(
        ...
        self._last_promotion_ambiguity_gate_reason = None
        self._last_promotion_anomaly_reason = None
        # 2026-08-15 diagnostic-only (offline replay/investigation use, no
        # behavioral effect): snapshot of the bounded_evidence vote inputs for
        # whichever next_idx this attempt evaluates, so a replay harness can
        # read the real close_enough/quality_ok/trend_ok/vote-count state
        # instead of re-deriving it. Always overwritten below; never read by
        # any decision path.
        self._last_vote_debug: Optional[dict] = None
        ...
        # 2026-07-19 fix: previously returned here immediately on a closure
        # mismatch (current/next disagree and neither side's quality clearly
        # dominates -- see _sequential_pair_closure_precheck), skipping this
        # whole method's promotion-vote bookkeeping AND
        # promotion_alias_stall_attempts's counter for the entire attempt.
        # Confirmed against real batch data (canonical_report_next_50ep_
        # 20260719): this silently starved bounded_evidence's vote window
        # (never recorded a vote at all, not even a failing one) on anchor
        # pairs that keep genuinely disagreeing without either side
        # dominating -- e.g. ep187's anchor (11,12) pair rejected 74/120
        # attempts this way, none of which ever reached the vote code below,
        # while the alias-aware stall counter also never advanced on those
        # 74 attempts, delaying its own 200-attempt relief valve well past
        # where it should have fired. closure_reject_reason is now deferred
        # (see below) and applied as a forced candidate_promote=False after
        # normal bookkeeping runs, mirroring
        # sequential_pair_short_baseline_require_resolution's own established
        # "run promotion normally, override only the final decision" pattern
        # instead of dropping the attempt from history entirely.
        ...
        # 2026-08-04: staleness bookkeeping (see _current_stale_attempts'
        # docstring at __init__) -- counts consecutive attempts since THIS
        # role's candidate was last actually present in `estimates`, purely
        # off whether a fresh reading showed up this attempt, independent of
        # anything closure/promotion/eviction decide to do with it below.
        self._current_stale_attempts = 0 if current_est is not None else self._current_stale_attempts + 1
        self._next_stale_attempts = 0 if next_est is not None else self._next_stale_attempts + 1
        # 2026-08-04 (growing-uncertainty variant): snapshot the robot's own
        # integrated position the moment each role's streak starts (0->1),
        # clear it the moment that role refreshes -- see
        # _current_stale_onset_pose's docstring at __init__.
        if self._current_stale_attempts == 0:
            self._current_stale_onset_pose = None
        elif self._current_stale_attempts == 1:
            self._current_stale_onset_pose = self._current_integrated_xy()
        if self._next_stale_attempts == 0:
            self._next_stale_onset_pose = None
        elif self._next_stale_attempts == 1:
            self._next_stale_onset_pose = self._current_integrated_xy()
        ...
        # 2026-08-03 (line-2 Phase 0.2, mechanism B part (ii)): track how many
        # CONSECUTIVE attempts current_idx has been persistently unreliable.
        # Resets to 0 the moment either current_idx changes (a promotion
        # already escaped it) or a reading comes back reliable again -- this
        # is deliberately a streak, not a cumulative count, mirroring
        # forced_stop_anchor_confirm_steps's "streak, not one-off" rationale
        # elsewhere in this project.
        evict_current = False
        if self.sequential_pair_evict_unreliable_current:
            if self.current_evict_mode == "window":
                # Bounded-window vote (2026-08-04): mirrors
                # _record_promotion_vote's own list-append-and-trim idiom.
                # Kept per current-anchor-index so a promotion (current_idx
                # changing) naturally starts a fresh vote history, same as
                # the streak variant resets on a promotion.
                history = self._current_evict_vote_history.setdefault(current_idx, [])
                history.append(bool(current_unreliable))
                if len(history) > self.current_evict_window:
                    del history[: len(history) - self.current_evict_window]
                evict_progress_met = sum(1 for cast in history if cast) >= self.current_evict_min_votes
            else:
                if current_unreliable:
                    if self._current_evict_streak_anchor == current_idx:
                        self._current_evict_streak_count += 1
                    else:
                        self._current_evict_streak_anchor = current_idx
                        self._current_evict_streak_count = 1
                else:
                    self._current_evict_streak_anchor = None
                    self._current_evict_streak_count = 0
                evict_progress_met = self._current_evict_streak_count >= self.current_evict_stall_attempts
            if evict_progress_met and next_est is not None:
                # Escape to whichever of {current, next} is more reliable
                # RIGHT NOW -- not a claim that next is good, only that it is
                # no worse than a current already confirmed degenerate for
                # current_evict_stall_attempts straight attempts. Missing
                # data counts as maximally unreliable (+inf), so a current
                # with no estimate this attempt never blocks escaping to a
                # next that does have one.
                current_u = self._reading_unreliability(current_est) if current_est is not None else None
                next_u = self._reading_unreliability(next_est)
                current_u_eff = current_u if current_u is not None else float("inf")
                next_u_eff = next_u if next_u is not None else float("inf")
                if next_u_eff <= current_u_eff:
                    evict_current = True
        ...
        if gate_next_est is not None and next_est is not None:
            ...
            # MODIFICATION_PLAN.md): quality_ok/close_enough/trend_ok are purely
            # relative-to-current or next's-own-distance-history checks -- none of
            # them looks at this attempt's own absolute confidence or ambiguity
            # flags. current_confidence_ambiguity_gate_enabled already computes
            # "is this reading's best_to_second_score_ratio ambiguous", but until
            # now only used it to cap the REPORTED confidence for downstream
            # consumers (stop_gate/hint_action_arbiter) -- it never fed back into
            # the promotion decision itself, letting ep19 (partial_pose_degenerate),
            # ep88 (ambiguous_high_confidence, ratio=0.98), and ep95 (conf 0.45-0.49)
            # promote anyway. An ambiguous or low-absolute-confidence single attempt
            # cannot cast a "promotable" vote here -- it can still be recorded as a
            # "no" vote by _record_promotion_vote just below, same convention
            # closure_reject_veto uses (override the vote, don't skip the attempt).
            if self.sequential_pair_promotion_ambiguity_gate and candidate_promote:
                ratio = gate_next_est.best_to_second_score_ratio
                if ratio is not None and float(ratio) >= self.current_confidence_ambiguity_gate_threshold:
                    candidate_promote = False
                    # 2026-08-04: this gate previously flipped candidate_promote
                    # silently -- no log line anywhere, so its real-world hit
                    # rate/effect could not be measured from an eval log, only
                    # inferred indirectly from covisibility_records after the
                    # fact (confirmed gap: investigations/2026-08-03-model-
                    # rollback-and-line2-phase0-1-resume/FINDINGS.md's own
                    # follow-up). Recorded here and surfaced into
                    # relocalization_events by update_relocalization, mirroring
                    # closure_reject_veto/eviction's own diagnostic-field
                    # convention rather than a raw print (this file has no
                    # other print side effects -- see round_trip_eval.py for
                    # where diagnostic fields actually get logged/printed).
                    self._last_promotion_ambiguity_gate_reason = "ambiguous_ratio"
                conf = gate_next_est.confidence
                if candidate_promote and conf is not None and float(conf) < self.sequential_pair_promotion_min_confidence:
                    candidate_promote = False
                    self._last_promotion_ambiguity_gate_reason = "low_confidence"
            # 2026-08-04 (line-2 stop_gate-review follow-up, mechanism from
            # investigations/2026-07-23-confidently-wrong-convergence-and-
            # vision-path/FINDINGS.md's proposed-but-never-built "cheap,
            # vision-free dynamic anomaly detection"): ambiguity_gate above
            # only looks at THIS attempt's own ratio/confidence -- it cannot
            # see that the reading itself is changing implausibly fast
            # between attempts, which is exactly ep93's real signature (real
            # numbers: believed distance collapsed 19.7m->2.6m over the same
            # window true distance only went 14.6m->8.7m, more than 2x too
            # fast) -- an entire downstream anchor chain got confidently-
            # wrong-matched at once, avalanche-promoting past both
            # close_enough/quality_ok simultaneously. Flags a single-attempt
            # bearing jump or distance-collapse-rate against the SAME
            # candidate anchor's own previous attempt as implausible and
            # blocks promotion on it, same override convention as
            # ambiguity_gate (a "no" vote, not a skipped attempt).
            if self.sequential_pair_promotion_anomaly_gate and candidate_promote:
                bearing_now = gate_next_est.bearing_to_anchor_deg
                distance_now = gate_next_est.distance_to_anchor_m
                prev = self._next_candidate_anomaly_history.get(next_idx)
                if prev is not None and bearing_now is not None and distance_now is not None:
                    ...
                    bearing_jump_deg = abs(
                        math.degrees(wrap_angle(math.radians(bearing_now - prev_bearing)))
                    )
                    collapse_m = prev_distance - distance_now
                    if bearing_jump_deg > self.promotion_anomaly_max_bearing_jump_deg:
                        candidate_promote = False
                        self._last_promotion_anomaly_reason = "bearing_jump"
                    elif collapse_m > self.promotion_anomaly_max_collapse_m:
                        candidate_promote = False
                        self._last_promotion_anomaly_reason = "distance_collapse"
            if gate_next_est.bearing_to_anchor_deg is not None and gate_next_est.distance_to_anchor_m is not None:
                self._next_candidate_anomaly_history[next_idx] = (
                    gate_next_est.bearing_to_anchor_deg, gate_next_est.distance_to_anchor_m,
                )
            closure_reject_veto = bool(closure_reject_reason is not None and not current_unreliable)
            if closure_reject_veto:
                ...
                # trend_ok/quality_ok computation says -- those are evaluated
                # against gate_current_est/gate_next_est independently of
                # closure precheck's cross-check, so they can still look
                # promotable in isolation even when the two sides disagree
                # with each other. Overriding the vote (not skipping it) is
            ...
            self._last_vote_debug = {
                ...
                "next_distance_to_anchor_m": gate_next_est.distance_to_anchor_m,
                "vote_history": list(self._promotion_vote_history.get(next_idx, [])),
                "promote_after_vote": promote,
            }
            # 2026-07-31 (Phase 3, promote/wait only -- see this flag's
            # docstring at __init__): when active, the learned model's own
            # promote-vs-wait call REPLACES the bounded_evidence vote result
            # just computed above -- but the vote call itself still ran
            # unmodified just above for bookkeeping (vote history, alias-aware
            # stall counter stay populated even while a benched heuristic vote
            # isn't driving the real decision), same established pattern the
            # short-baseline override below already uses. quality_ok/
            # close_enough/trend_ok are deliberately NOT reapplied here.
            # closure_reject_veto still applies (a bad current/next cross-check
            # disagreement is not something the model was trained to see).
            if self.sequential_pair_promotion_model_active_promote:
                model_decision = self._active_promotion_model_decision
                self._active_promotion_model_decision = None
                if model_decision is not None:
                    promote = bool(model_decision) and not closure_reject_veto
            ...
            # so this must gate both paths, not just the bypass. Also applies
            # after the active-model override above, for the same reason
            # closure_reject_veto does.
            if withhold_for_unresolved_short_baseline:
                promote = False
            # 2026-08-03 (line-2 Phase 0.2, mechanism B part (ii)): eviction is
            # a break-glass escape from an already-degenerate state, applied
            # AFTER every other gate/veto/withhold above (including
            # closure_reject_veto and the short-baseline withhold) -- those
            # gates are exactly what a poisoned current can keep tripping
            # forever, so re-subjecting the escape to them would defeat the
            # point. Deliberately does not touch quality_ok/close_enough/
            # trend_ok's own bookkeeping computed above -- this only overrides
            # the final vote, same convention closure_reject_veto/the model
            # override/short_baseline withhold above all already use.
            if evict_current:
                promote = True
        ...
        if promote and next_est is not None:
            ...
            self._next_candidate_anomaly_history = {
                k: v for k, v in self._next_candidate_anomaly_history.items() if k < next_idx
            }
            ...
            # 2026-08-14: the freshly-promoted current is, by definition, an
            # anchor the robot just approached -- restart the closure-cooldown
            # counter so sequential_pair_closure_cooldown_attempts applies to
            # its own dwell time, not whatever count the previous current had
            # accumulated.
            self._closure_cooldown_counter = 0
        ...
        if next_est is not None:
            ...
            self._next_candidate_update_seq += 1
    ...
    def inject_hint(
        ...
        # 2026-07-19: suppress the hint entirely this VLM turn if next's own
        # cached estimate hasn't been refreshed by a fresh accepted attempt
        # since the previous VLM query -- see sequential_pair_report_next_
        # anchor_suppress_if_stale's docstring at __init__. Checked once per
        # VLM query (this method's only call site), not per environment step.
        suppress_stale = (
            self.sequential_pair_report_next_anchor_suppress_if_stale
            and self.sequential_pair_report_next_anchor
            and self._next_candidate_seq_at_last_hint is not None
            and self._next_candidate_update_seq == self._next_candidate_seq_at_last_hint
        )
        self._next_candidate_seq_at_last_hint = self._next_candidate_update_seq
        if suppress_stale:
    ...
    def _append_anchor(
        ...
        *,
        ...
        """anchor_pose/anchor_distance_m let a caller pin the anchor's
        registered identity to a moment earlier than "right now" (2026-07-16,
        used by _finalize_pending_anchor so a symmetric-window anchor's
        position stays exactly at its trigger crossing, not wherever forward
        accumulation happened to end -- see multiframe_anchor_symmetric_
        enabled's docstring at __init__). Default None preserves the
        original behavior exactly for every other caller."""
        pose_for_identity = anchor_pose if anchor_pose is not None else self._outbound_pose_from_start
        distance_for_identity = (
            anchor_distance_m if anchor_distance_m is not None else self._outbound_distance_m
        )
        previous_pose = self.anchors[-1].pose_from_start if self.anchors else [0.0, 0.0, 0.0]
        edge_from_previous = relative_delta(previous_pose, pose_for_identity)
        if self.multiframe_anchor_window > 1:
            merged = self._merge_outbound_frame_buffer(pose_for_identity)
            if merged is not None:
                descriptor = merged
        anchor = RouteAnchor(
            index=len(self.anchors),
            pose_from_start=[float(x) for x in pose_for_identity],
            distance_from_start_m=float(distance_for_identity),
            descriptor=descriptor,
            metadata=dict(metadata or {}),
            edge_from_previous=[float(x) for x in edge_from_previous],
        )
        self.anchors.append(anchor)
        self._last_anchor_distance_m = distance_for_identity

    def _begin_pending_anchor(self, descriptor: Optional[object], metadata: Optional[dict]) -> None:
        """Freezes the anchor's identity (pose/distance) at the exact
        moment the anchor-spacing trigger fires, then keeps accumulating
        real outbound motion for up to multiframe_anchor_forward_distance_m
        (or multiframe_anchor_forward_stall_updates calls, whichever comes
        first) before actually calling _append_anchor -- see
        multiframe_anchor_symmetric_enabled's docstring at __init__.
        _should_save_anchor() cannot re-fire while a pending anchor exists,
        so _last_anchor_distance_m is set here (trigger-to-trigger spacing,
        same semantics as today) rather than at finalize time."""
        self._pending_anchor = {
            "trigger_distance_m": float(self._outbound_distance_m),
            "trigger_pose": list(self._outbound_pose_from_start),
    ...
    def _maybe_finalize_pending_anchor(self) -> None:
        pending = self._pending_anchor
        if pending is None:
            return
        pending["updates_pending"] += 1
        forward_m = self._outbound_distance_m - pending["trigger_distance_m"]
        if (
            forward_m >= self.multiframe_anchor_forward_distance_m
            or pending["updates_pending"] >= self.multiframe_anchor_forward_stall_updates
        ):
            self._finalize_pending_anchor()

    def _finalize_pending_anchor(self) -> None:
        pending = self._pending_anchor
        self._pending_anchor = None
        lo = pending["trigger_distance_m"] - self.multiframe_anchor_backward_distance_m
        hi = pending["trigger_distance_m"] + self.multiframe_anchor_forward_distance_m
        frames = [
            (pose, desc) for (dist, pose, desc) in self._outbound_symmetric_frame_buffer
            if lo <= dist <= hi
        ]
        merged = self._merge_point_frames(frames, pending["trigger_pose"])
        descriptor = merged if merged is not None else pending["fallback_descriptor"]
        self._append_anchor(
            descriptor=descriptor, metadata=pending["metadata"],
            anchor_pose=pending["trigger_pose"], anchor_distance_m=pending["trigger_distance_m"],
        )
    ...
    def _merge_outbound_frame_buffer(self, anchor_pose: list[float]) -> Optional[dict]:
        ...
        submap expressed in anchor_pose's own body frame. Thin wrapper around
        _merge_point_frames -- kept so multiframe_anchor_window's own
        behavior is byte-identical after the 2026-07-16 refactor that
        generalized the merge math for reuse by the symmetric anchor window
        and the return-side frame buffer (see _finalize_pending_anchor /
        _merge_return_frame_buffer)."""
        return self._merge_point_frames(self._outbound_frame_buffer, anchor_pose)

    @staticmethod
    def _merge_point_frames(frames: list[tuple[list[float], object]], target_pose: list[float]) -> Optional[dict]:
        """Shared SE(2) reproject-and-concatenate math for every frame-merge
        consumer in this file: multiframe_anchor_window (backward-only,
        same-vantage-point), the symmetric backward+forward anchor
        accumulation, and the return-side live frame buffer. `frames` is a
        list of (pose_at_capture, descriptor) tuples; `target_pose` is the
        frame every frame's points get reprojected into. Each buffered
        frame's own points are transformed body(old-capture) -> world ->
        body(target) via the same SE(2) composition compose_pose/
    ...
    def _sequential_pair_closure_precheck(
        ...
        if len(by_anchor) < 2:
            return estimates, None
        # 2026-08-14: this is a real "the closure check would have run" attempt
        # (two distinct anchors on offer) -- count it toward the cooldown
        # before anything else, so the counter reflects attempts against the
        # CURRENT current anchor regardless of what the check below decides.
        # Skipping (not rejecting, not reconstructing) while still under
        # cooldown avoids feeding a numerically-degenerate near-zero-distance
        # bearing comparison into either the reject-veto or bounded_evidence's
        # vote history -- see this attribute's docstring at __init__ for why
        # neither ICP's own distance nor accumulated dead-reckoning can gate
        # this safely instead. 0 (default) preserves existing behavior exactly.
        self._closure_cooldown_counter += 1
        if self._closure_cooldown_counter <= self.sequential_pair_closure_cooldown_attempts:
            return estimates, None
    ...
    def _current_integrated_xy(self) -> list[float]:
        """[x, y] of self.current_absolute_pose_from_start()-equivalent
        return-phase pose, bounded to this file's existing per-step
        _return_pose_from_return_start integration (NOT the deleted
        unbounded arc-length odometer). Used only to snapshot/diff a short,
        bounded travel distance for the growing-uncertainty staleness
        variant -- see _current_stale_onset_pose's docstring at __init__.
        """
        current_pose = compose_pose(self._return_start_pose_from_start, self._return_pose_from_return_start)
    ...
    def _anchor_progress_from_estimate(
        self, estimate: AnchorRelocalization, role: Optional[str] = None
    ) -> Optional[RelativeStartProgress]:
        anchor = self._anchor_by_index(estimate.anchor_index)
        if anchor is None:
            return None
        integrated_progress = self._action_integrated_progress()
        ...
        # 2026-08-04 (cross-role agreement, stop_gate-review follow-up):
        # current and next roles' ICP matches are against DIFFERENT anchors'
        # point clouds -- a genuine independent cross-check, unlike anchor-
        # corroboration's own reuse of this same reading. Expose whichever
        # role is NOT the one this progress object was built from, converted
        # through the identical distance_to_start_m formula above, so
        # stop_gate can compare the two independently-derived distances
        # before trusting an episode-ending FORCE/ACCEPT. None whenever the
        # other role currently has no cached estimate at all, or that
        # estimate's anchor was since quarantined/removed -- "cannot verify"
        # is deliberately not treated as "disagrees" (mirrors this file's own
        # established convention for missing match_class/confidence data).
        other_estimate = (
            self._latest_relocalization if role == "next" else self._latest_next_candidate_relocalization
        )
        cross_role_distance_to_start_m: Optional[float] = None
        if other_estimate is not None:
            other_anchor = self._anchor_by_index(other_estimate.anchor_index)
            if other_anchor is not None:
                cross_role_distance_to_start_m = float(
                    other_estimate.distance_to_anchor_m + other_anchor.route_remaining_to_start_m
                )
        ...
        # 2026-08-04: staleness distrust (see relocalization_stale_attempts'
        # docstring). Picks the counter matching whichever role this progress
        # object was actually built from -- "current" falls back to it when
        # role is None too (mirrors _anchor_progress()'s own current-fallback).
        stale_attempts = self._next_stale_attempts if role == "next" else self._current_stale_attempts
        stale_distrust = bool(
            self.sequential_pair_stale_relocalization_distrust
            and stale_attempts >= self.stale_relocalization_max_attempts
        )
        if stale_distrust:
            reported_confidence = min(reported_confidence, self.reliability_low_confidence_floor)
        # 2026-08-04 (growing-uncertainty variant, see
        # sequential_pair_relocalization_uncertainty_mode's docstring at
        # __init__): continuously-widening alternative to the hard-threshold
        # stale_distrust above -- starts accumulating from the FIRST stale
        # attempt, not only after stale_relocalization_max_attempts. None
        # (not 0.0) whenever mode="fixed" or this role is fresh, so stop_gate
        # can tell "no interval info, use point d" apart from "a genuinely
        # zero-width interval."
        distance_uncertainty_radius_m: Optional[float] = None
        if self.sequential_pair_relocalization_uncertainty_mode == "growing" and stale_attempts > 0:
            onset_pose = self._next_stale_onset_pose if role == "next" else self._current_stale_onset_pose
            traveled_m = 0.0
            if onset_pose is not None:
                current_xy = self._current_integrated_xy()
                traveled_m = float(math.hypot(current_xy[0] - onset_pose[0], current_xy[1] - onset_pose[1]))
            distance_uncertainty_radius_m = self.stale_uncertainty_base_floor_m + traveled_m
        ...
        current_idx = self._target_anchor_index
        current_unreliable_diag = (
            self._current_persistently_unreliable(current_idx)
            if self.sequential_pair_reliability_demote_current
            else None
        )
        if not self.sequential_pair_evict_unreliable_current:
            evict_streak_diag = None
        elif self.current_evict_mode == "window":
            evict_streak_diag = sum(
                1 for cast in self._current_evict_vote_history.get(current_idx, []) if cast
            )
        elif self._current_evict_streak_anchor == current_idx:
            evict_streak_diag = self._current_evict_streak_count
        else:
            evict_streak_diag = 0
        return RelativeStartProgress(
            target_dx_m=float(dx),
            ...
            anchor_progress_role=role,
            current_target_anchor_index=(
                int(current_idx) if current_idx is not None else None
            ),
            current_anchor_unreliable=current_unreliable_diag,
            current_evict_streak=evict_streak_diag,
            relocalization_stale_attempts=int(stale_attempts),
    ...
    def _anchor_progress(self) -> Optional[RelativeStartProgress]:
        ...
        if estimate is None:
            return None
        return self._anchor_progress_from_estimate(estimate, role=role)
    ...
    def _make_anchor_hint(self, progress: RelativeStartProgress) -> str:
        ...
        # 2026-07-18: an oracle-flagged-inaccurate hint (research-only supervision
        # experiment, see RelativeStartProgress.oracle_supervision_suppressed) takes
        # the same hedged branch -- from this method's own perspective it can't tell
        # the two apart, and shouldn't need to.
        if self._filter_lost(progress) or progress.oracle_supervision_suppressed:
            std = progress.filter_std_m or 0.0
