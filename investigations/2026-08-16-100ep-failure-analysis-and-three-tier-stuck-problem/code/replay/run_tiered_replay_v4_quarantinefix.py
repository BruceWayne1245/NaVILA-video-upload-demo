"""v4: identical to run_tiered_replay_v2.py in every respect EXCEPT one bug
fix -- v2's quarantine_check was a stub (`self.quarantined` was declared but
NEVER populated, discovered 2026-08-16 while answering the user's question
"as a result of accidentally omitting quarantine, was v2's overall result
net-positive or net-negative"). This file restores the REAL trend-based
quarantine logic (byte-identical to stuck_candidate_quality.py's version) so
persistently-disagreeing candidates actually get skipped via
next_candidate_index, instead of sitting unconfirmed-and-unquarantined
forever. Promotion rule is UNCHANGED from v2 (still requires cs.confirmed --
no force-promote bypass) -- this isolates ONLY the effect of quarantine
actually working, not the separate force-promote question already tested in
v3.
...
OUT_PATH = os.path.join(REPLAY_DIR, "tiered_v4_quarantinefix_results.json")
PROGRESS_PATH = os.path.join(REPLAY_DIR, "tiered_v4_quarantinefix_progress.log")
...
class TieredSim:
    ...
    def quarantine_check(self, cs, candidate_idx, candidate_est, next_est_hist):
        """Trend-mode quarantine (restored 2026-08-16, byte-identical to
        stuck_candidate_quality.py's version -- v2's original was a stub that
        never populated self.quarantined). Compares the candidate's own
        implied distance-to-start against next's independently-derived
        distance-to-start across the candidate's whole dwell so far;
        quarantines once a majority of that history disagrees by more than
        QUARANTINE_TREND_SPREAD_M."""
        ...
        if bad_fraction > QUARANTINE_TREND_BAD_FRACTION:
            cs.quarantined = True
            self.quarantined.add(candidate_idx)
    ...
    def step(self, attempt, next_est_hist, candidate_est, candidate_rec):
        ...
        if candidate_est is not None:
            ...
            if not cs.confirmed and not cs.quarantined:
                self.quarantine_check(cs, candidate_idx, candidate_est, next_est_hist)
        ...
        self.trace.append((attempt, self.current_idx, next_idx, candidate_idx, cs.confirmed, cs.quarantined))
...
def replay_episode(ep, traj_id, v11_bundle):
    ...
    gaps = []
    n_quarantine_events = sum(1 for row in sim.trace if row[5])
    for att, cur_idx, next_idx, cand_idx, cand_conf, cand_q in sim.trace:
    ...
    return dict(ep=ep, n_attempts=n_att, gaps=gaps, final_current_idx=sim.current_idx,
                n_quarantine_events=n_quarantine_events, trace_tail=sim.trace[-5:])
