...
main() {
  ...
  echo "Path-following geometry M50: outbound+confirm use NaVILA exactly as the online comparator; return uses non-oracle accumulated breadcrumbs + sequential-pair ICP + deterministic controller. No return VLM query, route hint, hint/action arbiter, stop gate, or stuck-recovery policy." | tee -a "${LOG_DIR}/batch.log"
