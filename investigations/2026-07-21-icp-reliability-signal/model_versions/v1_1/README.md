# NaVILA Reliability V1.1 development

This repository was branched from the frozen Reliability V1 offline-audit
commit. V1 remains unchanged at `/home/teambruce/navila-reliability-v1`.
...
5. `tools/diagnose_v1_shift.py` diagnoses calibration transfer, feature drift,
   and scene/role/return-stage subgroup failures before V1.1 feature work.
6. `tools/build_v11_dataset.py` joins the audited V1 labels back to raw records
   and writes a compact NPZ containing complete top-4 basins, yaw/scan-context,
   current-next disagreement, and strictly causal 4/8/16/32-anchor histories.
7. `tools/train_v11_nested.py` compares logistic/V1, HGB/V1, basin,
   basin+pair, and full-temporal candidates with physical-episode-grouped nested
   CV, inner-OOF calibration, and cluster-bootstrap upper-risk thresholds.
8. `tools/validate_v11_artifact.py` rejects identity/GT feature leakage and
   validates schema locks and sampled inference for the development artifact.
9. The candidate navigation scripts initially run the estimator in shadow mode.
