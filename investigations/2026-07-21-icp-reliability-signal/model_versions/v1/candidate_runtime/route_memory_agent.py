...
class RelativeStartProgress:
    ...
    reliability_bearing_bad_probability: Optional[float] = None
    reliability_distance_bad_probability: Optional[float] = None
    reliability_pose_bad_probability: Optional[float] = None
    reliability_bearing_trusted: Optional[bool] = None
    reliability_distance_trusted: Optional[bool] = None
    reliability_pose_trusted: Optional[bool] = None
    reliability_status: Optional[str] = None
    reliability_model_version: Optional[str] = None
...
class AnchorRelocalization:
    ...
    # Reliability-v1 raw scalar inputs. They remain separate from confidence:
    # ICP self-confidence and learned task reliability answer different questions.
    overlap_ratio: Optional[float] = None
    mean_residual_m: Optional[float] = None
    median_residual_m: Optional[float] = None
    anchor_points: Optional[int] = None
    current_points: Optional[int] = None
    anchor_z_span_m: Optional[float] = None
    current_z_span_m: Optional[float] = None
    icp_basin_count: Optional[int] = None
    icp_ambiguity: Optional[str] = None
    icp_best_to_second_rotation_delta_deg: Optional[float] = None
    icp_best_to_second_translation_delta_m: Optional[float] = None
    localizability_min_eigenvalue: Optional[float] = None
    localizability_min_normalized_eigenvalue: Optional[float] = None
    localizability_condition_number: Optional[float] = None
    # Shadow outputs. No candidate-selection or downstream gate reads these in
    # reliability-v1; they are propagated solely for logging and replay.
    reliability_bearing_bad_probability: Optional[float] = None
    reliability_distance_bad_probability: Optional[float] = None
    reliability_pose_bad_probability: Optional[float] = None
    reliability_bearing_trusted: Optional[bool] = None
...
class RouteMemoryAgent:
    ...
    def update_relocalization(
        ...
        self.relocalization_events.append({
            ...
            "accepted": True,
            "reliability": {
                "p_bearing_bad_30": estimate.reliability_bearing_bad_probability,
                "p_distance_bad_0p5": estimate.reliability_distance_bad_probability,
                "p_pose_bad": estimate.reliability_pose_bad_probability,
                "bearing_trusted": estimate.reliability_bearing_trusted,
                "distance_trusted": estimate.reliability_distance_trusted,
                "pose_trusted": estimate.reliability_pose_trusted,
                "status": estimate.reliability_status,
                "model_version": estimate.reliability_model_version,
                "policy": estimate.reliability_policy,
            },
        })
        return estimate
    ...
    def _coerce_relocalization(
        ...
        return AnchorRelocalization(
            ...
            overlap_ratio=relocalization.get("overlap_ratio"),
            mean_residual_m=relocalization.get("mean_residual_m"),
            median_residual_m=relocalization.get("median_residual_m"),
            anchor_points=relocalization.get("anchor_points"),
            current_points=relocalization.get("current_points"),
            anchor_z_span_m=relocalization.get("anchor_z_span_m"),
            current_z_span_m=relocalization.get("current_z_span_m"),
            icp_basin_count=relocalization.get("icp_basin_count"),
            icp_ambiguity=relocalization.get("icp_ambiguity"),
            icp_best_to_second_rotation_delta_deg=relocalization.get("icp_best_to_second_rotation_delta_deg"),
            icp_best_to_second_translation_delta_m=relocalization.get("icp_best_to_second_translation_delta_m"),
            localizability_min_eigenvalue=relocalization.get("localizability_min_eigenvalue"),
            localizability_min_normalized_eigenvalue=relocalization.get("localizability_min_normalized_eigenvalue"),
            localizability_condition_number=relocalization.get("localizability_condition_number"),
        )
    ...
    def _anchor_progress_from_estimate(self, estimate: AnchorRelocalization) -> Optional[RelativeStartProgress]:
        ...
        return RelativeStartProgress(
            ...
            relocalization_backend=estimate.backend,
            filter_std_m=float(filter_std_m),
            reliability_bearing_bad_probability=estimate.reliability_bearing_bad_probability,
            reliability_distance_bad_probability=estimate.reliability_distance_bad_probability,
            reliability_pose_bad_probability=estimate.reliability_pose_bad_probability,
            reliability_bearing_trusted=estimate.reliability_bearing_trusted,
            reliability_distance_trusted=estimate.reliability_distance_trusted,
            reliability_pose_trusted=estimate.reliability_pose_trusted,
            reliability_status=estimate.reliability_status,
            reliability_model_version=estimate.reliability_model_version,
            reliability_policy=estimate.reliability_policy,
        )
