...
def sequential_pair_anchor_relocalization(
    ...
    for anchor in (current_anchor, next_anchor):
        ...
        candidate = AnchorRelocalization(
            ...
            mean_residual_m=float(result["mean_residual_m"]),
            median_residual_m=residual,
            anchor_points=int(record["anchor_points"]),
            current_points=int(record["current_points"]),
            anchor_z_span_m=(
                float(record["anchor_z_span_m"])
                if record.get("anchor_z_span_m") is not None else None
            ),
            current_z_span_m=(
                float(record["current_z_span_m"])
                if record.get("current_z_span_m") is not None else None
            ),
            icp_basin_count=int(basin_metrics["basin_count"]),
            icp_ambiguity=str(basin_metrics["ambiguity"]),
            icp_best_to_second_rotation_delta_deg=(
                float(basin_metrics["best_to_second_rotation_delta_deg"])
                if basin_metrics.get("best_to_second_rotation_delta_deg") is not None else None
            ),
            icp_best_to_second_translation_delta_m=(
                float(basin_metrics["best_to_second_translation_delta_m"])
                if basin_metrics.get("best_to_second_translation_delta_m") is not None else None
            ),
            localizability_min_eigenvalue=(
                float(min(localizability["eigenvalues"]))
                if localizability.get("eigenvalues") else None
            ),
            localizability_min_normalized_eigenvalue=(
                float(localizability["min_normalized_eigenvalue"])
                if localizability.get("min_normalized_eigenvalue") is not None else None
            ),
            localizability_condition_number=(
                float(localizability["condition_number"])
                if localizability.get("condition_number") is not None else None
            ),
        )
        record["estimated_distance_to_anchor_m"] = float(candidate.distance_to_anchor_m)
        record["estimated_bearing_to_anchor_deg"] = float(candidate.bearing_to_anchor_deg)
