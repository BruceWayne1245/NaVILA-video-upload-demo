...
EVAL_SCRIPT="${BENCH}/scripts/round_trip_eval.py"
PORT_BASE="${PORT_BASE:-54321}"
PORT="${PORT:-54321}"
RUN_TAG="${RUN_TAG:-reliability_v1_shadow_pilot_20260721}"
...
START_AT="${START_AT:-0}"
ONLY_EPISODES="${ONLY_EPISODES:-20}"
...
if [[ "${RELIABILITY_ISOLATED_OVERLAY:-}" != "1" ]]; then
  echo "Refusing to run without the reliability-v1 mount-namespace overlay." >&2
  echo "Use /home/teambruce/navila-reliability-v1/tools/run_shadow_pilot_ep20.sh." >&2
  exit 97
fi
