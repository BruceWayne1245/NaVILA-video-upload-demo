...
parser.add_argument(
    "--reliability_model_path",
    type=str,
    default="",
    help=(
        "Path to the isolated reliability-v1 portable JSON. When provided, candidates are scored and logged "
        "in shadow mode; navigation decisions remain unchanged."
    ),
)
parser.add_argument(
    "--reliability_mode",
    choices=("shadow",),
    default="shadow",
    help="Reliability-v1 is intentionally shadow-only until a prospective batch passes its safety gates.",
)
parser.add_argument("--reliability_pose_high_risk_threshold", type=float, default=0.7, help=argparse.SUPPRESS)
parser.add_argument("--reliability_high_risk_consecutive", type=int, default=10, help=argparse.SUPPRESS)
parser.add_argument("--reliability_release_after_attempts", type=int, default=200, help=argparse.SUPPRESS)
parser.add_argument(
    "--hint_action_arbiter",
    action="store_true",
    default=False,
...
def route_progress_to_record(progress, configured_source):
    ...
    return {
        ...
        "reliability": {
            "p_bearing_bad_30": progress.reliability_bearing_bad_probability,
            "p_distance_bad_0p5": progress.reliability_distance_bad_probability,
            "p_pose_bad": progress.reliability_pose_bad_probability,
            "bearing_trusted": progress.reliability_bearing_trusted,
            "distance_trusted": progress.reliability_distance_trusted,
            "pose_trusted": progress.reliability_pose_trusted,
            "status": progress.reliability_status,
            "model_version": progress.reliability_model_version,
            "policy": progress.reliability_policy,
        },
    }
...
def main():
    ...
    if args_cli.reliability_model_path:
        from reliability_integration import ReliabilityShadowRuntime

        reliability_runtime = ReliabilityShadowRuntime(
            args_cli.reliability_model_path,
            mode=args_cli.reliability_mode,
            pose_high_risk_threshold=args_cli.reliability_pose_high_risk_threshold,
            high_risk_consecutive=args_cli.reliability_high_risk_consecutive,
            release_after_attempts=args_cli.reliability_release_after_attempts,
        )
        print(
            f"[reliability] shadow model loaded: {reliability_runtime.model_version} "
            f"artifact={args_cli.reliability_model_path}",
            flush=True,
        )
    ...
    if reliability_runtime is not None and route_relocalizer is not None:
        base_route_relocalizer = route_relocalizer

        def reliability_scored_relocalizer(descriptor, anchors):
            candidates = base_route_relocalizer(descriptor, anchors)
            return reliability_runtime.score_candidates(candidates, route_relocalization_diagnostics)

        route_relocalizer = reliability_scored_relocalizer
    ...
    measurements["round_trip"] = {
        ...
        "reliability": {
            "enabled": reliability_runtime is not None,
            "mode": args_cli.reliability_mode if reliability_runtime is not None else "off",
            "model_path": args_cli.reliability_model_path or None,
            "model_version": (
                reliability_runtime.model_version if reliability_runtime is not None else None
            ),
            "pose_high_risk_threshold": float(args_cli.reliability_pose_high_risk_threshold),
            "high_risk_consecutive": int(args_cli.reliability_high_risk_consecutive),
            "release_after_attempts": int(args_cli.reliability_release_after_attempts),
