...
MODEL_SCHEMA = "navila-route2-core-downstream-bundle-v1"
...
def load_features(
    ...
    for row in read_jsonl_gz(data_dir / filename):
        ...
        required_head = "pose" if task == "anchor_transition" else "distance"
        feature = filter_core_features(
            feature, required_head=required_head
        )
...
def fit_task(
    ...
    bundle = {
        ...
        "task": task,
        "required_v11_head": (
            "pose" if task == "anchor_transition" else "distance"
        ),
        "raw_icp_quality_authority": False,
        "legacy_u_authority": False,
    ...
    model_path = model_dir / f"{task}_core_v1.joblib"
