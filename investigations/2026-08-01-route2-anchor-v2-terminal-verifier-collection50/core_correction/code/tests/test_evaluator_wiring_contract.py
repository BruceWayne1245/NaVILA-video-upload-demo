...
def test_terminal_a0_collection_is_explicit_and_eager_before_unchanged_gate():
    flag = EVALUATOR.index(
        '"--terminal_a0_probe_all_return_queries_shadow"'
    )
    eager = EVALUATOR.index(
        "if args_cli.terminal_a0_probe_all_return_queries_shadow:", flag
    )
    probe = EVALUATOR.index("_probe_a0_visual_home()", eager)
    gate = EVALUATOR.index("_last_gate_decision = stop_gate.check(", probe)
    assert flag < eager < probe < gate
    assert '"event": "stop_gate_a0_visual_probe"' in EVALUATOR
