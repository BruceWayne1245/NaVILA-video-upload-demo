...
parser.add_argument(
    ...
    default=False,
    help=(
        "(2026-07-27, Route 2 recovery; off by default.) Populate synthetic "
        "anchor A0 from the evaluator's reset-time, zero-command RGB-D/local-map "
        "observation. A0's pose and route-distance identity remain exactly zero; "
        "only its descriptor and capture metadata are backfilled so the terminal "
        "A1/A0 sequential pair has two real ICP candidates."
    ),
)
parser.add_argument(
...
parser.add_argument(
    ...
    default="off",
    help=(
        "Frozen Anchor V1 previous-complete-attempt promotion guard. "
        "Active may only defer a baseline promotion and is bounded/fail-open."
    ),
)
parser.add_argument(
...
def main():
    ...
    def _evaluate_anchor_transition_promotion_guard(**proposal):
        ...
        if anchor_transition_pending is None:
            return baseline
        if (
            args_cli.anchor_transition_guard_mode == "active"
            and os.path.exists(
                os.path.abspath(
    ...
    if args_cli.anchor_transition_guard_mode != "off":
        ...
        if (
            float(args_cli.anchor_transition_guard_confidence_threshold)
            < 0.90
        ):
            raise RuntimeError(
    ...
    if args_cli.reliability_v11_online_shadow:
        ...
        if args_cli.anchor_transition_guard_mode != "off":
            ...
            anchor_transition_online = OnlineAnchorTransitionV1(
