...
# This candidate entry point lives outside NaVILA-Bench/scripts so the live
# working tree remains untouched during the canary. Keep its own directory
# first for candidate modules, then expose unchanged live-only helpers such as
# cli_args, instruction_rewriter, and topdown_route_map.
LIVE_BENCH_SCRIPTS = os.environ.get(
    "NAVILA_BENCH_SCRIPTS",
    "/mnt/SSD4T/teambruce/projects/navila-isaac/NaVILA-Bench/scripts",
)
if LIVE_BENCH_SCRIPTS not in sys.path:
    sys.path.append(LIVE_BENCH_SCRIPTS)
...
parser.add_argument(
    ...
    default=os.path.join(
        LIVE_BENCH_SCRIPTS, "generated", "reversed_instructions.json"
    ),
)
...
def main():
    ...
    log_root_path = os.path.join(
        LIVE_BENCH_SCRIPTS, "../logs", "rsl_rl", agent_cfg.experiment_name
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"[candidate-entry] main_exception={type(exc).__name__}: {exc}",
            flush=True,
        )
        traceback.print_exc()
        raise
