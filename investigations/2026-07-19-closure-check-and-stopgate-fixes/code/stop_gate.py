...
class ReturnStopGate:
    ...
    def check(
        ...
        if d is None:
            ...
            self._anchor_close_streak = 0
            return GateDecision("pass", None, conf if conf else 0.0, is_teleport)

        # Teleport frame: don't count toward arrival, don't arbitrate
        if is_teleport:
            return GateDecision("pass", d, conf, is_teleport)
        ...
        if not vlm_issued_stop and ((high_conf and self._confirm_count >= self.confirm_steps) or anchor_forces):
            return GateDecision("forced", d, conf, is_teleport)
        ...
        if not high_conf:
            ...
            return GateDecision("deferred", d, conf, is_teleport)

        if d <= self.r_in:
            # Inside arrival radius: accept
            return GateDecision("accepted", d, conf, is_teleport)
