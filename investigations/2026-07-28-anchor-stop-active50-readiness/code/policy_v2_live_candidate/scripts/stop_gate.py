...
class GateDecision:
    ...
    pre_stop_blind_query_count: int = 0
    ...
    def as_log_dict(self) -> Dict[str, Any]:
        result: Dict[str, Any] = {
            ...
            "gate_pre_stop_blind_query_count": int(
                self.pre_stop_blind_query_count
            ),
        }
...
class ReturnStopGate:
    ...
    def __init__(
        ...
        *,
        ...
        hold_steps: int = 10,
        pre_stop_blind_trigger_queries: int = 4,
        terminal_corridor_margin_m: float = 1.0,
    ) -> None:
        if r_in > r_out:
        ...
        for name, value in (
            ...
            ("pre_stop_blind_trigger_queries", pre_stop_blind_trigger_queries),
        ...
        self.pre_stop_blind_trigger_queries = int(
            pre_stop_blind_trigger_queries
        )
        if self.pre_stop_blind_trigger_queries > self.blind_max_queries:
            raise ValueError(
                "pre_stop_blind_trigger_queries must be <= blind_max_queries"
            )
        self.terminal_corridor_margin_m = max(
            0.0,
            float(terminal_corridor_margin_m),
        )
        ...
        self._pre_stop_blind_query_count = 0
        self._terminal_corridor_armed = False
    ...
    @property
    def home_visual_probe_requested(self) -> bool:
        return bool(
            self.state == TERMINAL_BLIND
            or (
                self._terminal_corridor_armed
                and self._pre_stop_blind_query_count + 1
                >= self.pre_stop_blind_trigger_queries
            )
        )
    ...
    def check(
        ...
        *,
        ...
        if direct_raw and interval is not None:
            ...
            self._travel_since_trusted_m = 0.0
            self._terminal_corridor_armed = bool(
                interval[0]
                <= self.r_out + self.terminal_corridor_margin_m
            )
        ...
        if not vlm_issued_stop:
            ...
            if self._terminal_corridor_armed:
                self._pre_stop_blind_query_count += 1
                if (
                    self._pre_stop_blind_query_count
                    >= self.pre_stop_blind_trigger_queries
                ):
                    # Count the pre-STOP unknown queries against the same
                    # total blind budget.  Entering the blind state must not
                    # grant a fresh second budget after navigation has
                    # already been blind for several VLM decisions.
                    self._enter_blind(
                        initial_blind_queries=(
                            self._pre_stop_blind_query_count - 1
                        )
                    )
                    visual = (
                    ...
                    return self._advance_blind(
                        d,
                        conf,
                        "pre_stop_terminal_signal_loss",
                        self._expanded_last_trusted_interval(),
                        visual=visual,
                    )
            return self._decision(
                "pass",
    ...
    def _enter_blind(self, *, initial_blind_queries: int = 0) -> None:
        self.state = TERMINAL_BLIND
        self._verify_query_count = 0
        self._blind_query_count = max(0, int(initial_blind_queries))
        self._near_evidence_streak = 0
    ...
    def _decision(
        ...
        *,
        ...
        return GateDecision(
            ...
            pre_stop_blind_query_count=int(
                self._pre_stop_blind_query_count
            ),
        )
