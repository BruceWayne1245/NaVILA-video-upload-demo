...
PORT="${PORT:-54321}"
RUN_TAG="${RUN_TAG:-pure_oracle_hint_100ep_20260811}"
START_AT="${START_AT:-0}"
...
main() {
  ...
  echo "Pure oracle_hint (2026-08-11): identical to pure_navila_baseline_100ep_20260810 (--round_trip_mode=phase_prompt --instruction_rewriter_provider=cache_only) PLUS ONLY --route_memory --route_hint_mode=compact --route_hint_source=oracle --route_relocalization_backend=none. Deliberately NO --stop_gate (its FORCED-terminal path auto-declares return success once inside r_in for confirm_steps consecutive VLM-query steps, and stop_gate treats any 'oracle' progress source as full confidence -- see scripts/stop_gate.py's _extract_d_and_conf -- so leaving it on would let the gate itself, not the VLM's own stop decision, drive success). Also deliberately NO --oracle_align_return_yaw_to_anchor_segment (that actively corrects the robot's return-phase yaw using ground truth, a second oracle intervention beyond the hint) and NO --hint_action_arbiter (that's the separate oracle_hint_action condition). This reproduces byte-identical route-memory flags to the 2026-06-29 direct_oracle_hard_fresh_20260629 batch (verified against that batch's own eval_log argv), just on baseline's full 100-episode canonical set instead of the hard-11 subset." | tee -a "${LOG_DIR}/batch.log"
  echo "Episode timeout: ${EPISODE_TIMEOUT_SECONDS}s (kill-after ${EPISODE_TIMEOUT_KILL_AFTER_SECONDS}s)" | tee -a "${LOG_DIR}/batch.log"

  # Same 100-episode canonical set as pure_navila_baseline_100ep_20260810 /
  # canonical_report_next_stopgate_100ep_20260720 (verified byte-identical via
  # diff against run_pure_baseline_100ep_20260810.sh's run_episode lines).
