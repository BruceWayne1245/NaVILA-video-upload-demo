    ...
    for anchor in candidates_to_search:
        ...
        record = {
            ...
            "anchor_role": (
                ...
                and int(anchor.index) == int(current_anchor.index)
                else "next"
                if next_anchor is not None
                and int(anchor.index) == int(next_anchor.index)
                else "other"
            ),
...
def sequential_pair_anchor_relocalization(
    ...
    additional_anchors: tuple[object, ...] = (),
    diagnostics: Optional[dict] = None,
    return_candidates: bool = False,
    corridor_degeneracy_skip_threshold: float = 0.15,
    heading_consistency_max_error_rad: float = math.radians(90.0),
    ...
    """Relocalize against the two active support roles plus Route-2 probes.

    ``additional_anchors`` are reliability probes only.  The caller must keep
    them out of Route1 closure, selection, promotion and hint authority.  They
    exist so VLM-only recovery can continue matching every forward anchor
    until one becomes trustworthy again.

    Relocalize against two known anchors -- the one the caller is
    ...
    for anchor in (current_anchor, next_anchor, *tuple(additional_anchors)):
        if anchor is None:
            continue
        anchor_index = int(anchor.index)
