...
class RouteMemoryAgentTest(unittest.TestCase):
    ...
    def _anchor_progress_for_oracle_supervision_test(self, oracle_supervision_suppressed):
        return RelativeStartProgress(
            ...
            relocalization_backend="oracle_direct_route_anchor",
            filter_std_m=None,
            oracle_supervision_suppressed=oracle_supervision_suppressed,
        )

    def test_oracle_supervision_suppressed_false_is_byte_identical_to_before_the_field_existed(self):
        # 2026-07-18: default False must reproduce the exact prior hint text --
        # this is the no-behavior-change guarantee for every existing batch.
        agent = RouteMemoryAgent(enabled=True, anchor_spacing_m=1.0)
        agent.update_outbound_motion([1.0, 0.0, 0.0])
        agent.update_outbound_motion([1.0, 0.0, 0.0])
        agent.finalize_outbound()
        progress = self._anchor_progress_for_oracle_supervision_test(oracle_supervision_suppressed=False)

        instruction, _ = agent.inject_hint("Return to start.", step=20, progress_override=progress)

        self.assertIn("route anchor A1", instruction)
        self.assertNotIn("position uncertain", instruction)
        self.assertNotIn("filter lost lock", instruction)

    def test_oracle_supervision_suppressed_true_falls_back_to_hedged_wording(self):
        # 2026-07-18: research-only oracle supervision experiment -- when
        # round_trip_eval.py has flagged this hint as oracle-verified-inaccurate,
        # _make_anchor_hint must take the same hedged branch as a lost particle
        # filter, not emit the specific (oracle-flagged-wrong) numeric hint.
        agent = RouteMemoryAgent(enabled=True, anchor_spacing_m=1.0)
        agent.update_outbound_motion([1.0, 0.0, 0.0])
        agent.update_outbound_motion([1.0, 0.0, 0.0])
        agent.finalize_outbound()
        progress = self._anchor_progress_for_oracle_supervision_test(oracle_supervision_suppressed=True)

        instruction, event = agent.inject_hint("Return to start.", step=20, progress_override=progress)

        self.assertIn("position uncertain", instruction)
        self.assertIn("do NOT stop until you visually confirm", instruction)
        self.assertIn("route anchor A1", instruction)  # anchor identity still named
        self.assertNotIn("dx=0.80 m", instruction)  # but not the specific numeric vector
        self.assertTrue(event["progress"]["oracle_supervision_suppressed"])
...
class SequentialPairReportNextAnchorSuppressIfStaleTest(unittest.TestCase):
    """2026-07-19: the simplest fix for the freezing bug found this session --
    next's own cached estimate (_latest_next_candidate_relocalization) has no
    time/attempt-based staleness handling at all, so a long streak of
    rejected/unconfirmed attempts leaves the hint reporting a byte-identical
    stale reading for many consecutive VLM turns (confirmed on real data: 19
    consecutive frozen hints / ~1075 steps / ~215 underlying ICP attempts in
    one episode). Deliberately not an odometry-based propagation fix (user's
    explicit call -- odometry drifts too easily to trust here): if next
    hasn't had a fresh accepted estimate since the previous VLM query,
    suppress the hint entirely this turn rather than repeat a known-stale
    number."""
    ...
    def _far_next_reading(self, dx: float = -3.0) -> AnchorRelocalization:
        return AnchorRelocalization(anchor_index=1, anchor_dx_m=dx, anchor_dy_m=0.5, anchor_dtheta_rad=0.0,
                                     confidence=0.6, backend="sequential_pair", inlier_count=200)

    def test_default_flag_is_off(self):
        agent = RouteMemoryAgent(enabled=True)
        self.assertFalse(agent.sequential_pair_report_next_anchor_suppress_if_stale)

    def test_noop_when_report_next_anchor_is_off(self):
        """Suppression must never engage unless report_next_anchor is also on --
        current's own reporting path is unaffected by this flag entirely."""
        agent = self._agent_with_pair(
            sequential_pair_report_next_anchor=False,
            sequential_pair_report_next_anchor_suppress_if_stale=True,
        )
        agent.update_relocalization(relocalization=[self._current_reading(), self._far_next_reading()])
        instruction, event = agent.inject_hint("base instruction", step=0)
        self.assertIsNotNone(event, "first hint must never be suppressed")
        # no further next reading between these two calls -- would suppress if report_next_anchor were on
        instruction2, event2 = agent.inject_hint("base instruction", step=1)
        self.assertIsNotNone(event2, "current's own reporting path must be unaffected by this flag")
        self.assertIn("System Hint", instruction2)

    def test_first_hint_never_suppressed(self):
        """No prior VLM-query reference point exists yet -- must not suppress
        the very first hint of the episode just because there's nothing to
        compare against."""
        agent = self._agent_with_pair(
        ...
        instruction, event = agent.inject_hint("base instruction", step=0)
        self.assertIsNotNone(event)
        self.assertIn("System Hint", instruction)

    def test_hint_suppressed_when_next_not_refreshed_since_last_vlm_query(self):
        agent = self._agent_with_pair(
            sequential_pair_report_next_anchor=True,
            sequential_pair_report_next_anchor_suppress_if_stale=True,
        )
        agent.update_relocalization(relocalization=[self._current_reading(), self._far_next_reading()])
        instruction1, event1 = agent.inject_hint("base instruction", step=0)
        self.assertIsNotNone(event1, "first hint must not be suppressed")

        # Only current gets a fresh reading this attempt -- next's ICP failed
        # (e.g. rejected/no candidate), matching the real freezing scenario.
        agent.update_relocalization(relocalization=[self._current_reading()])
        instruction2, event2 = agent.inject_hint("base instruction", step=25)
        self.assertIsNone(event2, "hint must be suppressed -- next has not been refreshed since the last VLM query")
        self.assertEqual(instruction2, "base instruction", "must fall back to the plain instruction, no hint text at all")

    def test_hint_not_suppressed_when_next_refreshed_since_last_vlm_query(self):
        agent = self._agent_with_pair(
            sequential_pair_report_next_anchor=True,
            sequential_pair_report_next_anchor_suppress_if_stale=True,
        )
        ...
        # A fresh (different) next reading lands before the next VLM query.
        agent.update_relocalization(relocalization=[self._current_reading(), self._far_next_reading(dx=-2.5)])
        instruction2, event2 = agent.inject_hint("base instruction", step=25)
        self.assertIsNotNone(event2, "must not suppress -- next was refreshed since the last VLM query")
        self.assertIn("System Hint", instruction2)

    def test_repeated_suppression_across_multiple_stale_vlm_queries(self):
        """Mirrors the real freezing scenario more closely: several consecutive
        VLM queries in a row, none of which see a fresh next reading in
        between -- every one after the first must be suppressed, not just the
        first stale one."""
        agent = self._agent_with_pair(
            sequential_pair_report_next_anchor=True,
            sequential_pair_report_next_anchor_suppress_if_stale=True,
        ...
        agent.update_relocalization(relocalization=[self._current_reading(), self._far_next_reading()])
        _, event0 = agent.inject_hint("base instruction", step=0)
        self.assertIsNotNone(event0)
        for step in (25, 50, 75, 100):
            agent.update_relocalization(relocalization=[self._current_reading()])  # next never refreshes
            instruction, event = agent.inject_hint("base instruction", step=step)
            self.assertIsNone(event, f"step {step}: must stay suppressed across repeated stale queries")
            self.assertEqual(instruction, "base instruction")
...
class MultiframeAnchorSymmetricWindowTest(unittest.TestCase):
    """2026-07-16: separate, opt-in mechanism from multiframe_anchor_window
    above (which merges frames captured BEFORE the anchor-spacing trigger
    only, as a fixed step count -- confirmed 2026-07-08/09 not to help
    genuine rotational self-similarity, since a short one-sided window
    sampled from essentially the same vantage point sees the same repeating
    local structure). This instead accumulates real DISTANCE both BEFORE and
    AFTER the trigger crossing -- motivated by a 2026-07-16 ground-truth
    check finding 9/10 known-bad anchors have a real turn within 0-5.3m of
    the anchor's own position (mostly 0-2m or ~0.00m): the landmark
    objectively exists nearby, a single instantaneous capture at an
    arbitrary trigger moment just frequently misses it, on either side."""

    def test_default_flag_is_off(self):
        agent = RouteMemoryAgent(enabled=True)
        self.assertFalse(agent.multiframe_anchor_symmetric_enabled)

    def test_symmetric_window_merges_backward_and_forward_frames_hand_computed(self):
        agent = RouteMemoryAgent(
            enabled=True, anchor_spacing_m=1.0, multiframe_anchor_symmetric_enabled=True,
            multiframe_anchor_backward_distance_m=0.5, multiframe_anchor_forward_distance_m=0.5,
        )
        pt = np.array([[1.0, 0.0]])
        agent.update_outbound_motion([0.5, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=0.5, no trigger
        self.assertEqual(len(agent.anchors), 1, "no anchor yet -- spacing not crossed")
        agent.update_outbound_motion([0.5, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.0, TRIGGER
        self.assertIsNotNone(agent._pending_anchor, "anchor must be pending, not yet finalized")
        self.assertEqual(len(agent.anchors), 1, "still just the start anchor -- forward accumulation not done")
        agent.update_outbound_motion([0.3, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.3
        agent.update_outbound_motion([0.3, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.6 >= 1.0+0.5 -> finalize
        self.assertIsNone(agent._pending_anchor)
        self.assertEqual(len(agent.anchors), 2)
        merged = agent.anchors[-1].descriptor["local_map_points_body"]
        # frames at dist 0.5, 1.0, 1.3 fall within [trigger-0.5, trigger+0.5] = [0.5, 1.5];
        # the dist=1.6 frame (captured after the finalize decision) must NOT be included.
        self.assertEqual(len(merged), 3)
        np.testing.assert_allclose(sorted(merged[:, 0].tolist()), [0.5, 1.0, 1.3], atol=1e-5)

    def test_anchor_pose_and_distance_pinned_to_trigger_not_finalize_time(self):
        agent = RouteMemoryAgent(
            enabled=True, anchor_spacing_m=1.0, multiframe_anchor_symmetric_enabled=True,
            multiframe_anchor_backward_distance_m=0.5, multiframe_anchor_forward_distance_m=0.5,
        )
        pt = np.array([[1.0, 0.0]])
        agent.update_outbound_motion([1.0, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.0, TRIGGER
        agent.update_outbound_motion([0.6, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.6, finalizes
        anchor = agent.anchors[-1]
        self.assertAlmostEqual(anchor.distance_from_start_m, 1.0, places=5,
                                msg="must be pinned to the trigger distance, not 1.6")
        np.testing.assert_allclose(anchor.pose_from_start, [1.0, 0.0, 0.0], atol=1e-5)
        # edge_from_previous is computed from the same pinned trigger pose.
        np.testing.assert_allclose(anchor.edge_from_previous, [1.0, 0.0, 0.0], atol=1e-5)

    def test_should_save_anchor_does_not_refire_while_pending(self):
        agent = RouteMemoryAgent(
            enabled=True, anchor_spacing_m=1.0, multiframe_anchor_symmetric_enabled=True,
            multiframe_anchor_backward_distance_m=0.5, multiframe_anchor_forward_distance_m=2.0,
        )
        pt = np.array([[1.0, 0.0]])
        agent.update_outbound_motion([1.0, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.0, TRIGGER
        # keep moving forward well past a SECOND anchor_spacing_m worth of distance
        # while still inside the (large) forward window -- must not create anchor #2 yet.
        for _ in range(3):
            agent.update_outbound_motion([0.5, 0.0, 0.0], descriptor={"local_map_points_body": pt})
        self.assertEqual(len(agent.anchors), 1, "must not re-fire while a pending anchor is accumulating")
        self.assertIsNotNone(agent._pending_anchor)

    def test_finalize_outbound_flushes_pending_anchor_before_final_anchor_check(self):
        agent = RouteMemoryAgent(
            enabled=True, anchor_spacing_m=1.0, multiframe_anchor_symmetric_enabled=True,
            multiframe_anchor_backward_distance_m=0.5, multiframe_anchor_forward_distance_m=5.0,
        )
        pt = np.array([[1.0, 0.0]])
        agent.update_outbound_motion([1.0, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.0, TRIGGER
        agent.update_outbound_motion([0.3, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.3, still pending
        self.assertIsNotNone(agent._pending_anchor)
        agent.finalize_outbound()
        self.assertIsNone(agent._pending_anchor, "outbound ending mid-accumulation must flush, not drop, the pending anchor")
        # the flushed anchor (trigger distance 1.0) plus the "outbound_final" anchor
        # check: since the flushed anchor's distance (1.0) < _outbound_distance_m (1.3),
        # a genuine second "outbound_final" anchor is also appended -- both present.
        distances = [a.distance_from_start_m for a in agent.anchors]
        self.assertIn(1.0, [round(d, 5) for d in distances])

    def test_forward_stall_releases_pending_anchor(self):
        agent = RouteMemoryAgent(
            enabled=True, anchor_spacing_m=1.0, multiframe_anchor_symmetric_enabled=True,
            multiframe_anchor_backward_distance_m=0.5, multiframe_anchor_forward_distance_m=5.0,
            multiframe_anchor_forward_stall_updates=2,
        )
        pt = np.array([[1.0, 0.0]])
        agent.update_outbound_motion([1.0, 0.0, 0.0], descriptor={"local_map_points_body": pt})  # dist=1.0, TRIGGER (updates_pending -> 1)
        self.assertIsNotNone(agent._pending_anchor, "forward target (5.0m) far from met yet")
        agent.update_outbound_motion([0.0, 0.0, 0.1], descriptor={"local_map_points_body": pt})  # pure rotation, forward dist stays 0 (updates_pending -> 2)
        self.assertIsNone(agent._pending_anchor, "stall counter must force-finalize even though forward distance never accrued")
        self.assertEqual(len(agent.anchors), 2)


class ReturnFrameBufferTest(unittest.TestCase):
    """2026-07-16: today, a fresh real point cloud is built every single
    return-phase step (local_map_descriptor_from_env in round_trip_eval.py)
    but silently discarded on every non-interval update_relocalization call --
    every ICP attempt matches a single instantaneous live scan, even once the
    anchor side may hold a richer multi-frame submap. This accumulates the
    last return_frame_buffer_window_m of real per-step return frames into one
    merged 'current' descriptor at each actual ICP-attempt step."""

    def _agent(self, relocalizer=None, **kwargs) -> RouteMemoryAgent:
        agent = RouteMemoryAgent(enabled=True, anchor_spacing_m=1.0, relocalizer=relocalizer, **kwargs)
        for _ in range(2):
            agent.update_outbound_motion([1.0, 0.0, 0.0])
        agent.finalize_outbound()
        return agent  # anchors: 0 @ 0m, 1 @ 1m, 2 @ 2m (return-start); current starts at 2
    ...
    def test_default_flag_is_off(self):
        agent = self._agent()
        self.assertFalse(agent.return_frame_buffer_enabled)

    def test_default_off_passes_raw_descriptor_through_unchanged(self):
        received = []
        agent = self._agent(relocalizer=lambda descriptor, anchors: received.append(descriptor) or None)
        desc = {"local_map_points_body": np.array([[5.0, 0.0]])}
        agent.update_return_motion([0.1, 0.0, 0.0], local_descriptor=desc)
        self.assertEqual(len(received), 1)
        self.assertIs(received[0], desc, "must be the exact same object -- unchanged when the flag is off")

    def test_buffer_accumulates_and_prunes_by_distance(self):
        agent = self._agent(
            return_frame_buffer_enabled=True, return_frame_buffer_window_m=1.0,
            relocalization_interval_updates=1000,
        )
        pt = {"local_map_points_body": np.array([[1.0, 0.0]])}
        for _ in range(5):
            agent.update_return_motion([0.5, 0.0, 0.0], local_descriptor=pt)
        # distances after each step: 0.5,1.0,1.5,2.0,2.5; window=1.0 -> only entries >= 2.5-1.0=1.5 survive.
        dists = [d for d, _pose, _desc in agent._return_frame_buffer]
        self.assertTrue(all(d >= 1.5 - 1e-6 for d in dists))
        self.assertIn(round(dists[-1], 5), [2.5])

    def test_merged_descriptor_reaches_relocalizer_with_multiple_frames(self):
        received = []
        agent = self._agent(
            relocalizer=lambda descriptor, anchors: received.append(descriptor) or None,
            relocalization_interval_updates=3,
            return_frame_buffer_enabled=True, return_frame_buffer_window_m=10.0,
        )
        # The first update_return_motion after finalize_outbound() always forces one
        # relocalizer call (_force_next_relocalization=True, independent of the interval)
        # -- reset it here so the interval-modulo gating below is tested cleanly on its
        # own, not conflated with that separate, pre-existing forced-first-attempt behavior.
        agent._force_next_relocalization = False
        pt1 = {"local_map_points_body": np.array([[1.0, 0.0]])}
        agent.update_return_motion([0.3, 0.0, 0.0], local_descriptor=pt1)  # count=1, 1%3!=0 -> skip
        agent.update_return_motion([0.3, 0.0, 0.0], local_descriptor=pt1)  # count=2, 2%3!=0 -> skip
        self.assertEqual(len(received), 0, "non-interval steps must not call the relocalizer at all")
        agent.update_return_motion([0.3, 0.0, 0.0], local_descriptor=pt1)  # count=3, 3%3==0 -> attempt
        self.assertEqual(len(received), 1)
        merged = received[0]["local_map_points_body"]
        self.assertEqual(len(merged), 3, "merged descriptor should contain points from all 3 buffered steps")

    def test_max_frames_cap_bounds_buffer_during_stationary_dwell(self):
        agent = self._agent(
            return_frame_buffer_enabled=True, return_frame_buffer_window_m=100.0,
            return_frame_buffer_max_frames=5, relocalization_interval_updates=1000,
        )
        pt = {"local_map_points_body": np.array([[1.0, 0.0]])}
        for _ in range(20):
            agent.update_return_motion([0.0, 0.0, 0.05], local_descriptor=pt)  # pure rotation, ~0 distance per step
        self.assertLessEqual(len(agent._return_frame_buffer), 5)
