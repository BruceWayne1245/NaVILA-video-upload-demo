...
class TestCameraRotationToBodyYaw(unittest.TestCase):
    ...
    def test_extrinsic_camera_axes_extract_anchor_yaw(self):
        """Real captured extrinsics use a DIFFERENT axis convention than the
        fallback matrix above (camera-native x-right/y-down/z-forward duplicated
        onto "body", not a proper forward/left/up body frame -- see the
        2026-07-26 fix docstring on camera_rotation_to_body_yaw). Re-deriving
        this synthetically by composing a world-frame yaw rotation with the
        real extrinsic (as this test did before 2026-07-26) is easy to get
        subtly wrong -- composing with a standard Z-axis "yaw" rotation
        silently assumes the wrong body convention again, one level up. To
        avoid that trap, this test uses real captured-data fixtures instead
        (rotation matrices + camera_rotation_body extrinsics recorded verbatim
        from `investigations/2026-07-26-camera-yaw-fix-and-residual-confidence-
        gate/`), with ground truth from the same episodes' oracle poses.
        Tolerance is a few tenths of a degree (real RANSAC/Kabsch fit on noisy
        LoFTR matches), not exact-analytic precision.
        """
        front_bc = np.eye(3, dtype=np.float32)
        rear_bc = np.array([[-1, 0, 0], [0, -1, 0], [0, 0, 1]], dtype=np.float32)
        zero_pos = np.zeros(3, dtype=np.float32)

        front_descriptor = {"camera_rotation_body": front_bc, "camera_position_body": zero_pos}
        rear_descriptor = {"camera_rotation_body": rear_bc, "camera_position_body": zero_pos, "view": "rear"}

        cases = [
            # (label, rotation_anchor_to_current_camera, current_descriptor, anchor_descriptor, expected_deg)
            (
                "ep1069_anchor3_front_front",
                np.array([[0.5647972, -0.0650418, -0.8226626],
                          [0.01701826, 0.9975951, -0.06718858],
                          [0.8250542, 0.02394764, 0.56454587]], dtype=np.float32),
                front_descriptor, front_descriptor, 55.4,
            ),
            (
                "ep929_anchor3_anchorRear_currentFront",
                np.array([[0.42413735, -0.03172567, 0.90504193],
                          [-0.02222117, 0.99872065, 0.04542321],
                          [-0.9053252, -0.03937677, 0.42288977]], dtype=np.float32),
                front_descriptor, rear_descriptor, 116.5,
            ),
            (
                "ep288_anchor4_anchorFront_currentRear",
                np.array([[0.9410582, 0.09683877, 0.32408604],
                          [-0.07908832, 0.9945774, -0.06753428],
                          [-0.32886857, 0.03792223, 0.943614]], dtype=np.float32),
                rear_descriptor, front_descriptor, 161.0,
            ),
        ]

        for label, rotation, current_descriptor, anchor_descriptor, expected_deg in cases:
            dtheta = camera_rotation_to_body_yaw(rotation, current_descriptor, anchor_descriptor)
            err_deg = abs(math.degrees(_angle_diff(dtheta, math.radians(expected_deg))))
            self.assertLess(err_deg, 2.0, msg=f"{label}: err={err_deg:.2f} deg")
...
def _scattered_cluster_points() -> np.ndarray:
    """An incoherent random scatter -- no line/edge structure for a rigid
    rotation to exploit, used as an alias_score negative control. (A single
    straight diagonal line was tried first and rejected: rotated ~45 degrees
    it aligns closely with the rectangle outline's own straight edges,
    scoring ~0.90 -- a genuinely useful reminder of exactly how generous
    ICP's overlap_ratio is for simple shapes at this point density, which is
    the whole reason this precompute exists.)"""
    rng = np.random.default_rng(0)
    return rng.uniform(-0.5, 0.5, size=(60, 2)).astype(np.float32)
...
class TestScanContextDescriptor(unittest.TestCase):
    ...
    def test_agreement_mask_matches_region_size_on_hand_built_grid(self):
        """largest_connected_agreement_mask (added 2026-07-04 so
        plot_route_memory_diagnostic_frames.py can render *which* cells a
        Scan Context match considers matched) must report the same cell count
        as the existing size-only largest_connected_agreement_region, and the
        mask's true cells must be exactly the connected 3-cell L-shape, not
        the isolated 4th cell."""
        sc_a = np.zeros((4, 4), dtype=np.float32)
        sc_a[0, 0] = 1.0
        sc_a[0, 1] = 1.0
        sc_a[1, 0] = 1.0
        sc_a[3, 3] = 1.0  # isolated (not 4-connected, even with circular sector axis)
        sc_b = sc_a.copy()

        region_size = largest_connected_agreement_region(sc_a, sc_b, shift=0)
        mask = largest_connected_agreement_mask(sc_a, sc_b, shift=0)

        self.assertEqual(region_size, 3)
        self.assertEqual(int(mask.sum()), region_size)
    ...
    def test_agreement_mask_matches_region_size_on_realistic_descriptor(self):
        sc_a = build_scan_context(_l_shape_points())
        sc_b = build_scan_context(_l_shape_points(offset=(0.05, 0.05)))
        _similarity, shift, region_size = column_shift_search_with_region(sc_a, sc_b)
        mask = largest_connected_agreement_mask(sc_a, sc_b, shift)
        self.assertEqual(int(mask.sum()), region_size)
...
class TestSequentialPairAnchorRelocalization(unittest.TestCase):
    """sequential_pair_anchor_relocalization: matches only the two anchors the
    caller explicitly names (current + next), never searches a candidate
    list -- 2026-07-04, per the user's design that identity is known by
    construction once return has started."""

    def _make_anchor(self, index: int, points: np.ndarray) -> RouteAnchor:
        return RouteAnchor(
            index=index,
            pose_from_start=[0.0, 0.0, 0.0],
            distance_from_start_m=float(index),
    ...
    def test_both_anchors_matchable_returns_two_tagged_candidates(self):
        ...
        result = sequential_pair_anchor_relocalization(
            current_descriptor, current_anchor, next_anchor, return_candidates=True,
        )

        self.assertIsNotNone(result)
        self.assertEqual(len(result), 2)
        returned_indices = sorted(c.anchor_index for c in result)
        self.assertEqual(returned_indices, [4, 5])
        for candidate in result:
            self.assertEqual(candidate.backend, "sequential_pair")

    def test_degenerate_next_anchor_is_not_skipped_by_p2(self):
        """P2 is now diagnostic-only for sequential_pair: a corridor-like next
        anchor is still returned so current/next closure and promotion logic can
        decide how to use it."""
        current_anchor = self._make_anchor(5, _rectangle_outline_points())
        next_anchor = self._make_anchor(4, _corridor_points())
        current_descriptor = {"local_map_points_body": _rectangle_outline_points()}
    ...
    def test_missing_next_anchor_is_handled_gracefully(self):
        """RouteMemoryAgent.sequential_target_anchor_pair() returns next=None
        at the very last (index-0) anchor -- must not crash."""
        current_anchor = self._make_anchor(0, _rectangle_outline_points())
        current_descriptor = {"local_map_points_body": _rectangle_outline_points()}

        result = sequential_pair_anchor_relocalization(
            current_descriptor, current_anchor, None, return_candidates=True,
        )

        self.assertIsNotNone(result)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].anchor_index, 0)

    def test_both_anchors_missing_returns_none(self):
        current_descriptor = {"local_map_points_body": _rectangle_outline_points()}
        result = sequential_pair_anchor_relocalization(
            current_descriptor, None, None, return_candidates=True,
        )
        self.assertIsNone(result)

    def test_single_candidate_mode_returns_best_scoring_one(self):
        current_anchor = self._make_anchor(5, _rectangle_outline_points())
        next_anchor = self._make_anchor(4, _rectangle_outline_points())
        current_descriptor = {"local_map_points_body": _rectangle_outline_points()}

        result = sequential_pair_anchor_relocalization(
            current_descriptor, current_anchor, next_anchor, return_candidates=False,
        )
...
def _cross_shape_points() -> np.ndarray:
    ...
    identical, a genuine yaw ambiguity a correct diagnostic must flag."""
    xs = np.linspace(-3.0, 3.0, 40)
    horizontal = np.stack([xs, np.zeros_like(xs)], axis=1)
    ys = np.linspace(-3.0, 3.0, 40)
    vertical = np.stack([np.zeros_like(ys), ys], axis=1)
    return np.concatenate([horizontal, vertical], axis=0).astype(np.float32)


def _asymmetric_l_shape_points() -> np.ndarray:
    """A long wall plus one short, clearly unequal-length perpendicular arm --
    has no rotational symmetry, so ICP should converge to one sharp, isolated
    peak in yaw with no meaningfully-competitive alternative."""
    xs = np.linspace(-3.0, 3.0, 60)
    main_wall = np.stack([xs, np.zeros_like(xs)], axis=1)
    ys = np.linspace(0.3, 1.0, 15)
    short_wall = np.stack([np.full_like(ys, 3.0), ys], axis=1)
    return np.concatenate([main_wall, short_wall], axis=0).astype(np.float32)


class TestSequentialPairYawDiagnostics(unittest.TestCase):
    """2026-07-10: yaw-specific diagnostics added to `sequential_pair`'s
    covisibility records, diagnostic-only (no gating/rejection behavior
    changes) -- see investigations/2026-07-10-.../FINDINGS.md and
    investigations/2026-07-09-.../route_memory_literature_survey.md §2.1/2.2.
    Targets the "clean_full_pose, confidently wrong rotation" failure mode
    documented in investigations/2026-07-09-.../FINDINGS.md §3, which the
    pre-existing `match_class`/`icp_near_tie_basin_count`/
    `corridor_degeneracy_ratio` diagnostics were found not to catch."""
    ...
    def test_yaw_curve_flags_rotationally_symmetric_shape_as_a_wide_plateau(self):
        record = self._pose_candidate_record(_cross_shape_points())
        yaw_curve = record["yaw_curve"]
        self.assertTrue(yaw_curve["available"])
        self.assertGreaterEqual(yaw_curve["yaw_peak_width_deg"], 90.0)
        self.assertIsNotNone(yaw_curve["yaw_top1_next_distinct_gap_deg"])
        self.assertGreaterEqual(yaw_curve["yaw_top1_next_distinct_score_ratio"], 0.95)
        json.dumps(yaw_curve)

    def test_yaw_curve_flags_asymmetric_shape_as_a_narrow_single_peak(self):
        record = self._pose_candidate_record(_asymmetric_l_shape_points())
        yaw_curve = record["yaw_curve"]
        self.assertTrue(yaw_curve["available"])
        self.assertLess(yaw_curve["yaw_peak_width_deg"], 5.0)
        json.dumps(yaw_curve)

    def test_localizability_yaw_observability_full_for_well_conditioned_shape(self):
        record = self._pose_candidate_record(_rectangle_outline_points())
        localizability = record["localizability"]
        self.assertEqual(localizability.get("quality"), "full")
        self.assertEqual(localizability.get("yaw_observability"), "full")
        self.assertIsNotNone(localizability.get("yaw_normalized_marginal_information"))
        json.dumps(localizability)

    def test_localizability_yaw_observability_is_unknown_not_falsely_full_when_translation_is_singular(self):
        """A pure corridor's translation-along-corridor is unconstrained, so
        the Schur complement used to marginalize yaw over translation can't
        be computed (the translation block isn't invertible) -- this must
        report "unknown", never silently claim "full"."""
        record = self._pose_candidate_record(_corridor_points())
        localizability = record["localizability"]
        self.assertEqual(localizability.get("quality"), "degenerate")
        self.assertIsNone(localizability.get("yaw_marginal_information"))
    ...
    using the same ICP machinery as live matching -- confirmed against real
    hard-11 data before being wired into anything (see
    RouteMemoryAgent.compute_anchor_alias_scores' docstring)."""
    ...
    def test_immediate_neighbor_is_excluded_regardless_of_shape(self):
        """Two adjacent anchors (offset 1) sharing the exact same shape must
        not count towards either one's alias_score -- consecutive anchors are
        *expected* to overlap since they're only ~anchor_spacing_m apart;
        that's not evidence of route-level self-similarity."""
        anchors = [
            self._make_anchor(0, _rectangle_outline_points()),
            self._make_anchor(1, _rectangle_outline_points()),
        ]
        scores = compute_anchor_alias_scores(anchors, min_neighbor_offset=2, max_neighbor_offset=5)
        # Neither anchor has any candidate inside [2, 5] offset -- both entries
        # exist (the anchor was matchable) but score 0.0 (no pair was ever compared).
        self.assertEqual(scores[0], 0.0)
        self.assertEqual(scores[1], 0.0)

    def test_duplicated_shape_at_non_adjacent_offset_scores_high(self):
        """anchor0 and anchor2 (offset 2, i.e. within the default window and
        past the excluded immediate neighbor) share the identical rectangle
        shape -- both must be flagged with a high alias_score."""
        anchors = [
            self._make_anchor(0, _rectangle_outline_points()),
            self._make_anchor(1, _corridor_points()),
            self._make_anchor(2, _rectangle_outline_points()),
        ]
        scores = compute_anchor_alias_scores(anchors, min_neighbor_offset=2, max_neighbor_offset=5)
        self.assertGreater(scores[0], 0.6)
        self.assertGreater(scores[2], 0.6)

    def test_distinct_shapes_at_non_adjacent_offset_score_low(self):
        """anchor0 (rectangle) and anchor3 (incoherent scatter, offset 3) are
        genuinely different local structure -- anchor3 must land below the
        default 0.6 threshold, and anchor0's score must stay dominated by its
        real duplicate (anchor2), not raised by the dissimilar anchor3."""
        anchors = [
            self._make_anchor(0, _rectangle_outline_points()),
            self._make_anchor(1, _corridor_points()),
            self._make_anchor(2, _rectangle_outline_points()),
            self._make_anchor(3, _scattered_cluster_points()),
        ]
        scores = compute_anchor_alias_scores(anchors, min_neighbor_offset=2, max_neighbor_offset=5)
        self.assertGreater(scores[0], 0.6)
        self.assertLess(scores[3], 0.6)

    def test_anchor_with_no_candidates_in_window_scores_zero(self):
        """Route too short for this anchor to have anything between
        min/max_neighbor_offset away -- must not crash, just score 0.0."""
        anchors = [
            self._make_anchor(0, _rectangle_outline_points()),
            self._make_anchor(1, _rectangle_outline_points()),
        ]
        scores = compute_anchor_alias_scores(anchors, min_neighbor_offset=4, max_neighbor_offset=5)
        self.assertEqual(scores[0], 0.0)
        self.assertEqual(scores[1], 0.0)
