...
## Live round-trip A/B launched (same day)

Per the user's explicit direction: a real 50-episode A/B is now running, using the SAME "outbound most likely to succeed" 50-episode set Route 2 (Codex, model-based ICP-trust track) selected for its 2026-07-25 active-50ep run (`episode_idx` column of `/home/teambruce/navila-reliability-v1_1-policy-v2-active50-20260725/experiments/2026-07-25-policy-v2-active-50ep/episodes.tsv`), so both routes' results land on the same cohort. Two full-50-episode arms on the live Route1 code path (`NaVILA-Bench/scripts/round_trip_eval.py`, not any isolated candidate): `diagnostic` (baseline, current best CANONICAL config, no behavior change) vs `downgrade` (same config + `vision_disagreement_mode=downgrade`, penalty 0.3). Both arms pay the same LoFTR/GPU overhead (`--sequential_pair_loftr_rear_yaw_check` on in both) so a result difference can't be attributed to the extra compute itself. Per a later request, run order was swapped to downgrade-first so that arm's data is available sooner.

Scripts: `code/live_batch_scripts/run_vision_disagreement_ab_50ep_driver_20260726.sh` (per-episode VLM-server/timeout/summary machinery, adapted unchanged from `scripts/run_oracle_anchor_100ep_batch_20260720.sh`) + `run_vision_disagreement_ab_50ep_master_20260726.sh` (waits for the concurrently-running Route2 5ep canary to finish first, since both share the one GPU, then runs both arms back to back). Launched via `systemd-run --user` (linger confirmed enabled, cgroup under `user@1006.service`) so it survives session/SSH disconnect.

**Caught and fixed a real bug in the wait-for-Route2 logic before it could cause a resource conflict**: the first version keyed off a specific PID's own cmdline for a substring that actually only appeared in that PID's *child* process's arguments, so it wrongly concluded the Route2 job had already finished on its very first check. A secondary safety check (polling actual GPU-consuming process names) kept it from proceeding regardless, and this was caught and corrected before any real conflict occurred -- verified via direct process inspection before relaunching. Fixed version polls actual process-name patterns (the Route2 round_trip_eval.py path, its VLM server port, and its batch wrapper scripts) rather than a specific PID.

Results pending -- check `NaVILA-Bench/batch_logs/vision_disagreement_ab_50ep_20260726_{downgrade,diagnostic}/summary.tsv` or a follow-up investigation entry.
