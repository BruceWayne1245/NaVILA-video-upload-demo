...
## 11. Live smoke test (2026-08-03, ~21:00-22:06 BST) — the blocked "next step" above, executed

GPU freed up (Route2 finished its prior work); ran a 4-episode live smoke test (`run_line2_phase01_smoke_4ep_20260803.sh`, adapted from `run_promotion_shadow_bugfix_smoke_1ep_20260730.sh`'s COMMON_EXTRA with the two model-shadow lines removed and today's three new flags added), one episode per new mechanism plus a regression check, all three flags combined in one arm (not single-variable — each is independently gated to a narrow, non-overlapping trigger condition, so this was treated as a crash/regression smoke test, not full statistical validation):

- **ep205 (Phase 1.2 arbiter stop_veto target) — round_trip_success=True.** `stop_veto_anchor_far` fired live 3× (steps 2836, 3361, 3436), exactly matching the offline-replay prediction in §9. Forced stop at anchor_remaining=0.0, conf=0.801. distance_to_start=2.25m.
- **ep95 (Phase 1.1 ambiguity gate target) — infra failure, not a code regression.** VLM server crashed at checkpoint load (`ValueError: too many values to unpack (expected 0)` inside `transformers`' `_initialize_weights`/`named_children`, `vlm_server.py` → `NaVILA/llava/model/builder.py`), 19s after launch. Matches this project's previously-documented VLM-server-startup flakiness (see 2026-07-08's "ep134/187/367 swings... unrelated to the fix" entry). The ambiguity gate therefore still has zero live confirmation, offline replay only.
- **ep344 / ep368 (Phase 0.2 eviction target / regression check) — both manually killed on user request after showing no escape.** ep344: ~2450 steps (400+ relocalization attempts) with the reported anchor pinned at the same route-remaining value throughout, `stuck_recovery` cycling wedge/reverse-turn states without resolving, no `evict` firing. ep368: return-phase `distance_to_start` went 6.013m (start) → 7.942m (worse) → 6.930m over 2500 steps — net negative progress — same pinned-anchor/no-eviction pattern. Neither showed net improvement before being killed.

## 12. Root cause of "no eviction observed": a real diagnostic gap, not (only) a bad reading

Reading `route_memory_agent.py:1658-2015` (`_current_persistently_unreliable`, the eviction block in `_select_sequential_pair_relocalization`) after the smoke test found that the `anchor_idx`/`anchor_remaining` fields being watched in `[stop_gate]` log lines reflect the **NEXT** role (per §7 Phase 0.1's own finding: `--sequential_pair_report_next_anchor` biases `RelativeStartProgress`/`GateDecision`'s reported anchor toward "next", not "current") — **not** the internal `_target_anchor_index` (current) that eviction actually operates on. There was **no live print of current_idx / current_unreliable / the eviction streak counter anywhere in the codebase** — a genuine diagnostic gap discovered by this session, not a one-off misreading of the smoke-test logs.

**Fix (pure diagnostic, zero behavior change):** added `current_target_anchor_index` / `current_anchor_unreliable` / `current_evict_streak` to `RelativeStartProgress` (`route_memory_agent.py`, populated in `_anchor_progress_from_estimate`) and `GateDecision` (`stop_gate.py`), threaded through every `GateDecision(...)` construction site via the existing `_diag` dict pattern (all 10 sites use `**_diag`, confirmed by grep), printed in `round_trip_eval.py`'s `[stop_gate]` line as `current_idx=`/`current_unreliable=`/`evict_streak=`. Full suite re-run: 362 tests, same 1 pre-existing unrelated `cv2`-import failure, 0 new regressions.

**Confirmed live** on a 1-episode check (ep4, killed once confirmed): `current_idx=11` while `anchor_idx=10` (next role) at the same step (`step=1576`) — directly proves these are genuinely different values, validating the fix's purpose. Both `current_unreliable=False` and `evict_streak=0` at that early-return-phase reading, as expected (conf=1.000).

**Open design concern flagged but NOT changed (needs a decision, not a unilateral fix):** eviction's streak counter resets to 0 the moment a single "reliable-looking" reading occurs (`route_memory_agent.py:1996-1998`), while `current_unreliable` itself is a bad-fraction over retained history (`bad/len(history) > 0.5`) that can flicker across that threshold attempt-to-attempt. Against the intermittent-confidence noise this project already knows is common (the whole "confidently wrong" story — see §1, §4), this streak design may structurally under-fire: a reading that ticks back above the reliability floor even once resets 29 attempts of progress. Worth checking directly in the 30ep results (below) before deciding whether to redesign as a windowed vote (mirroring `bounded_evidence`'s own `promotion_window`/`promotion_min_votes` pattern) instead of a hard consecutive streak.

## 13. 30ep validation batch launched (2026-08-03T22:19 BST)

`run_line2_phase01_30ep_20260803.sh`, adapted from `run_promotion_quarantine_veto_30ep_20260801.sh`'s COMMON_EXTRA with the four model lines (`sequential_pair_promotion_model_shadow`/`_quarantine_threshold`/`_active_promote`/`_quarantine_veto`) removed and the three new Phase 0.2/1.1/1.2 flags (plus the §12 diagnostic fields, which need no flag — always populated) added in their place. **Same 30 episode_idx values and PORT_BASE=63000 as `promotion_shadow_reliable30v3_20260731`** (4, 5, 89, 134, 187, 205, 268, 295, 354, 367, 368, 381, 408, 409, 420, 430, 488, 498, 500, 647, 671, 678, 680, 688, 708, 974, 994, 1038, 1040, 1058) — deliberately, so this run is directly comparable to that closest pre-model pure-heuristic baseline without needing a separate control arm. Uses the `reliable30v3` driver (10min log-growth stall watchdog + 40min per-episode timeout cap), not the plain smoke-test driver — stuck episodes are meant to run to their natural stall/timeout cutoff and land in `summary.tsv` as real failures here, not be manually killed (standard protocol for every prior 30ep batch in this project).

Running as a `systemd-run --user` transient service (`navila-line2-phase01-30ep-20260803.service`, `KillMode=control-group`, `TimeoutStopUSec=90s`) under `loginctl enable-linger` — confirmed its cgroup is `user@1006.service/app.slice/...` (not the launching session's scope), so it survives session disconnect. First episode (ep4) hit the same VLM-startup-flakiness infra failure as ep95 above (exit_code=98); driver correctly moved on to ep5 automatically, confirming the batch's own error-handling doesn't stall on a single infra hiccup.

**Route2 queued behind it**, on explicit user instruction: `wait_for_line2_30ep_then_run_route2_anchorv2_semanticfix30_20260803.sh` (own `systemd-run --user` service, own dedicated `LOCK_FILE` to avoid colliding with the Route2 driver's reused default lock name), which waits for this 30ep batch's driver process to exit, then launches Route2's `anchor_v2_full_active_semanticfix30_20260803` batch (30 episodes, `navila-route2-v11-core-20260801/launch/run_manifest_batch_driver.sh`, PORT_BASE=54321 — confirmed non-overlapping with Route1's 63000+ range). Completion detection deliberately gates on the **driver process itself** (persists across every normal inter-episode gap, VLM restart, and cleanup sleep — only exits at genuine completion or a real crash), not on "any process running," specifically so a routine gap can never be misread as done; 3 consecutive confirm-polls (90s) before treating the driver as gone. Distinguishes clean completion (`batch.log` contains the driver's own `Batch finished at` line — reliable per §14 below) from an abnormal exit, and on the latter, actively force-kills any leftover eval/VLM processes (by `result_suffix` and by the exact port list for these 30 episodes) before checking GPU is free and proceeding — reinforces (rather than replaces) the driver's own existing per-episode `sweep_kill_stragglers` cleanup and systemd's own `KillMode=control-group` cgroup-wide cleanup (confirmed via `systemctl show`) that already fire on a driver crash.

## 14. Why `Batch finished at` is a reliable clean-completion signal

The `reliable30v3` driver uses `set -u` only, **not** `set -e` (checked directly). Every `run_episode` failure path (VLM-start failure, `cd` failure, timeout/stall-kill, or normal exit) falls through to the same tail logic (kill, sweep, wait-for-port-closed, write summary row) and returns normally — no individual episode's failure can abort `main()`'s sequential list of 30 `run_episode` calls early. So the trailing `echo "Batch finished at $(date -Is)"` reliably prints unless something kills the whole driver process itself (crash, OOM-kill, `kill -9` on the driver's own PID) — confirming its absence is a genuine "did not end correctly" signal for the §13 queue worker to act on, not a false positive from a merely-failed episode.

## 15. Current status / next steps (supersedes §10)

- Phase 0 and Phase 1 are implemented, tested, and now partially live-validated: Phase 1.2 (arbiter stop_veto) confirmed working live (ep205); Phase 1.1 (ambiguity gate) still has zero live confirmation (ep95 infra failure, needs a retry); Phase 0.2 (eviction) has not been observed to fire live yet in either of the two episodes that ran long enough to plausibly trigger it (ep344, ep368) — whether that's the streak-reset fragility in §12, a genuine "eviction rarely needed" outcome, or something else is still open.
- **30ep batch (`line2_phase01_30ep_20260803`) is running now**, directly comparable to `promotion_shadow_reliable30v3_20260731`. Once finished: compare round-trip rate to that baseline; check whether `current_idx`/`current_unreliable`/`evict_streak` (§12) shows eviction firing anywhere across the 30 episodes; if it never fires, revisit the streak-vs-window design question in §12.
- Route2's `anchor_v2_full_active_semanticfix30_20260803` (30 episodes) is queued to launch automatically once the 30ep batch above finishes, per explicit user instruction.
- Phase 2 (mechanism G's graduated near-miss band) is still not started.
- Code changes today live in the dev sandbox (`NaVILA-Bench/scripts/route_memory_agent.py`, `stop_gate.py`, `round_trip_eval.py`) — `code/stop_gate.py` in this folder updated to match (now carries the §12 diagnostic fields on top of the Phase 1.2 stop_veto logic already there). `route_memory_agent.py`/`round_trip_eval.py` remain too large to include in full per §10's original note; the §12 diagnostic addition is described there with enough context (field names, function names, exact line ranges) to relocate and reapply directly.
