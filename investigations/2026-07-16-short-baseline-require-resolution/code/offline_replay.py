...
def make_agent(promotion_mode, anchors, interval=5, promotion_window=5, promotion_min_votes=3,
                ...
                short_baseline_disambiguation=False, short_baseline_min_travel_m=0.3,
                short_baseline_max_rotation_disagreement_deg=20.0,
                short_baseline_require_resolution=False, short_baseline_stall_attempts=60):
    agent = RouteMemoryAgent(
        ...
        sequential_pair_short_baseline_disambiguation=short_baseline_disambiguation,
        sequential_pair_short_baseline_min_travel_m=short_baseline_min_travel_m,
        sequential_pair_short_baseline_max_rotation_disagreement_deg=short_baseline_max_rotation_disagreement_deg,
        sequential_pair_short_baseline_require_resolution=short_baseline_require_resolution,
        sequential_pair_short_baseline_stall_attempts=short_baseline_stall_attempts,
    )
...
def pose_delta_body(previous_pose, current_pose):
    """2026-07-16 harness fix: body-frame motion delta between two consecutive
    captured `robot_world_pose` entries, transcribed exactly from
    round_trip_eval.py's own pose_delta_body (that module can't be imported
    here directly -- it pulls in Isaac Sim at module scope, defeating the
    point of an Isaac-free offline harness). Feeding this through
    RouteMemoryAgent.update_return_motion() every captured step (not just
    sampled attempt positions) is what was missing before -- see the
    docstring on replay_episode() below for why that mattered."""
    prev_yaw = pose_yaw(previous_pose)
    cur_yaw = pose_yaw(current_pose)
    dx_world = current_pose[0] - previous_pose[0]
    dy_world = current_pose[1] - previous_pose[1]
    cos_yaw, sin_yaw = math.cos(prev_yaw), math.sin(prev_yaw)
    return [
        cos_yaw * dx_world + sin_yaw * dy_world,
        -sin_yaw * dx_world + cos_yaw * dy_world,
        wrap_angle(cur_yaw - prev_yaw),
    ]
...
def replay_episode(batch_suffix, ep, promotion_mode, interval=5, icp_objective="point_to_point",
                    ...
                    short_baseline_max_rotation_disagreement_deg=20.0,
                    short_baseline_require_resolution=False, short_baseline_stall_attempts=60):
    """2026-07-16 harness fix (investigations/2026-07-12-.../PROGRESS.md flagged
    this as a known, deferred bug): this used to call
    agent.update_relocalization(...) directly at only the sampled attempt
    positions, never agent.update_return_motion(...) -- so
    _return_pose_from_return_start (and hence current_absolute_pose_from_start())
    never advanced past its [0,0,0] constructor default for the whole replay,
    regardless of how much the real captured robot actually moved. Any
    mechanism that depends on "how far has the robot moved since X"
    (short-baseline disambiguation's whole design) could therefore never
    resolve under the old version of this harness -- travel_dist_m was always
    0.0. Fixed by walking every captured step (not just sampled attempts),
    reconstructing each step's body-frame motion delta from two consecutive
    real robot_world_pose entries (pose_delta_body, above), and driving that
    delta through update_return_motion every step -- real ICP candidates are
    only computed at the sampled attempt positions; every other step passes
    relocalization=None (a no-op for the relocalization decision itself, since
    this harness's agent has no agent.relocalizer configured -- only the pose
    ...
    agent = make_agent(promotion_mode, anchors, interval=interval,
                        ...
                        short_baseline_disambiguation=short_baseline_disambiguation,
                        short_baseline_min_travel_m=short_baseline_min_travel_m,
                        short_baseline_max_rotation_disagreement_deg=short_baseline_max_rotation_disagreement_deg,
                        short_baseline_require_resolution=short_baseline_require_resolution,
                        short_baseline_stall_attempts=short_baseline_stall_attempts)

    attempt_positions = {idx: attempt for attempt, idx in attempt_schedule(len(steps), interval)}

    results = []
    prev_world_pose = steps[0]["robot_world_pose"] if steps else None
    for idx, step in enumerate(steps):
        if idx > 0:
            delta = pose_delta_body(prev_world_pose, step["robot_world_pose"])
        else:
            delta = [0.0, 0.0, 0.0]
        prev_world_pose = step["robot_world_pose"]

        attempt = attempt_positions.get(idx)
        if attempt is None:
            agent.update_return_motion(delta, local_descriptor=None, relocalization=None)
            continue
        if max_attempts is not None and attempt > max_attempts:
            break
        if attempt % 25 == 0:
            print(f"    [progress] ep{ep} mode={promotion_mode} attempt {attempt}", flush=True)
        ...
        if candidates is None:
            candidates = []
        # Capture whether THIS attempt specifically accepted (not just whether
        # _latest_relocalization is non-None, which would stay stale-truthy
        # from an earlier attempt even when this one is rejected).
        n_events_before = len(agent.relocalization_events)
        agent.update_return_motion(delta, local_descriptor=None, relocalization=candidates)
        accepted = None
        if len(agent.relocalization_events) > n_events_before and agent.relocalization_events[-1].get("accepted"):
            accepted = agent._latest_relocalization
        ...
        results.append(dict(
            ...
            anchor_heading_reliable=(accepted.anchor_heading_reliable if accepted is not None else None),
        ))
    return results
