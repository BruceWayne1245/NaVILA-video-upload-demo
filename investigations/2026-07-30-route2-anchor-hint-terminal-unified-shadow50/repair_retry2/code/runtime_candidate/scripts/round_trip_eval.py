...
parser.add_argument(
    ...
    default="",
    help=(
        "Path to the frozen Policy V2 consumer-guard JSON. The policy preserves "
        "raw candidates and reversible relocalization updates, and evaluates "
        "only promotion, route-hint, hint-action, and stop-gate consumers."
    ),
)
parser.add_argument(
    ...
    default="off",
    help=(
        "Policy V2 consumer mode. Shadow logs counterfactual decisions without "
        "controller effects. Active requires an independently approved active "
        "policy artifact; a shadow artifact is structurally rejected."
    ),
)
parser.add_argument(
...
def main():
    ...
    if (
        args_cli.reliability_v11_consumer_mode != "off"
        and not args_cli.reliability_v11_online_shadow
    ):
        raise RuntimeError(
            "Reliability V1.1 Policy V2 requires "
            "--reliability_v11_online_shadow"
        )
    if (
        args_cli.reliability_v11_integrated_promotion_mode != "off"
        and args_cli.reliability_v11_consumer_mode == "off"
    ):
        raise RuntimeError(
            "Integrated promotion shadow requires Reliability V1.1 "
            "Policy V2 consumer mode"
        )
    if (
        ...
        raise RuntimeError(
            "Derived evidence evaluation requires Policy V2 consumer mode"
        )
    if (
        args_cli.reliability_v11_derived_evidence_mode == "active"
        and args_cli.reliability_v11_consumer_mode != "active"
    ):
        raise RuntimeError(
            "Active derived evidence requires active Policy V2 consumer"
        )
    ...
    if args_cli.reliability_v11_online_shadow:
        ...
        consumer_policy_path = os.path.abspath(
            args_cli.reliability_v11_consumer_policy_v2
        )
        ...
        if (
            args_cli.reliability_v11_consumer_mode != "off"
            and (
                not args_cli.reliability_v11_consumer_policy_v2
                or not os.path.isfile(consumer_policy_path)
            )
        ):
            raise RuntimeError(
                "Reliability V1.1 Policy V2 is missing or not a file"
            )
        if (
        ...
        if args_cli.reliability_v11_consumer_mode != "off":
            v11_consumer_guard = V11ConsumerGuardV2.load(
                consumer_policy_path,
                mode=args_cli.reliability_v11_consumer_mode,
            )
            ...
            v11_consumer_log_handle = open(
                os.path.join(result_dir, "reliability_v11_consumer_v2.jsonl"),
                "a",
                encoding="utf-8",
                buffering=1,
            )
            v11_consumer_log_handle.write(
                json.dumps({
                    "event": "v11_consumer_v2_session_start",
                    "episode_key": episode_key,
                    "physical_episode_id": int(args_cli.episode_idx),
        ...
        print(
            ...
            f"consumer_v2={args_cli.reliability_v11_consumer_mode} "
            ...
            if num_steps == target_steps:
                if phase in ("outbound", "return"):
                    ...
                    if (
                        ...
                        # Policy V2 request counts aligned 1:1 with actual
                        ...
                        if proposed_hint_event is None:
                            ...
                            if v11_route_hint_allowed:
                                ...
                                phase_events.append({
                                    "step": int(num_steps),
                                    "phase": phase,
                                    "event": "v11_consumer_v2_route_hint_suppressed",
                                    "target_anchor_index": (
    ...
    if v11_consumer_guard is not None:
        ...
        v11_consumer_decisions = [
            ...
            if event.get("event") == "v11_consumer_v2_decision"
        ]
        ...
        if v11_consumer_log_handle is not None:
            v11_consumer_log_handle.write(
                json.dumps({
                    "event": "v11_consumer_v2_session_end",
                    "episode_key": episode_key,
                    **v11_consumer_summary,
                }, sort_keys=True, separators=(",", ":")) + "\n"
            )
    ...
    measurements["round_trip"] = {
        ...
        "reliability_v11_consumer_policy_v2": v11_consumer_summary,
    ...
    completion_manifest = {
        ...
        "v11_consumer_v2_log": (
            {
                "path": "reliability_v11_consumer_v2.jsonl",
                "sha256": sha256_file(
                    os.path.join(
                        result_dir,
