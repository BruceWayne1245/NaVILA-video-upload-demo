...
ANCHOR_PACKAGE="${ROOT}/anchor_transition_runtime/__init__.py"
...
EPISODE_VALIDATOR="${ROOT}/validation/validate_episode_capture.py"
...
RUN_ROOT="${ROOT}/runs/unified_shadow50_retry2_20260730"
CANARY_TAG="unified_shadow50_ep670_replication_retry2_20260730"
BATCH_TAG="unified_shadow50_fresh49_retry2_20260730"
PORT_BASE=59000
CANARY_EPISODE=670
...
EXPECTED_DRIVER_SHA="8a11cd8385abb2aa53e34798e7f18114ae6fcd87c8926c7dffd54b31dc73fb54"
EXPECTED_EVAL_SHA="5e6ba9413c59087403078ad578bc9e75a6eaec144ad3f2c3100e471be803a70c"
...
EXPECTED_ANCHOR_PACKAGE_SHA="2d29bbcd68605f1e04b211295ba99d2147a017b69ce328bc4c133d4818f1cd77"
...
EXPECTED_EPISODE_VALIDATOR_SHA="aaebc0f1f86bc468750d34e4a20ee90f309b424d8cbed78d61be76f0f41976af"
...
preflight() {
  ...
  require_sha "${ANCHOR_PACKAGE}" "${EXPECTED_ANCHOR_PACKAGE_SHA}"
  ...
  require_sha "${EPISODE_VALIDATOR}" "${EXPECTED_EPISODE_VALIDATOR_SHA}"
...
PY
  "${ISAAC_PYTHON}" -m py_compile \
    "${SCORER}" "${EVAL_SCRIPT}" "${STOP_GATE}" "${EPISODE_VALIDATOR}" \
    "${ANCHOR_PACKAGE}" "${ANCHOR_ONLINE}" "${ANCHOR_GUARD}"
  "${ISAAC_PYTHON}" - "${ROOT}" <<'PY'
import sys
import types

foreign = types.ModuleType("reliability")
foreign.__path__ = ["/preloaded/v11/reliability"]
sys.modules["reliability"] = foreign
sys.path.insert(0, sys.argv[1])
from anchor_transition_runtime import OnlineAnchorTransitionV1
assert OnlineAnchorTransitionV1.__module__ == "anchor_transition_runtime.online"
PY
  bash -n "${BASE_DRIVER}"
...
run_driver() {
  ...
  MANIFEST="${MANIFEST}" \
  FAIL_FAST="${fail_fast}" \
  REQUIRE_ALL_EPISODES="${require_all}" \
...
write_provenance() {
  cp "${MANIFEST}" "${RUN_ROOT}/unified50.tsv"
  {
    ...
    echo "github_authoritative_commit=b80020499ab2a33eb3cc8e6ae6693ec8d7fb3809"
    ...
    echo "anchor_package_sha256=${EXPECTED_ANCHOR_PACKAGE_SHA}"
    ...
    echo "episode_validator_sha256=${EXPECTED_EPISODE_VALIDATOR_SHA}"
...
run_driver "${CANARY_TAG}" "${CANARY_EPISODE}" 1 1
"${ISAAC_PYTHON}" - \
...
batch_driver_status=0
if run_driver "${BATCH_TAG}" "${BATCH_EPISODES}" 0 1; then
  log "fresh49 driver integrity PASS"
else
  batch_driver_status=$?
  log "fresh49 completed with one or more infrastructure failures status=${batch_driver_status}"
fi
...
if (( batch_driver_status != 0 )); then
  touch "${RUN_ROOT}/NEEDS_INFRA_RETRY"
  exit "${batch_driver_status}"
fi
