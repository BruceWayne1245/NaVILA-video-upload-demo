...
class RouteMemoryAgent:
    ...
    def _select_sequential_pair_relocalization(
        ...
        current_quality = self._relocalization_quality(current_est) if current_est is not None else 0.0
        next_quality = self._relocalization_quality(next_est) if next_est is not None else 0.0
        if next_est is not None:
            self._record_promotion_sample(next_est, next_quality)

        promote = False
        if next_est is not None:
            close_enough = next_est.distance_to_anchor_m <= self.promotion_close_radius_m
            trend_ok = self._promotion_trend_improving(next_idx)
            quality_ok = (
                current_est is None
                or next_quality >= self.promotion_score_ratio * max(current_quality, 1e-9)
            )
            candidate_promote = bool(quality_ok and (close_enough or trend_ok or current_est is None))
            if current_est is None or self.sequential_pair_promotion_mode != "bounded_evidence":
    ...
    def _next_candidate_index(self, current_index: int) -> int:
        ...
        if self.sequential_pair_quarantine_enabled:
    ...
    def _sequential_pair_closure_precheck(
        ...
        heading_disagreement = abs(wrap_angle(dtheta - a.anchor_dtheta_rad))
        if (
