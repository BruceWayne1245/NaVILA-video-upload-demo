...
PORT="${PORT:-54321}"
RUN_TAG="${RUN_TAG:-pure_oracle_hint_action_stopgate_highsuccess100ep_20260813}"
START_AT="${START_AT:-0}"
...
main() {
  ...
  echo "Pure oracle_hint_action + stop_gate, high-outbound-success 100ep variant (2026-08-13): step 3 of the数据补全 ablation chain (oracle_hint -> oracle_hint_action -> oracle_hint_action+stop_gate). Base is pure_oracle_hint_action_highsuccess100ep_20260812 (52/86=60.5% return-rate) UNCHANGED -- identical eval flags (--route_memory --route_hint_mode=compact --route_hint_source=oracle --route_relocalization_backend=none --topdown_route_map --hint_action_arbiter) and the identical episode list (same high-outbound-success 100-episode manifest, unchanged from pure_oracle_hint_highsuccess100ep_20260811 / weighted historical outbound success 884/932=94.85%) -- the ONLY addition is --stop_gate --stop_gate_r_in=3.0 --stop_gate_r_out=3.0 --stop_gate_confirm_steps=3 --stop_gate_min_confidence=0.5, matching the four sub-params used in every historical batch that included stop_gate (e.g. stop_gate_r3_hint_arbiter_hard11_20260630). --oracle_align_return_yaw_to_anchor_segment remains deliberately excluded, same as steps 1 and 2. This is NOT the 7/20 canonical-100 set -- not episode-for-episode comparable to pure_navila_baseline_100ep_20260810 / pure_oracle_hint_100ep_20260811 / pure_oracle_hint_action_100ep_20260812. Directly comparable to pure_oracle_hint_action_highsuccess100ep_20260812 (same manifest, same base flags, +stop_gate only)." | tee -a "${LOG_DIR}/batch.log"
