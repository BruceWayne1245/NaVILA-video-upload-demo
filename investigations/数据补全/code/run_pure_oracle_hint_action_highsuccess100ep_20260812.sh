...
PORT="${PORT:-54321}"
RUN_TAG="${RUN_TAG:-pure_oracle_hint_action_highsuccess100ep_20260812}"
START_AT="${START_AT:-0}"
...
main() {
  ...
  echo "Pure oracle_hint_action, high-outbound-success 100ep variant (2026-08-12): takes pure_oracle_hint_action_100ep_20260812's eval flags (--route_memory --route_hint_mode=compact --route_hint_source=oracle --route_relocalization_backend=none --topdown_route_map --hint_action_arbiter -- same exclusions: no --stop_gate, no --oracle_align_return_yaw_to_anchor_segment) and swaps in the high-outbound-success 100-episode manifest from pure_oracle_hint_highsuccess100ep_20260811 (episode list unchanged from that script; only the flags differ). This run replaces an in-progress canonical-100 pure_oracle_hint_action_100ep_20260812 run that was manually stopped at 98/100 by explicit user instruction (2026-08-12) specifically to switch to this higher-outbound-success manifest -- see that RUN_TAG's own batch_logs for the partial (98-episode) canonical-100 oracle_hint_action data, which remains valid but incomplete. This is NOT the 7/20 canonical-100 set. Episode selection methodology (unchanged from pure_oracle_hint_highsuccess100ep_20260811): scanned all 196 batch_logs/*/summary.tsv files project-wide (137 usable after excluding StreamVLN/older-format logs lacking an episode_id column and infra-failure rows), aggregated outbound_success by episode_id (the only stable cross-batch join key -- episode_idx verified to have zero id<->idx conflicts across all 297 distinct episode_ids seen), then took the top 100 by historical outbound success_rate (ties broken by more historical attempts). Weighted aggregate historical outbound success rate across the chosen 100: 884/932 = 94.85% attempts (vs ~30-38% for the canonical-100 set, and 72% for Route2's own 2026-07-25 reliability_v11_policy_v2_active_50ep_outbound_top_20260725 selection, which 46/50 overlaps with this list). Caveat: 52/100 have >=5 historical attempts backing that rate, 20/100 have 2-4, and 28/100 have only a single historical attempt (100% on n=1 -- statistically weak, flagged explicitly, not hidden). Only 30/100 overlap with the 7/20 canonical-100 manifest, so results from this batch are NOT episode-for-episode comparable to pure_navila_baseline_100ep_20260810 / pure_oracle_hint_100ep_20260811 / pure_oracle_hint_action_100ep_20260812 / any other canonical-100 batch -- this is a deliberately different, easier-outbound sample, testing hint_action_arbiter's effect on top of oracle_hint on easier episodes, not a replacement for the canonical-100 ablation chain." | tee -a "${LOG_DIR}/batch.log"
