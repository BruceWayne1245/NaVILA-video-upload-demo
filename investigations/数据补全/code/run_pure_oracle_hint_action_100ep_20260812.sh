...
PORT="${PORT:-54321}"
RUN_TAG="${RUN_TAG:-pure_oracle_hint_action_100ep_20260812}"
START_AT="${START_AT:-0}"
...
main() {
  ...
  echo "Pure oracle_hint_action (2026-08-12, final): identical to pure_oracle_hint_100ep_20260811 (--round_trip_mode=phase_prompt --instruction_rewriter_provider=cache_only --route_memory --route_hint_mode=compact --route_hint_source=oracle --route_relocalization_backend=none, same 100-episode canonical set) PLUS --hint_action_arbiter and --topdown_route_map. Deliberately NOT reproducing the full 2026-06-30/07-01 historical oracle_hint_action recipe (stop_gate_r3_hint_arbiter_hard11_20260630 + oracle_shadow_loftr_v4_30_return_anchor_fix_20260701, the source of the '4.3% intervention rate lifts return success ~50%->97%' finding), which additionally used --oracle_align_return_yaw_to_anchor_segment and --stop_gate(+r_in=3.0/r_out=3.0/confirm_steps=3/min_confidence=0.5). User explicitly excluded both after review (2026-08-12): --oracle_align_return_yaw_to_anchor_segment directly overwrites the robot's return-phase yaw from ground truth (a second, stronger oracle intervention on the robot's own pose, orthogonal to hint_action_arbiter's action-replacement mechanism) and --stop_gate independently arbitrates episode termination/success declaration (a 'when to stop' mechanism, unrelated to hint_action_arbiter's 'which way to move' mechanism, and the reason the plain oracle_hint batch avoids it in the first place -- oracle's constant relocalization_confidence=1.0 would let stop_gate's FORCED path auto-declare success). --topdown_route_map IS kept: it is not an independent decision mechanism, only a fallback obstacle-map data source for hint_action_arbiter's own existing clear-path check (used only if the primary LiDAR local-map check is unavailable), so keeping it stays consistent with a 'measure hint_action_arbiter's own effect' design while making the arbiter's clear-path signal as available as it was in every historical batch that used it. This is therefore NOT directly comparable to the historical 4.3%/97% number -- it isolates hint_action_arbiter (with its own most robust data source) on top of the plain oracle_hint condition, not a full reproduction of the historical bundle." | tee -a "${LOG_DIR}/batch.log"
  echo "Episode timeout: ${EPISODE_TIMEOUT_SECONDS}s (kill-after ${EPISODE_TIMEOUT_KILL_AFTER_SECONDS}s)" | tee -a "${LOG_DIR}/batch.log"

  # Same 100-episode canonical set as pure_oracle_hint_100ep_20260811 /
  # pure_navila_baseline_100ep_20260810 / canonical_report_next_stopgate_100ep_20260720
  # (byte-identical run_episode lines, only the eval command's flags differ).
