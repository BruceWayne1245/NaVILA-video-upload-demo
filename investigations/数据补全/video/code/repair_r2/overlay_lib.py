...
def draw_overlay(frame, row, rows, idx, config_label, speed_label):
    ...
    fs_bold = 0.35 if narrow else 0.48
    max_chars = 34 if narrow else 70
    panel = np.zeros((PANEL_H, w, 3), dtype=np.uint8)
    panel[:] = (18, 18, 18)

    hint = row.get("hint", "") or ""
    vlm_action = "VLM: " + (row.get("vlm_action_raw", "") or "")
    ...
    _put(panel, _fit_text(hint if hint else "(no route hint — language-only)", max_chars),
         (10, 24), fs, hint_color)
    vlm_safe = _ascii_safe(vlm_action)
    vlm_width = cv2.getTextSize(vlm_safe, FONT, fs, 1)[0][0]
    vlm_scale = fs * min(1.0, (w - 20) / max(1, vlm_width))
    _put(panel, vlm_action, (10, 48), vlm_scale, (255, 255, 255))
    ...
    if terminal_state:
        ...
        _put(panel, _fit_text(terminal_state, max_chars), (10, 96), fs_bold, color, thickness=1, shadow=False)
    ...
    if distance not in ("", None):
        try:
            ...
            _put(panel, f"d = {d:.2f} m", (dx, 24), fs_bold, dcolor, thickness=1, shadow=False)
            cv2.circle(panel, (cx, 55), r, dcolor, 1, cv2.LINE_AA)
            if d <= 3.0:
                cv2.circle(panel, (cx, 55), max(3, r // 3), dcolor, -1, cv2.LINE_AA)
        except ValueError:
            pass
    ...
    _put(out, config_label, (8, 264), fs, (255, 255, 0), thickness=1, shadow=False)
...
def draw_direction_arrows(out, w, h, original_deg, overridden_deg, progress):
    """Green marks the original direction; red marks the overridden direction.
    Zero degrees points straight ahead in the chase camera."""
    ...
    if overridden_deg is not None:
        cx = int(w * 0.42)
        cv2.arrowedLine(out, (cx, base_y), endpoint(cx, overridden_deg), (50, 50, 230), 3,
                         cv2.LINE_AA, tipLength=0.3)
    if original_deg is not None:
        cx = int(w * 0.58)
        cv2.arrowedLine(out, (cx, base_y), endpoint(cx, original_deg), (60, 200, 60), 3,
                         cv2.LINE_AA, tipLength=0.3)
    return out
...
def render_pair_side_by_side(video_path_l, csv_path_l, label_l,
    ...
    -- for selected override events -- direction arrows (green = original,
    red = overridden). The caller picks pause_raw_frames so that, once this piece's
    ...
    to show for it."""
    arrow_overrides = arrow_overrides or {}
    ...
    if pause_raw_frames > 0:
        ...
        for i, kind in keep:
            events_by_i[i] = kind
            il, ir = idx_pairs[i]
            event_step = int(rows_l[il]["step"]) if longer_is_l else int(rows_r[ir]["step"])
            print(f"PAUSE_EVENT {os.path.basename(out_path)} frame={i} step={event_step} kind={kind}", flush=True)
    ...
    for i in range(n_steps):
        ...
        if i in events_by_i:
            ...
            # These two retained override pauses have different sources for
            # the executed direction. Resolve each explicitly so the arrows
            # cannot inherit a color or source choice from another segment.
            arrows_l = arrows_r = None
            event_step = int(row_l["step"]) if longer_is_l else int(row_r["step"])
            arrow_override = arrow_overrides.get(event_step, {})
            if kind == "override":
                source = arrow_override.get("overridden_source")
                if source not in ("right_hint", "right_vlm"):
                    raise ValueError(f"Missing arrow semantics for override step {event_step}")
                original_deg = parse_vlm_bearing(row_l.get("vlm_action_raw"))
                overridden_deg = (parse_hint_bearing(row_r.get("hint")) if source == "right_hint"
                                  else parse_vlm_bearing(row_r.get("vlm_action_raw")))
                if original_deg is None or overridden_deg is None:
                    raise ValueError(f"Missing direction for override step {event_step}")
                arrows_l = (original_deg, None)
                arrows_r = (original_deg, overridden_deg)
