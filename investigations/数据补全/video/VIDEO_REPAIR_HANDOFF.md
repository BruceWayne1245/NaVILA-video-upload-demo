...
`final_v2_source_render_fix_20260920_r2_submission_720p.mp4` 是下方第二次修订成片的压缩副本。只缩放至 1280×720，并以 H.264 双遍编码压缩；没有重新剪辑、删帧或更改播放速度。原始 1920×1080 成片仍保留。

- 文件大小：18,033,319 字节（低于 20,000,000 字节）。
- 时长、帧率和帧数与原片一致：176.36 秒、25 fps、4,409 帧。原片无音轨，投稿版也无音轨。
- SHA-256：`929918db18cf6d3651089c54e64ffd0e690e4c8221ee2e61a89c8b97e8b6fc01`。
- 抽帧核对了约 28 秒、1:26、1:50 和结尾图表。重新编码会改变像素细节和清晰度，但不改变视频内容与时间顺序。
- 压缩参数：`libx264`、`preset=slow`、双遍目标视频码率 `820k`、`yuv420p`、Lanczos 缩放、`faststart`。
