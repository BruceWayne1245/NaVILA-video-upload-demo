...
class RelativeStartProgress:
    ...
    # 2026-07-27 Route-2 recovery provenance.  The target anchor identifies
    # where the vector points; these fields identify where its evidence came
    # from.  A geometry-reconstructed A1 vector sourced from trusted A2 must
    # not be judged as if it were A1's raw ICP output.
    estimate_kind: str = "raw_icp"
    estimate_source_anchor_index: Optional[int] = None
    estimate_edge_hop_count: int = 0
    estimate_source_confidence: Optional[float] = None
    estimate_target_raw_confidence: Optional[float] = None
    evidence_age_updates: Optional[int] = None
    # The terminal gate must distinguish the forward guide ("next") from the
    # already-passed rear support ("current").  A correct current-anchor match
    # is useful for recovery but is not, by itself, proof of distance home.
    estimate_role: str = "unknown"
...
class AnchorRelocalization:
    ...
    estimate_kind: str = "raw_icp"
    source_anchor_index: Optional[int] = None
    edge_hop_count: int = 0
    source_confidence: Optional[float] = None
    target_raw_confidence: Optional[float] = None
    evidence_update_count: Optional[int] = None
...
class RouteMemoryAgent:
    def __init__(
        ...
        sequential_pair_reconstructed_confidence_source_one_hop: bool = False,
        ...
        v11_consumer_guard: Optional[Callable[[str, int], bool]] = None,
        v11_promotion_evidence_policy: Optional[Callable[..., bool]] = None,
        v11_candidate_transition_shadow: Optional[Callable[..., None]] = None,
        ...
        **_: object,
        ...
        # Reliability V1.1 Policy V2 hook. The callback receives only a
        # high-consequence operation and the proposed authoritative anchor.
        # Raw candidates and reversible relocalization updates never pass
        # through this hook.
        self._v11_consumer_guard = v11_consumer_guard
        # Scheme-1 Stage 1 hook. This is invoked before a promotion vote enters
        # Route1's bounded history. The wired implementation is shadow-only,
        # so its return value remains byte-equivalent to the baseline vote.
        self._v11_promotion_evidence_policy = v11_promotion_evidence_policy
        # Scheme-1 execution-interface observer. The callback sees Route1's
        # selector state immediately after the Stage-1 evidence assessment,
        # but has no reference that can mutate Route1 state.
        self._v11_candidate_transition_shadow = (
            v11_candidate_transition_shadow
        )
        # Active v0 controller state is intentionally separate from Route1's
        # permanent quarantine set. The callback may atomically replace this
        # temporary set and suppress promotion of the just-quarantined next,
        # but it cannot mutate Route1's own evidence histories.
        self._v11_active_quarantined_anchor_indices: set[int] = set()
        # Route-2 anchor-support recovery keeps role identity independent from
        # Route1's exact-position `_target_anchor_index`.  These are guidance
        # pointers: rear support and forward guide may be non-adjacent.
        self._v11_support_current_index: Optional[int] = None
        self._v11_guidance_next_index: Optional[int] = None
        self._v11_probe_anchor_indices: tuple[int, ...] = ()
        self._v11_promotion_blocked_anchor_indices: set[int] = set()
        self._v11_raw_hint_blocked_anchor_indices: set[int] = set()
        self._v11_route_consumers_enabled: bool = True
        self._v11_vlm_only: bool = False
        self._v11_last_requested_pair: Optional[tuple[int, int]] = None
        self._v11_last_directive_requested_pair: Optional[tuple[int, int]] = None
        self._v11_reconstruct_requested_next: bool = False
        self._v11_reconstructed_next_min_distance_m: Optional[float] = None
        ...
        # Off by default.  Legacy closure reconstruction assigns
        # min(source_confidence, bad_target_confidence), which makes an
        # accurate current->next projection inherit the very raw target
        # failure it was built to replace.  The opt-in recovery path uses the
        # source reading's confidence for exactly one fixed edge and carries
        # explicit provenance so V1.1 can authorize the source by purpose.
        self.sequential_pair_reconstructed_confidence_source_one_hop = bool(
    ...
    def initialize_start_anchor_descriptor(
        self,
        descriptor: Optional[object],
        metadata: Optional[dict] = None,
    ) -> bool:
        """Attach the reset-time sensor observation to the synthetic A0.

        A0 is created in ``__init__`` before the evaluator has a sensor
        descriptor, so the historical implementation left it permanently
        ``descriptor=None``.  That makes A0 impossible to offer to ICP and
        removes the current/next pair exactly on the terminal A1->A0 segment.

        The evaluator calls this once from the initial, zero-command
        observation.  Only the descriptor and capture metadata are updated:
        A0's identity remains pinned to pose/distance zero, so no outbound
        odometry or anchor-to-anchor edge is changed.
        """
        if not self.enabled or descriptor is None or len(self.anchors) != 1:
            return False
        anchor = self.anchors[0]
        if (
            int(anchor.index) != 0
            or anchor.descriptor is not None
            or abs(float(anchor.distance_from_start_m)) > 1e-9
        ):
            return False
        anchor.descriptor = descriptor
        anchor.metadata.update(dict(metadata or {}))
        anchor.metadata["start_descriptor_initialized"] = True
        return True
    ...
    def finalize_outbound(
        ...
        self._v11_support_current_index = None
        self._v11_guidance_next_index = None
        self._v11_probe_anchor_indices = ()
        self._v11_promotion_blocked_anchor_indices.clear()
        self._v11_raw_hint_blocked_anchor_indices.clear()
        self._v11_route_consumers_enabled = True
        self._v11_vlm_only = False
        self._v11_last_requested_pair = None
        self._v11_last_directive_requested_pair = None
        self._v11_reconstruct_requested_next = False
        self._v11_reconstructed_next_min_distance_m = None
    ...
    def update_relocalization(
        ...
        if not estimates:
            return None
        legacy_estimates = estimates
        if self._v11_last_requested_pair is not None:
            requested_indices = set(self._v11_last_requested_pair)
            legacy_estimates = [
                estimate for estimate in estimates
                if int(estimate.anchor_index) in requested_indices
            ]
        # Expanded Route-2 probes are model evidence only.  Prevent every
        # legacy Route1 recorder (not just final selection) from interpreting
        # a probe as the active next role or quarantining it.
        self._record_next_anchor_stability(legacy_estimates)
        self._record_next_anchor_quality(legacy_estimates)
        self._record_next_anchor_reliability(legacy_estimates)
        self._record_current_anchor_reliability(legacy_estimates)

        estimate, observation, reject_reason = self._select_sequential_pair_relocalization(
            legacy_estimates
        )
        if estimate is None:
        ...
        self.relocalization_events.append({
            ...
            "source_anchor_index": estimate.source_anchor_index,
            "edge_hop_count": int(estimate.edge_hop_count),
            "source_confidence": estimate.source_confidence,
            "target_raw_confidence": estimate.target_raw_confidence,
            "evidence_update_count": estimate.evidence_update_count,
    ...
    def _v11_consumer_allows(self, operation: str, anchor_index: int) -> bool:
        if self._v11_consumer_guard is None:
            return True
        return bool(self._v11_consumer_guard(str(operation), int(anchor_index)))

    def _v11_integrated_promotion_vote(
        self,
        *,
        ...
        if self._v11_promotion_evidence_policy is None:
            return bool(baseline_vote)
        return bool(self._v11_promotion_evidence_policy(
    ...
    def _v11_integrated_candidate_transition_observe(
        self,
        *,
        current_anchor_index: int,
        next_anchor_index: int,
    ) -> bool:
        if self._v11_candidate_transition_shadow is None:
            return False
        directive = self._v11_candidate_transition_shadow(
            current_anchor_index=int(current_anchor_index),
            next_anchor_index=int(next_anchor_index),
            available_anchor_indices=tuple(
                int(anchor.index) for anchor in self.anchors
            ),
            route1_quarantined_anchors=tuple(sorted(
                int(index) for index in self._quarantined_anchor_indices
            )),
            route1_quarantine_budget_used=int(
                self._reliability_quarantines_since_promotion
            ),
            route1_quarantine_budget_limit=int(
                self.reliability_quarantine_max_chain
            ),
        )
        if directive is None:
            return False
        if not isinstance(directive, dict):
            raise RuntimeError("invalid V2 candidate-controller directive")
        replacement = {
            int(index)
            for index in directive.get(
                "active_quarantined_anchors", ()
            )
        }
        if any(
            index < 0 or index >= int(current_anchor_index)
            for index in replacement
        ):
            raise RuntimeError(
                "V2 active quarantine violates descending-anchor identity"
            )
        if len(replacement) > self.reliability_quarantine_max_chain:
            raise RuntimeError(
                "V2 active quarantine exceeds Route1 chain budget"
            )
        if directive.get("controller_effect") is True:
            self._v11_active_quarantined_anchor_indices = replacement
        suppress_promotion = bool(
            directive.get("suppress_promotion", False)
        )
        if (
            suppress_promotion
            and int(next_anchor_index)
            not in self._v11_active_quarantined_anchor_indices
        ):
            raise RuntimeError(
                "V2 promotion suppression requires quarantined next"
            )
        return suppress_promotion
    ...
    def _select_sequential_pair_relocalization(
        ...
        requested_pair = self._v11_last_requested_pair
        if requested_pair is not None:
            requested_indices = set(requested_pair)
            # Expanded candidates are probes for Route-2 only.  They must not
            # enter Route1 closure/promotion or silently acquire a pair role.
            estimates = [
                estimate for estimate in estimates
                if int(estimate.anchor_index) in requested_indices
            ]
        ...
        if requested_pair is not None:
            current_idx, next_idx = requested_pair
        else:
        ...
        if current_est is not None:
            current_est = replace(
                current_est,
                evidence_update_count=int(self._return_update_count),
            )
        ...
        if (
            self._v11_reconstruct_requested_next
            and self._v11_last_directive_requested_pair == (current_idx, next_idx)
            and current_est is not None
        ):
            projected = self._project_estimate_to_anchor(current_est, next_idx)
            if projected is not None:
                next_est = replace(
                    projected,
                    backend=f"{current_est.backend}+v11_support_reconstructed",
                    estimate_kind="geometry_reconstructed",
                    source_anchor_index=int(current_idx),
                    edge_hop_count=abs(int(current_idx) - int(next_idx)),
                    source_confidence=float(current_est.confidence),
                    target_raw_confidence=(
                        float(next_est.confidence) if next_est is not None else None
                    ),
                    evidence_update_count=int(self._return_update_count),
                )
        ...
        if gate_next_est is not None and next_est is not None:
            ...
            pre_closure_candidate_promote = bool(
                quality_ok and (close_enough or trend_ok or gate_current_est is None)
            )
            candidate_promote = pre_closure_candidate_promote
            ...
            candidate_promote = self._v11_integrated_promotion_vote(
                current_anchor_index=current_idx,
                next_anchor_index=next_idx,
                pre_closure_vote=pre_closure_candidate_promote,
                baseline_vote=candidate_promote,
                closure_rejected=closure_reject_reason is not None,
            )
            v11_suppress_promotion = (
                self._v11_integrated_candidate_transition_observe(
                    current_anchor_index=current_idx,
                    next_anchor_index=next_idx,
                )
            )
            ...
            if withhold_for_unresolved_short_baseline:
                promote = False
            if v11_suppress_promotion:
                promote = False

        if next_idx in self._v11_promotion_blocked_anchor_indices:
            promote = False

        if promote and next_est is not None:
            promote = self._v11_consumer_allows(
                "anchor_promotion", int(next_idx)
            )

        if promote and next_est is not None:
            selected = next_est
            self._target_anchor_index = int(next_idx)
            if self._v11_support_current_index is not None:
                # Normal trusted progression advances both guidance roles.
                # This is the only path that lets a next become current;
                # promotion-blocked bad anchors were forced to `promote=False`
                # above and therefore can never enter this branch.
                self._v11_support_current_index = int(next_idx)
                self._v11_guidance_next_index = (
                    int(next_idx) - 1 if int(next_idx) > 0 else int(next_idx)
                )
                self._v11_reconstruct_requested_next = False
                self._v11_reconstructed_next_min_distance_m = None
            ...
            self._v11_active_quarantined_anchor_indices.clear()
        ...
        if next_est is not None:
            raw_hint_blocked = (
                int(next_est.anchor_index)
                in self._v11_raw_hint_blocked_anchor_indices
                and next_est.estimate_kind != "geometry_reconstructed"
            )
            if raw_hint_blocked:
                self._latest_next_candidate_relocalization = None
            else:
                self._latest_next_candidate_relocalization = next_est
                self._next_candidate_update_seq += 1
            if (
                next_est.estimate_kind == "geometry_reconstructed"
                and self._v11_guidance_next_index == int(next_idx)
            ):
                distance = float(next_est.distance_to_anchor_m)
                minimum = self._v11_reconstructed_next_min_distance_m
                self._v11_reconstructed_next_min_distance_m = (
                    distance if minimum is None else min(minimum, distance)
                )
                minimum = self._v11_reconstructed_next_min_distance_m
                if (
                    minimum <= self.anchor_pass_max_min_distance_m
                    and distance >= minimum + self.anchor_pass_hysteresis_m
                    and int(next_idx) > 0
                ):
                    # The bad guide was passed using trusted-current geometry.
                    # Keep the rear support fixed and move only the forward
                    # role.  Promotion remains blocked for the bad anchor.
                    self._v11_guidance_next_index = int(next_idx) - 1
                    self._v11_reconstruct_requested_next = False
                    self._v11_reconstructed_next_min_distance_m = None
    ...
    def sequential_target_anchor_pair(self) -> tuple[Optional[RouteAnchor], Optional[RouteAnchor]]:
        ...
        """
        if (
            self._v11_support_current_index is not None
            and self._v11_guidance_next_index is not None
        ):
            current_index = int(self._v11_support_current_index)
            next_index = int(self._v11_guidance_next_index)
            self._v11_last_requested_pair = (current_index, next_index)
            return (
                self._anchor_by_index(current_index),
                self._anchor_by_index(next_index),
            )
        ...
        next_index = self._next_candidate_index(current_index)
        next_anchor = self._anchor_by_index(next_index)
        self._v11_last_requested_pair = (int(current_index), int(next_index))
        return current, next_anchor

    def sequential_probe_anchors(self) -> tuple[RouteAnchor, ...]:
        """Expanded Route-2 probes, independent of promotion/hint blocks."""
        pair = set(self._v11_last_requested_pair or ())
        anchors = []
        for index in self._v11_probe_anchor_indices:
            if index in pair:
                continue
            anchor = self._anchor_by_index(index)
            if anchor is not None:
                anchors.append(anchor)
        return tuple(anchors)

    @property
    def route_consumers_enabled(self) -> bool:
        return bool(self._v11_route_consumers_enabled)

    @property
    def v11_anchor_support_state(self) -> dict:
        return {
            "current_anchor_index": self._v11_support_current_index,
            "next_anchor_index": self._v11_guidance_next_index,
            "probe_anchor_indices": list(self._v11_probe_anchor_indices),
            "promotion_blocked_anchors": sorted(
                self._v11_promotion_blocked_anchor_indices
            ),
            "raw_hint_blocked_anchors": sorted(
                self._v11_raw_hint_blocked_anchor_indices
            ),
            "route_consumers_enabled": self._v11_route_consumers_enabled,
            "vlm_only": self._v11_vlm_only,
        }

    def apply_v11_anchor_support_directive(
        self,
        directive: object,
        *,
        requested_pair: tuple[int, int],
    ) -> None:
        """Atomically apply Route-2 guidance state for subsequent attempts."""
        field = (
            lambda name: directive[name]
            if isinstance(directive, dict)
            else getattr(directive, name)
        )
        current = int(field("current_anchor_index"))
        next_anchor = int(field("next_anchor_index"))
        if self._anchor_by_index(current) is None:
            raise RuntimeError(f"unknown V11 support current anchor {current}")
        if self._anchor_by_index(next_anchor) is None:
            raise RuntimeError(f"unknown V11 guidance next anchor {next_anchor}")
        self._v11_support_current_index = current
        self._v11_guidance_next_index = next_anchor
        self._v11_probe_anchor_indices = tuple(
            int(index) for index in field("probe_anchor_indices")
            if self._anchor_by_index(int(index)) is not None
        )
        self._v11_promotion_blocked_anchor_indices = {
            int(index) for index in field("promotion_blocked_anchors")
        }
        self._v11_raw_hint_blocked_anchor_indices = {
            int(index) for index in field("raw_hint_blocked_anchors")
        }
        latest_next = self._latest_next_candidate_relocalization
        if (
            latest_next is not None
            and int(latest_next.anchor_index)
            in self._v11_raw_hint_blocked_anchor_indices
            and latest_next.estimate_kind != "geometry_reconstructed"
        ):
            # The directive is applied after this attempt's raw ICP selection.
            # Remove that just-rejected reading immediately so no VLM query in
            # the interval before the next reconstruction can consume it.
            self._latest_next_candidate_relocalization = None
        self._v11_route_consumers_enabled = bool(
            field("route_consumers_enabled")
        )
        self._v11_vlm_only = bool(field("vlm_only"))
        self._v11_last_directive_requested_pair = tuple(
            int(index) for index in requested_pair
        )
        self._v11_reconstruct_requested_next = bool(
            field("reconstruct_next_from_current")
        )
        if not self._v11_reconstruct_requested_next:
            self._v11_reconstructed_next_min_distance_m = None
    ...
    def _next_candidate_index(self, current_index: int) -> int:
        ...
        if (
            ...
            or self._v11_active_quarantined_anchor_indices
        ):
            combined_quarantines = (
                self._quarantined_anchor_indices
                | self._v11_active_quarantined_anchor_indices
            )
            while next_index in combined_quarantines and next_index >= 0:
                next_index -= 1
        return next_index
    ...
    def propose_hint(
        ...
        """Build the baseline hint proposal without recording it as emitted.

        Freshness suppression belongs to baseline RouteMemoryAgent semantics
        and therefore runs before any downstream consumer guard.  The caller
        may inspect/guard the returned proposal, then call
        :meth:`commit_hint_event` only if the hint is actually injected.
        """
        ...
        # VLM query, not per environment step.
        suppress_stale = (
    ...
    def commit_hint_event(self, event: dict) -> None:
        """Record a proposal only after its final consumer guard allows it."""
    ...
    def inject_hint(
        ...
        """Backward-compatible one-shot baseline hint injection."""
        instruction, event = self.propose_hint(
            base_instruction,
            step,
            progress_override=progress_override,
        )
        if event is not None:
            self.commit_hint_event(event)
    ...
    def _coerce_relocalization(
        ...
        return AnchorRelocalization(
            ...
            estimate_kind=str(
                relocalization.get("estimate_kind", "raw_icp")
            ),
            source_anchor_index=relocalization.get("source_anchor_index"),
            edge_hop_count=int(relocalization.get("edge_hop_count", 0)),
            source_confidence=relocalization.get("source_confidence"),
            target_raw_confidence=relocalization.get(
                "target_raw_confidence"
            ),
            evidence_update_count=relocalization.get(
    ...
    def _sequential_pair_closure_precheck(
        ...
        if b_quality > 1.5 * a_quality:
            reconstructed_a = self._geometry_reconstructed_estimate(
                target=a,
                source=b,
                dx=dx,
                dy=dy,
                dtheta=dtheta,
                backend_suffix="closure_reconstructed",
            )
        ...
        if a_quality > 1.5 * b_quality:
            ...
            reconstructed_b = self._geometry_reconstructed_estimate(
                target=b,
                source=a,
                dx=rdx,
                dy=rdy,
    ...
    def _geometry_reconstructed_estimate(
        self,
        *,
        target: AnchorRelocalization,
        source: AnchorRelocalization,
        dx: float,
        dy: float,
        dtheta: float,
        backend_suffix: str,
    ) -> AnchorRelocalization:
        """Build a derived target estimate with auditable one-hop provenance."""
        hop_count = abs(int(source.anchor_index) - int(target.anchor_index))
        confidence = float(min(source.confidence, target.confidence))
        if (
            self.sequential_pair_reconstructed_confidence_source_one_hop
            and hop_count == 1
        ):
        ...
        return replace(
            ...
            backend=f"{target.backend}+{backend_suffix}",
            estimate_kind="geometry_reconstructed",
            source_anchor_index=int(source.anchor_index),
            edge_hop_count=hop_count,
            source_confidence=float(source.confidence),
            target_raw_confidence=float(target.confidence),
        )
    ...
    def _sequential_pair_closure_belief_trust_aware_reconstruct(
        ...
        if b_trustworthy:
            reconstructed_a = self._geometry_reconstructed_estimate(
                ...
                dx=b_dx_in_a_frame,
                dy=b_dy_in_a_frame,
                dtheta=b_dtheta_in_a_frame,
                backend_suffix="belief_trust_aware_reconstructed",
            )
            return [reconstructed_a if e is a else e for e in estimates], None
        reprojected_a = self._reproject_delta_to_anchor(
            a.anchor_index, a.anchor_dx_m, a.anchor_dy_m, a.anchor_dtheta_rad, b.anchor_index,
    ...
    def _anchor_progress_from_estimate(
        ...
        *,
        ...
        evidence_age_updates = (
            max(
                0,
                int(self._return_update_count)
                - int(estimate.evidence_update_count),
            )
            if estimate.evidence_update_count is not None
            else None
        )
        return RelativeStartProgress(
            target_dx_m=float(dx),
            ...
            estimate_kind=str(estimate.estimate_kind),
            estimate_source_anchor_index=estimate.source_anchor_index,
            estimate_edge_hop_count=int(estimate.edge_hop_count),
            estimate_source_confidence=estimate.source_confidence,
            estimate_target_raw_confidence=estimate.target_raw_confidence,
            evidence_age_updates=evidence_age_updates,
            estimate_role=str(estimate_role),
        )
    ...
    def _anchor_progress(self) -> Optional[RelativeStartProgress]:
        ...
        if self.sequential_pair_report_next_anchor and self._latest_next_candidate_relocalization is not None:
            ...
            estimate = self._latest_relocalization
            estimate_role = "current"
        if estimate is None:
            return None
        return self._anchor_progress_from_estimate(
            estimate,
            estimate_role=estimate_role,
        )
